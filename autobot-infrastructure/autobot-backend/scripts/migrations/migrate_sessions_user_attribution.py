...
class SessionMigrator:
    ...
    async def migrate_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            key = f"chat:session:{session_id}"
            await self.redis_client.set(key, json.dumps(session_data))

            # Register ownership in Redis indices
            await self._register_ownership(session_id, owner_id, metadata)

            logger.info(f"Migrated session {session_id} with owner: {owner_id}")
            self.stats["migrated"] += 1
            return True
    ...
    async def _register_ownership(
        ...
        try:
            ...
            logger.debug(f"Registered ownership for {session_id}: {owner_id}")
        except Exception as e:
            logger.warning(f"Failed to register ownership for {session_id}: {e}")
