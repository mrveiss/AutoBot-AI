    ...
    async def backfill_message_user_ids(
        ...
        try:
            ...
            for start in range(0, message_count, batch_size):
                ...
                messages_data = await self.redis_client.lrange(messages_key, start, end)
        ...
        try:
            ...
            for activity_key in activity_keys:
                try:
                    ...
                    logger.warning(f"Failed to process activity {activity_key}: {e}")
            ...
            logger.error(f"Failed to backfill activities for {session_id}: {e}")
            return 0
    ...
    async def run(self) -> None:
        ...
        for session_id in session_ids:
            ...
            if not session_owner:
                logger.warning(f"Session {session_id} has no owner, skipping")
                continue
