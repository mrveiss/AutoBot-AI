...
def discover_files(root_dir: Path, tier: Optional[int] = None) -> List[Tuple[str, int]]:
    """
...
def _handle_incremental_mode(
    ...
    logger.info(f"Incremental mode: {len(files)} changed, {unchanged_count} unchanged")
    if not files:
...
async def index_documentation(
    ...
    summary = _build_indexing_summary(files, results, elapsed, dry_run, incremental)
    return summary
