...
def _create_new_index(redis_client):
    ...
    try:
        result = redis_client.execute_command(*LLAMA_INDEX_CREATE_COMMAND)
...
async def main():
    """Fix Redis index dimensions to match existing vector data."""
    try:
        logger.info("🔧 Fixing Redis index dimensions to match " "existing vectors...")

        redis_client = get_redis_client(database="knowledge", async_client=False)
        logger.info(f"✅ Connected to Redis: {redis_client.ping()}")

        _log_index_info(redis_client, "Current index info", ["num_docs", "dim"])
