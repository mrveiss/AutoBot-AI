...
class TestingCoverageAnalyzer:
    ...
    def __init__(self, redis_client=None):
        ...
        # Test file patterns
        self.test_patterns = [r"test_.*\.py$", r".*_test\.py$", r"tests/.*\.py$"]

        # Test function patterns
        self.test_function_patterns = [r"^test_", r"_test$", r"^should_"]
    ...
    async def _analyze_coverage_gaps(
        ...
        [f for f in functions if f.complexity > 5 and f.is_public]
        [t for t in tests if t.covers_edge_cases]
    ...
    def _is_critical_function(self, func: CodeFunction) -> bool:
        ...
        return (
            func.calls_external_apis
            or func.has_database_operations
            or func.has_file_operations
            or func.complexity > 8
            or "auth" in func.name.lower()
            or "security" in func.name.lower()
        )
    ...
    def _is_high_priority_function(self, func: CodeFunction) -> bool:
        ...
        return (
            func.complexity > 3
            or len(func.parameters) > 3
            or func.is_async
            or "process" in func.name.lower()
            or "handle" in func.name.lower()
            or "execute" in func.name.lower()
        )
    ...
    def _calculate_testing_metrics(
        self,
...
async def main():
    ...
    results = await analyzer.analyze_testing_coverage(
        root_path=".", patterns=["src/**/*.py", "backend/**/*.py", "tests/**/*.py"]
    )
