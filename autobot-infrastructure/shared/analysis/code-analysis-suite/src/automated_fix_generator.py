...
def _build_security_templates():
    ...
    return [
        ...
        FixTemplate(
            pattern=(
                r'subprocess\.(?:run|call|Popen)\s*\([\'"]' r'([^\'"]*)\{([^}]+)\}([^\'"]*)[\'"]\s*,\s*shell=True\)'
            ),
...
def _build_performance_templates():
    ...
    return [
        FixTemplate(
            pattern=(r"open\s*\(\s*([^)]+)\s*\)" r"(?!\s*(?:with|\.close\(\)|as\s+))"),
                ...
                if files:
                    ...
                    fixes.append(
                        CodeFix(
                            ...
                            explanation=(
                                f'Found {group.get("count")} similar code blocks. ' "Extract common functionality."
                            ),
                            confidence=0.7,
                            risk_level="medium",
