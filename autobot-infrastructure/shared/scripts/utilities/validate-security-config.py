...
def _test_auth_and_audit(security) -> bool:
    ...
    if not security.enable_auth:
        logger.error("Authentication is DISABLED - SECURITY VULNERABILITY!")
        return False
...
def _test_allowed_users(security) -> bool:
    ...
    logger.info("   Number of allowed users: %s", len(security.allowed_users))
...
def _test_roles_config(security) -> bool:
    ...
    for role in expected_roles:
        ...
        logger.info("      %s permissions defined for '%s'", len(perms), role)
    return True
...
def test_security_config():
    ...
    try:
        ...
        logger.info("   Users configured: %s", len(security.allowed_users))
