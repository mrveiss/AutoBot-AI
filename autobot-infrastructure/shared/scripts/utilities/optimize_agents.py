...
class AgentOptimizer:
    ...
    def print_statistics(self) -> None:
        ...
        if "total_savings_bytes" in self.stats:
            print(
                f"Total savings:          {self.stats['total_savings_bytes']:,} bytes "
...
def commit_optimization(stats: Dict[str, any]) -> None:
    ...
    try:
        ...
        # Add modified files
        subprocess.run(["git", "add", ".claude/agents/"], check=True, env=scrubbed_git_env())
...
Generated by: scripts/utilities/optimize_agents.py"""
        ...
        # Commit changes
        subprocess.run(["git", "commit", "-m", message], check=True, env=scrubbed_git_env())
...
def restore_agents(tag_name: Optional[str] = None) -> None:
    ...
    try:
        ...
        subprocess.run(["git", "restore", ".claude/agents/", f"--source={tag_name}"], check=True, env=scrubbed_git_env())
