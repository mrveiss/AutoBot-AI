...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[ServiceURLs.FRONTEND_LOCAL, "http://127.0.0.1:5173"],
...
async def execute_workflow(request: dict):
    ...
    if user_message.lower().strip() in ["hello", "hi", "hey"]:
        return {
            ...
            "result": {
                "status": "success",
                "response_text": (
                    "Hello! I'm AutoBot's minimal backend. The main backend had performance issues, "
                    "but this proves the basic functionality works. What can I help you with?"
                ),
            },
        }
    ...
    return {
        ...
        "result": {
            "status": "success",
            "response_text": (
                f"You said: '{user_message}'. This is a minimal backend response - "
                "the full backend is having performance issues."
            ),
        },
    }
