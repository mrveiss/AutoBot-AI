...
class NPUPerformanceMeasurement:
    ...
    async def _run_benchmark_steps(self, results: Dict[str, Any]):
        ...
        results["semantic_search_tests"] = await self._test_semantic_search_engine()
    ...
    async def _test_cpu_embedding_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_text = "This is a test sentence for CPU embedding performance."
            ...
            cpu_results["batch_times"] = self._run_batch_embedding_test(
                chunker._compute_sentence_embeddings,
                [1, 5, 10, 25, 50],
            )
    ...
    async def _test_gpu_embedding_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_text = "This is a test sentence for GPU embedding performance."
    ...
    async def _test_npu_embedding_generation(
        ...
        try:
            ...
            for batch_size in [5, 10, 25]:
                ...
                embedding_results["batch_embeddings"][str(batch_size)] = batch_result

            embedding_results["performance_metrics"] = self._calc_embedding_metrics(
                    ...
                    search_tests.append(
                        {
                            "query": query,
                            "response_time_ms": elapsed,
                            "api_search_time_ms": data.get("search_time_ms", 0),
    ...
    async def _generate_recommendations(
        ...
        try:
            ...
            recommendations.extend(self._get_npu_availability_recs(npu_worker))
            ...
            if hardware_util:
                recommendations.extend(self._get_hardware_util_recs(hardware_util))
    ...
    def _log_hardware_summary(self, results: Dict[str, Any]):
        ...
        logger.info("   CPU Cores: %s", cpu_info.get("cpu_count_logical", "Unknown"))
        logger.info("   Memory: %s GB", cpu_info.get("memory_total_gb", "Unknown"))
        logger.info("   GPU Available: %s", gpu_info.get("gpu_available", False))
        if gpu_info.get("gpu_available"):
            logger.info("   GPU: %s", gpu_info.get("gpu_name", "Unknown"))
            logger.info(
                "   GPU Memory: %s GB",
    ...
    def _log_npu_and_comparison_summary(self, results: Dict[str, Any]):
        ...
        logger.info("   Connected: %s", connectivity.get("success", False))
...
async def main():
    ...
    logger.info("This will comprehensively test NPU semantic search acceleration")
