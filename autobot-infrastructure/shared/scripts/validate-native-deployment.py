...
class DeploymentValidator:
    def __init__(self):
        ...
        self.services = [
            ServiceCheck(
                "WSL Backend", "172.16.168.20", 8001, "http", "/api/health", None, 10
            ),
            ...
            ServiceCheck(
                "NPU Worker", "172.16.168.22", 8081, "http", "/health", None, 15
            ),
            ServiceCheck(
                "Redis Stack", "172.16.168.23", 6379, "redis", None, "PONG", 5
            ),
            ServiceCheck("RedisInsight", "172.16.168.23", 8002, "http", "/", None, 10),
            ServiceCheck(
                "AI Stack", "172.16.168.24", 8080, "http", "/health", None, 15
            ),
            ServiceCheck(
                "Ollama", "172.16.168.24", 11434, "http", "/api/tags", None, 20
            ),
            ServiceCheck(
                "Browser Service", "172.16.168.25", 3000, "http", "/health", None, 10
            ),
        ]
        ...
        try:
            r = redis.Redis(
                ...
                password=os.environ.get(
                    "REDIS_PASSWORD", os.environ.get("AUTOBOT_REDIS_PASSWORD", "")
                ),
    ...
    async def test_backend_vm_connectivity(self) -> Dict:
        ...
        try:
            ...
            connectivity_tests.append(
                {"test": "Backend → Redis", "status": "failure", "message": str(e)}
            )
