...
declare -A SERVICES=(
    ...
    ["Redis VM"]="${AUTOBOT_REDIS_HOST:-localhost}:${AUTOBOT_REDIS_PORT:-6379}"
...
echo "  Frontend: http://${AUTOBOT_FRONTEND_HOST:-localhost}:${AUTOBOT_FRONTEND_PORT:-5173}"
...
echo "  Main WSL (${AUTOBOT_BACKEND_HOST:-localhost}): Backend API + Ollama + VNC"
echo "  Frontend VM (${AUTOBOT_FRONTEND_HOST:-localhost}): Vue.js Web Interface"
echo "  NPU Worker VM (${AUTOBOT_NPU_WORKER_HOST:-localhost}): Intel OpenVINO + Hardware Acceleration"
echo "  Redis VM (${AUTOBOT_REDIS_HOST:-localhost}): Redis Stack + Vector Storage"
echo "  AI Stack VM (${AUTOBOT_AI_STACK_HOST:-localhost}): AI Processing Services"
echo "  Browser VM (${AUTOBOT_BROWSER_SERVICE_HOST:-localhost}): Playwright Automation"
