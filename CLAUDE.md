...
- **Security reviews are findings-first:** read the diff only, open with a one-line verdict (`SAFE` / `ISSUES FOUND` / `NEEDS DEEPER LOOK`), then a severity/`file:line`/issue/fix table — all within 3 tool calls; verify *after*, and ask before widening into the codebase (inside a `/loop`, post the question and continue) — never explore before the verdict lands (skill: `secreview`)
- **Long analyses go to a file, not the response:** start every one in a scratch path *outside* the repo, written incrementally as produced. **Filing an umbrella issue or a design doc is the trigger** to move it into `docs/research/<topic>.md` (or `docs/audit/`, `docs/design/`) and cross-link it both ways — a one-directional move on an observable event, never an up-front guess at eventual size. The reply is the path plus a short summary; an interrupted or token-capped response must never lose the work
