...
- **PR queue limit is 6 open PRs:** below 6, solve issues one by one without deferring — every PR still gets a `code-reviewer` pass before merge, and PRs merge as their CI finishes. At 6 or more the tick **drains** (review → merge → close with evidence) instead of starting new work; `~/.claude/scripts/backlog-governor.sh status` is the authority, not a judgement call. Issue *filing* is rate-limited by the same verdict and parked verbatim when blocked; PRs, comments and closures never are.
