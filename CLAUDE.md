...
- **Security reviews are findings-first:** read the diff only, open with a one-line verdict (`SAFE` / `ISSUES FOUND` / `NEEDS DEEPER LOOK`), then a severity/`file:line`/issue/fix table — all within 3 tool calls; verify *after*, and ask before widening into the codebase — never explore before the verdict lands (skill: `secreview`)
- **Long analyses go to a file, not the response — scale decides where:** research yielding only **1-2 issues** goes to a scratch path *outside* the repo; research that produces an **umbrella issue or a design doc** is committed to `docs/research/<topic>.md` (or `docs/audit/`, `docs/design/`) and cross-linked with its umbrella. Either way it is written incrementally as produced; the reply is the path plus a short summary — an interrupted or token-capped response must never lose the work
...
- **PR queue limit is 6 open PRs:** below 6, solve issues one by one without deferring — every PR still gets a `code-reviewer` pass before merge, and PRs merge as their CI finishes. At 6 or more the tick **drains** (review → merge → close with evidence) instead of starting new work. Count it with `gh pr list --state open --json number --jq length` — a number, not a judgement call. Issue *filing* is throttled by the same threshold and any blocked filing is parked verbatim, never dropped; PRs, comments and closures are never throttled.
