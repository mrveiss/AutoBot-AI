...
def get_github_permalink(file_path: str, line: int | None, commit_ref: str = "main") -> str:
...
def format_status_emoji(status: str) -> str:
    ...
    status_map = {"wired": "✅", "partial": "⚠️", "broken": "❌"}
...
def group_claims_by_category(claims: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    categories = {"infrastructure": [], "api": [], "features": [], "architecture": []}
...
def generate_report(inventory: Dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "claude /claims-audit",
            "```\n",
            "The `/claims-audit` skill at "
            "[`.claude/skills/claims-audit/SKILL.md`](../.claude/skills/claims-audit/SKILL.md):",
...
def main():
    ...
    parser.add_argument(
        "--inventory", type=Path, default=Path("docs/verification-inventory.json"), help="Path to inventory JSON file"
    )
    parser.add_argument(
        "--output", type=Path, default=Path("docs/verification.md"), help="Path to output verification.md file"
    )
    ...
    # A CLI's result summary: stdout is its interface, not a log stream.
    summary = inventory["summary"]
    sys.stdout.write(
        f"✅ Generated {args.output}\n"
        f"   {summary['total']} claims verified\n"
        f"   ✅ {summary['wired']} wired\n"
        f"   ⚠️  {summary['partial']} partial\n"
