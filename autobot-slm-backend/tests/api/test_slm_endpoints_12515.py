...
# ---------------------------------------------------------------------------
# #15549 — the fire-and-forget replication job must own its session
# ---------------------------------------------------------------------------
...
def _load_real_replication_service():
    """Exec the real ``services/replication.py`` once and return its singleton.

    The root conftest stubs ``services.replication`` as a ``MagicMock``, and a
    MagicMock accepts any call signature — so every assertion about *which*
    session the background job uses would pass vacuously against it.  The
    property under test belongs to the real coroutine, so the module is exec'd
    here under the real sqlalchemy/models swap (the idiom ``_build_real_modules``
    already uses for ``services.code_status``) and ``sys.modules`` is put back
    afterwards, leaving the stub in place for every other test.

    ``ReplicationService.__init__`` calls ``Path(settings.ansible_dir)``, which
    a MagicMock attribute cannot satisfy; a throwaway path is substituted for
    the exec and restored immediately.  Nothing writes there: every Ansible/SSH
    entry point is replaced per-test.
    """
    if _REAL_REPLICATION_SERVICE:
        return _REAL_REPLICATION_SERVICE[0]

    saved_module = sys.modules.get("services.replication")
    ...
    try:
        with _real_modules_swapped():
            module = _load_real_module("services.replication", _SLM_ROOT / "services" / "replication.py")
        _REAL_REPLICATION_SERVICE.append(module.replication_service)
        return module.replication_service
    finally:
        settings.ansible_dir = saved_ansible_dir
        if saved_module is not None:
            sys.modules["services.replication"] = saved_module
        else:
            sys.modules.pop("services.replication", None)


# Loaded on first use, not at import: a failure here must cost the two #15549
# tests below, not collection of this whole module.
_REAL_REPLICATION_SERVICE: list = []
...
class _RequestScopedSession:
    ...
    FastAPI closes the ``Depends(get_db)`` session as soon as the response is
    sent.  SQLAlchemy does not enforce that: a later ``execute()`` on a closed
    session silently checks a *fresh* connection out of the pool and succeeds,
    which is precisely why #15549 was a nondeterministic corruption rather than
    a loud failure.  This proxy makes the contract explicit — every attribute
    reaches the real session until ``close()``, and any database call after
    that is recorded and raises.
    """
...
class _JobSessionFactory:
    """Stand-in for ``services.database.db_service`` that records what it opens.

    ``session()`` mirrors ``DatabaseService.session`` (commit on clean exit,
    close in ``finally``) so the job under test runs against the same lifecycle
...
async def replication_env():
    ...
    try:
        ...
        await _close_session_and_engine(request_session._session, engine)


def _install_replication_stubs(monkeypatch, job_sessions):
    """Point the router at the real service and neutralise its Ansible/SSH legs."""
    service = _load_real_replication_service()
    ...
    monkeypatch.setattr(_stateful, "replication_service", service)
    ...
    # Would otherwise schedule a real 30s polling task that outlives the test.
...
class TestReplicationJobOwnsItsSession:
    ...
    async def test_job_runs_on_its_own_session_after_request_teardown(self, replication_env, monkeypatch):
        ...
        This pins the *lifetime* property, not a call shape: the coroutine
        handed to ``fire_and_forget`` is captured instead of scheduled, the
        request-scoped session is then closed exactly as FastAPI's dependency
        teardown closes it, and only afterwards is the coroutine awaited.

        Against the pre-#15549 code the endpoint passed ``db`` and two ORM rows
        bound to it straight into ``setup_replication``, so the first
        ``db.execute`` inside the job lands on the torn-down session and
        ``_RequestScopedSession`` raises — and ``opened`` stays empty because
        the job never asked ``db_service`` for a session of its own.
        """
        request_session, job_sessions, maker = replication_env
        _install_replication_stubs(monkeypatch, job_sessions)

        captured: list = []
        monkeypatch.setattr(_stateful, "fire_and_forget", lambda coro, **_: captured.append(coro))

        await _insert_node(request_session, "n-src", ip="10.0.0.61")
        await _insert_node(request_session, "n-tgt", ip="10.0.0.62")
        ...
        # FastAPI dependency teardown — the request's session is done.
        ...
        # The work really landed, read back through a third, independent session.
        verifier = maker()
        try:
            row = (
                await verifier.execute(_select(Replication).where(Replication.replication_id == created.replication_id))
            ).scalar_one()
    ...
    async def test_no_orm_object_crosses_the_background_boundary(self, replication_env, monkeypatch):
        """``start_replication`` hands the job plain data only.

        An ORM row loaded through the request session is detached the moment
        that session is closed, so passing one across the boundary is the same
        defect as passing the session itself even when the session is not
        touched again.
        """
        request_session, job_sessions, _maker = replication_env
        service = _install_replication_stubs(monkeypatch, job_sessions)
        ...
        monkeypatch.setattr(service, "setup_replication", _record)
        captured: list = []
        monkeypatch.setattr(_stateful, "fire_and_forget", lambda coro, **_: captured.append(coro))

        await _insert_node(request_session, "n-src2", ip="10.0.0.63")
        await _insert_node(request_session, "n-tgt2", ip="10.0.0.64")
        ...
        handed_over = list(recorded["args"]) + list(recorded["kwargs"].values())
        ...
        assert {"n-src2", "n-tgt2"} <= set(v for v in handed_over if isinstance(v, str))
