...
from autobot_shared import async_compat, time_utils
...
async def _run_discover_job(
    ...
    job["status"] = "running"
    job["started_at"] = time_utils.utc_timestamp()
    ...
    try:
        ...
        if not result["success"] and not host_results:
            ...
            job["completed_at"] = time_utils.utc_timestamp()
...
async def discover_updates(
    ...
    async_compat.fire_and_forget(_run_discover_job(job_id, request.node_ids, request.role), name=job_id)
