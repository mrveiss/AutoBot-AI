    ...
    <div class="advanced-step-modal" @click.stop tabindex="0" @keyup.enter="$event.target.click()" @keyup.space="$event.target.click()">
      ...
      <div class="workflow-management">
        ...
        <div v-if="showStepsManager" class="steps-manager">
          ...
          <div class="add-step-section">
            <button @click="showNewStepForm = !showNewStepForm" class="add-step-btn" aria-label="➕ add new step">
              ➕ Add New Step
            </button>
            ...
            <div v-if="showNewStepForm" class="new-step-form">
              ...
              <div class="form-actions">
                <button @click="addNewStep" class="save-btn" aria-label="💾 add step">💾 Add Step</button>
                <button @click="cancelNewStep" class="cancel-btn" aria-label="❌ cancel">❌ Cancel</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="modal-actions">
        <div class="primary-actions">
          <button @click="executeStep" class="execute-btn" :disabled="!canExecute" aria-label="✅ execute & continue">
            ✅ Execute & Continue
          </button>
          <button @click="skipStep" class="skip-btn" aria-label="⏭️ skip this step">
            ⏭️ Skip This Step
          </button>
          <button @click="takeManualControl" class="manual-btn" aria-label="👤 take manual control">
            👤 Take Manual Control
          </button>
        </div>
        
        <div class="secondary-actions">
          <button @click="executeAll" class="execute-all-btn" v-if="workflowSteps.length > 1" aria-label="Execute all remaining steps">
            🚀 Execute All Remaining ({{ workflowSteps.length - currentStepIndex }} steps)
          </button>
          <button @click="saveWorkflow" class="save-workflow-btn" aria-label="💾 save workflow as template">
