...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for endpoint, method, data, timeout in endpoints_to_test:
            success, result = await test_endpoint(session, endpoint, method, data, timeout)
            if not success:
        ...
        success, chat_result = await test_endpoint(session, "/api/chats/new", "POST", {}, 10)
        if success and "chat_id" in chat_result:
            ...
            success, result = await test_endpoint(session, "/api/chat/direct", "POST", chat_data, 15)
            if success:
            ...
            success, result = await test_endpoint(session, f"/api/chats/{chat_id}/message", "POST", chat_data, 25)
            if success:
