...
class DocumentationChangeHandler(FileSystemEventHandler):
    ...
    def _handle_change(self, file_path: str, change_type: str) -> None:
        ...
        asyncio.create_task(
            self.watcher.queue_change(path, change_type)
        )
...
class DocumentationWatcherService:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Documentation watcher started - monitoring: %s",
                DOCS_DIR
            )
            return True
    ...
    async def _process_single_change(
        self, file_path: Path, change_type: str
    ) -> None:
        """
        ...
        try:
            logger.info(
                "Processing documentation change: %s (%s)",
                file_path.name, change_type
            )
            ...
            logger.error(
                "Error processing documentation change %s: %s",
                file_path, e
            )
            self._stats["errors"] += 1
    ...
    async def _handle_deletion(self, file_path: Path) -> None:
        ...
        try:
                ...
                if keys:
                    for key in keys:
                        try:
                            ...
                            if fact_data:
                                metadata = json.loads(
                                    fact_data.get(b"metadata", b"{}")
                                )
                                if metadata.get("file_path") == relative_path:
                                    await asyncio.to_thread(
                                        kb.redis_client.delete, key
                                    )
                                    removed_count += 1
                        except Exception:
                            continue
            ...
            logger.info(
                "Removed %d facts for deleted file: %s",
                removed_count,
                file_path.name
            )
