...
class CommandExecutor:
    ...
    async def run_shell_command(self, command: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"SECURITY: Executing command through secure wrapper: {command[:50]}...")
        ...
        if security_info.get('blocked'):
            logger.warning(f"SECURITY: Command blocked - {command} - Reason: {security_info.get('reasons')}")
