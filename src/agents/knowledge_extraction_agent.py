...
Implements intelligent extraction of atomic facts from text content with temporal awareness.
This agent uses LLM-powered analysis to identify discrete facts, classify them, and 
assign temporal characteristics for dynamic knowledge management.
"""
...
from src.models.atomic_fact import (
    AtomicFact, FactExtractionResult, FactType, TemporalType, ConfidenceLevel
)
from src.utils.logging_manager import get_llm_logger
from src.utils.config_manager import config_manager
from src.llm_interface_unified import get_unified_llm_interface, LLMType
...
class KnowledgeExtractionAgent:
    ...
    This agent analyzes text to identify discrete, verifiable facts and classifies
    them by type and temporal characteristics. It supports the RAG optimization
    pipeline by creating structured, time-aware knowledge representations.
    """
    ...
    def _build_extraction_prompt(self, content: str, context: Optional[str] = None) -> str:
        ...
        return f"""
Analyze the following text and extract atomic facts. Each fact should be a single, verifiable statement that can be independently confirmed or contradicted.
...
4. Fact Type: FACT (objective), OPINION (subjective), PREDICTION (future), INSTRUCTION (procedural), or DEFINITION (explanatory)
5. Temporal Type: STATIC (unchanging), DYNAMIC (can change), ATEMPORAL (not time-bound), or TEMPORAL_BOUND (specific time period)
...
"""
    ...
    def _classify_temporal_type(self, fact_text: str, context: str = "") -> TemporalType:
        ...
        dynamic_count = sum(1 for keyword in self.temporal_keywords["dynamic_indicators"] 
                           if keyword in text_lower)
        static_count = sum(1 for keyword in self.temporal_keywords["static_indicators"] 
                          if keyword in text_lower)
        future_count = sum(1 for keyword in self.temporal_keywords["future_indicators"] 
                          if keyword in text_lower)
        past_count = sum(1 for keyword in self.temporal_keywords["past_indicators"] 
                        if keyword in text_lower)
        ...
        has_temporal_bound = any(re.search(pattern, text_lower) for pattern in date_patterns)
    ...
    def _extract_entities(self, fact_text: str) -> List[str]:
        ...
        common_words = {'the', 'is', 'are', 'was', 'were', 'has', 'have', 'had', 'will', 'can', 'could', 'should'}
        entities = list(set(entity for entity in entities 
    ...
    def _validate_fact(self, fact_data: Dict[str, Any]) -> bool:
        ...
        """
        required_fields = ['subject', 'predicate', 'object', 'fact_type', 'temporal_type', 'confidence']
        ...
        if any(not isinstance(fact_data.get(field), str) or not fact_data.get(field).strip() 
    ...
    async def extract_facts_from_text(self, 
                                    content: str, 
                                    source: str,
                                    context: Optional[str] = None,
                                    chunk_id: Optional[str] = None) -> FactExtractionResult:
        """
        ...
        try:
            ...
            if not response or response.error:
                logger.warning(f"No response from LLM for fact extraction: {response.error if response else 'No response'}")
                return FactExtractionResult(facts=[], extraction_metadata={
            ...
            for fact_data in raw_facts:
                try:
                    ...
                    if not fact_data.get('entities') or len(fact_data['entities']) < 2:
                        fact_text = f"{fact_data['subject']} {fact_data['predicate']} {fact_data['object']}"
                        additional_entities = self._extract_entities(fact_text)
                        fact_data['entities'] = list(set(
                            fact_data.get('entities', []) + additional_entities
                        ))
            ...
            logger.info(f"Extracted {len(extracted_facts)} facts in {processing_time:.2f}s "
    ...
    async def extract_facts_from_chunks(self, 
                                      chunks: List[Dict[str, Any]], 
                                      source: str) -> FactExtractionResult:
        """
        ...
        logger.info(f"Extracted {len(all_facts)} total facts from {successful_extractions} chunks "
    ...
    def filter_facts(self, 
                    ...
                    active_only: bool = True) -> List[AtomicFact]:
        """
