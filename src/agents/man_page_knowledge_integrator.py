...
class ManPageParser:
    ...
    def _parse_options_section(self, content: str) -> List[Dict[str, str]]:
        ...
        option_pattern = re.compile(r'^\s*(-\w+(?:,\s*--\w+[-\w]*)*|\s*--\w+[-\w]*)\s+(.*?)(?=^\s*-|\Z)', 
    ...
    def _parse_examples_section(self, content: str) -> List[Dict[str, str]]:
        ...
        for block in example_blocks:
            ...
            for line in lines:
                ...
                elif command_line is None and any(line.startswith(cmd) for cmd in ['ls', 'cat', 'grep', 'find', 'awk', 'sed']):
                    command_line = line
                else:
...
class ManPageKnowledgeIntegrator:
    ...
    def __init__(self):
        ...
        self.priority_commands = [
            # Network tools
            'ping', 'curl', 'wget', 'netstat', 'ss', 'nmap', 'arp', 'dig', 'nslookup',
            'traceroute', 'ifconfig', 'ip', 'iptables', 'ufw',
            
            # File operations
            'ls', 'find', 'grep', 'cat', 'head', 'tail', 'less', 'more', 'wc',
    ...
    async def check_man_page_exists(self, command: str) -> bool:
        ...
        try:
            result = subprocess.run(['man', '-w', command], 
    ...
    async def extract_man_page(self, command: str, section: int = 1) -> Optional[ManPageInfo]:
        ...
        try:
            ...
            result = subprocess.run(['man', str(section), command], 
    ...
    def convert_to_knowledge_yaml(self, man_info: ManPageInfo) -> Dict[str, Any]:
        ...
        tool_entry = {
            ...
            'usage': {
                'basic': man_info.synopsis if man_info.synopsis else f"{man_info.command} [options]"
            },
    ...
    async def integrate_priority_commands(self) -> Dict[str, Any]:
        ...
        logger.info(f"Processing {len(commands_to_process)} priority commands available on this machine")
    ...
    async def search_man_pages(self, query: str) -> List[ManPageInfo]:
        ...
        for cache_file in self.man_cache_dir.glob("*.json"):
            man_info = await self.load_cached_man_page(
                cache_file.stem.split('_')[0], 
                int(cache_file.stem.split('_')[1])
            )
            ...
            searchable_text += " ".join(opt['description'].lower() for opt in man_info.options)
...
if __name__ == "__main__":
    ...
    async def test_integration():
        ...
        successful_commands = [cmd for cmd, status in results['commands'].items() 
                             if status == 'success'][:5]
