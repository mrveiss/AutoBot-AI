...
class AsyncChatWorkflow:
    ...
    async def process_chat_message(
        ...
        try:
            ...
            await self.add_workflow_message(
                "debug", 
                "🏁 WORKFLOW: Response generation completed",
                step="completion"
            )
    ...
    async def _classify_message(self, message: str) -> MessageType:
        ...
        if any(word in message_lower for word in ["terminal", "command", "bash", "shell", "run"]):
            return MessageType.TERMINAL_TASK
        elif any(word in message_lower for word in ["desktop", "gui", "window", "screen"]):
            return MessageType.DESKTOP_TASK
        elif any(word in message_lower for word in ["system", "config", "install", "setup"]):
