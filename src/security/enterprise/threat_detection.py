...
class ThreatDetectionEngine:
    ...
    def __init__(self, config_path: str = "/home/kali/Desktop/AutoBot/config/security/threat_detection.yaml"):
        ...
        self.profile_storage_path = Path("/home/kali/Desktop/AutoBot/data/security/user_profiles.pkl")
    ...
    async def analyze_event(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        if self.config.get("detection_modes", {}).get("command_injection_detection", True):
        ...
        if detected_threats:
            primary_threat = max(detected_threats, 
    ...
    async def _detect_command_injection(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        for pattern_info in self.command_injection_patterns:
            ...
            if re.search(pattern_info["pattern"], command_content, re.IGNORECASE):
                ...
                if pattern_info["severity"] == "critical":
                    ...
                    max_severity = "high"
                elif pattern_info["severity"] == "medium" and max_severity not in ["critical", "high"]:
                    max_severity = "medium"
        ...
        if detected_patterns:
            ...
            return ThreatEvent(
                ...
                raw_event=event,
                mitigation_actions=["block_command", "quarantine_session", "alert_security_team"]
            )
    ...
    async def _detect_behavioral_anomaly(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        current_hour = datetime.fromisoformat(event.get("timestamp", datetime.utcnow().isoformat())).hour
        ...
        deviation_threshold = self.config.get("behavioral_analysis", {}).get("deviation_threshold", 2.0)
        ...
        if anomalies:
            ...
            return ThreatEvent(
                ...
                raw_event=event,
                mitigation_actions=["monitor_user", "require_additional_auth", "alert_security_team"]
            )
    ...
    async def _detect_brute_force(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        window_minutes = self.config.get("thresholds", {}).get("brute_force_window_minutes", 15)
        threshold = self.config.get("thresholds", {}).get("brute_force_attempts", 5)
        
        recent_failures = self._count_recent_failures(user_id, source_ip, window_minutes)
        ...
        if recent_failures >= threshold:
            ...
            return ThreatEvent(
                ...
                details={
                    ...
                    "attack_pattern": "credential_stuffing" if user_id != "unknown" else "dictionary_attack"
                },
                raw_event=event,
    ...
    async def _detect_malicious_file(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        size_threshold = self.config.get("thresholds", {}).get("file_size_suspicious_mb", 100) * 1024 * 1024
        ...
        if threats:
            ...
            return ThreatEvent(
                ...
                raw_event=event,
                mitigation_actions=["quarantine_file", "scan_with_antivirus", "alert_security_team"]
            )
    ...
    async def _detect_api_abuse(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        # Check rate limiting
        rate_limit = self.config.get("thresholds", {}).get("api_rate_limit_per_minute", 100)
        recent_requests = self._count_recent_api_requests(user_id, source_ip, 1)  # 1 minute window
        ...
        bulk_threshold = self.config.get("thresholds", {}).get("bulk_data_threshold_mb", 100) * 1024 * 1024
        ...
        if threats:
            ...
            return ThreatEvent(
                ...
                raw_event=event,
                mitigation_actions=["rate_limit_user", "monitor_api_usage", "alert_security_team"]
            )
    ...
    async def _detect_insider_threat(self, event: Dict) -> Optional[ThreatEvent]:
        ...
        resource = event.get("resource", "")
        if resource and any(sensitive in resource.lower() for sensitive in ["admin", "config", "secret", "key", "password"]):
        ...
        if len(risk_indicators) >= 2:  # Multiple indicators suggest insider threat
            ...
            return ThreatEvent(
                ...
                details={
                    ...
                    "data_sensitivity": "high" if any("sensitive" in indicator for indicator in risk_indicators) else "medium"
                },
                raw_event=event,
                mitigation_actions=["enhanced_monitoring", "require_manager_approval", "alert_security_team", "document_incident"]
            )
        
        return None

    def _get_recent_action_frequency(self, user_id: str, action: str, hours: int = 1) -> int:
    ...
    def _count_recent_failures(self, user_id: str, source_ip: str, window_minutes: int) -> int:
        ...
        for event in reversed(self.recent_events):
            ...
            if event_time < cutoff_time:
                break
            if (event.get("action") == "authentication" and 
    ...
    def _count_recent_api_requests(self, user_id: str, source_ip: str, window_minutes: int) -> int:
    ...
    def _get_recent_endpoint_usage(self, user_id: str, endpoint: str, hours: int = 24) -> int:
        ...
        for event in reversed(self.recent_events):
            ...
            if (event.get("user_id") == user_id and 
                event.get("action") == "api_request" and
                event.get("resource") == endpoint):
                count += 1
    ...
    async def _update_user_behavior(self, user_id: str, event: Dict):
        ...
        action = event.get("action", "")
        if action:
            profile.baseline_actions[action] = profile.baseline_actions.get(action, 0) + 1
        
        # Update typical hours
        event_hour = datetime.fromisoformat(event.get("timestamp", datetime.utcnow().isoformat())).hour
        ...
        if action in ["file_read", "file_write", "file_delete"]:
            resource = event.get("resource", "")
            if resource:
                profile.file_access_patterns[resource] = profile.file_access_patterns.get(resource, 0) + 1
        
        # Update API usage patterns
        if action == "api_request":
            endpoint = event.get("resource", "")
            if endpoint:
                profile.api_usage_patterns[endpoint] = profile.api_usage_patterns.get(endpoint, 0) + 1
    ...
    async def _execute_response_actions(self, threat: ThreatEvent):
        ...
        for action in threat.mitigation_actions:
            if action == "block_ip" and response_config.get("auto_block_critical", False):
                await self._block_ip_address(threat.source_ip)
            
            elif action == "quarantine_file" and response_config.get("auto_quarantine_files", True):
                await self._quarantine_file(threat.resource)
            
            elif action == "rate_limit_user" and response_config.get("rate_limit_suspicious_ips", True):
                await self._apply_rate_limiting(threat.user_id, threat.source_ip)
            
            elif action == "alert_security_team" and response_config.get("alert_security_team", True):
    ...
    async def _apply_rate_limiting(self, user_id: str, ip_address: str):
        ...
        logger.warning(f"SECURITY ACTION: Rate limiting user {user_id} from IP {ip_address}")
    ...
    async def _retrain_models(self):
        ...
        if len(self.recent_events) < min_samples:
            logger.info(f"Insufficient data for model training: {len(self.recent_events)} < {min_samples}")
            return
    ...
    def _calculate_user_risk_score(self, user_id: str) -> float:
        ...
        high_risk_actions = ["admin_action", "system_configuration", "privilege_escalation"]
        recent_high_risk = sum(1 for event in self.recent_events 
                              if event.get("user_id") == user_id and 
                              event.get("action") in high_risk_actions)
        ...
        off_hours_activity = sum(1 for event in self.recent_events
                               if event.get("user_id") == user_id and
                               datetime.fromisoformat(event.get("timestamp", datetime.utcnow().isoformat())).hour < 6)
    ...
    async def _cleanup_old_data(self):
        ...
        try:
            ...
            logger.debug(f"Cleanup completed: removed {len(expired_sessions)} expired sessions")
