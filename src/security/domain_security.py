...
class DomainSecurityManager:
    ...
    async def validate_domain_safety(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if cache_key in self.domain_cache:
                ...
                if time.time() - cached["timestamp"] < self.config.config.get("domain_security", {}).get("cache_duration", 3600):
            ...
            reputation_threshold = self.config.config.get("domain_security", {}).get("reputation_threshold", 0.7)
    ...
    async def _validate_network_access(self, parsed) -> Dict[str, Any]:
        ...
        try:
            ...
            allowed_ports = self.config.config.get("domain_security", {}).get("network_validation", {}).get("allowed_ports", [80, 443])
            ...
            if network_config.get("block_cloud_metadata", True):
                metadata_ips = ["169.254.169.254", "100.100.100.200"]  # AWS, Alibaba Cloud
    ...
    def _is_whitelisted(self, domain: str) -> bool:
        ...
        for pattern in self.whitelist_patterns:
            if pattern.match(domain):
                logger.debug(f"Domain {domain} matched whitelist pattern: {pattern.pattern}")
                return True
        return False
    ...
    def _check_blacklist(self, domain: str) -> Dict[str, Any]:
        ...
        for pattern in self.blacklist_patterns:
            if pattern.match(domain):
                logger.warning(f"Domain {domain} matched blacklist pattern: {pattern.pattern}")
                return {
                    "blocked": True,
    ...
    async def _check_reputation_services(self, domain: str) -> float:
        ...
        for service_name, service_config in reputation_services.items():
            if service_config.get("enabled", False) and service_config.get("api_key"):
                logger.debug(f"Reputation service {service_name} configured but not implemented yet")
    ...
    def _calculate_basic_reputation(self, domain: str) -> float:
        ...
        # Suspicious keywords
        suspicious_keywords = ['secure', 'bank', 'update', 'verify', 'confirm', 'urgent']
    ...
    def _cache_result(self, cache_key: str, result: Dict[str, Any]):
        ...
        if len(self.domain_cache) > 1000:
            ...
            self.domain_cache = {
                k: v for k, v in self.domain_cache.items()
