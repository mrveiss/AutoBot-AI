...
class SecureLLMCommandParser:
    ...
    def parse_commands(self, llm_response: str, user_goal: str = "") -> List[ValidatedCommand]:
        ...
        logger.info(f"✅ Validated {len(validated_commands)}/{len(raw_commands)} commands")
    ...
    def _validate_single_command(self, 
                                  ...
                                  user_goal: str) -> Optional[ValidatedCommand]:
        """
        ...
        if not validator_result.get("valid", False):
            ...
            self._log_security_event(
                ...
                command=command,
                risk_level="high",
                patterns=[validator_result.get("reason", "CommandValidator validation failed")]
            )
        ...
        if explanation:
            explanation_validation = self.injection_detector.detect_injection(
                explanation,
                context="command_explanation"
            )
            
            if explanation_validation.risk_level in [InjectionRisk.HIGH, InjectionRisk.CRITICAL]:
                logger.warning(f"⚠️ Suspicious explanation detected for command: {command}")
                logger.warning(f"Explanation patterns: {explanation_validation.detected_patterns}")
