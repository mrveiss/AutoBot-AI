...
from src.models.atomic_fact import AtomicFact, FactExtractionResult, FactType, TemporalType
...
class FactExtractionService:
    ...
    async def extract_and_store_facts(self, 
        ...
        try:
            ...
            if self.enable_deduplication:
                deduplicated_facts = await self._deduplicate_facts(extraction_result.facts)
                logger.info(f"Deduplication: {len(extraction_result.facts)} -> {len(deduplicated_facts)} facts")
                extraction_result.facts = deduplicated_facts
            
            # Resolve entities if enabled
            if self.enable_entity_resolution:
                resolved_facts = await entity_resolver.resolve_facts_entities(extraction_result.facts)
    ...
    async def extract_facts_from_chunks(self, 
        ...
        try:
            ...
            logger.info(f"Processed {len(chunks)} chunks, extracted {len(extraction_result.facts)} facts")
    ...
    async def _store_facts(self, 
        ...
        try:
            ...
            for i in range(0, len(facts), self.batch_size):
                ...
                for fact in batch:
                    try:
                        ...
                        temporal_index_key = f"facts_by_temporal:{fact.temporal_type.value}"
                ...
                try:
                    await pipe.execute()
                    stored_count += len(batch) - (error_count - (stored_count // self.batch_size * error_count))
            ...
            logger.info(f"Fact storage complete: {stored_count} stored, {error_count} errors")
    ...
    async def _store_facts_in_kb(self, 
                                facts: List[AtomicFact], 
        ...
        try:
            ...
            logger.debug(f"Stored {len(facts)} facts in knowledge base as structured content")
            
        except Exception as e:
            logger.error(f"Error storing facts in knowledge base: {e}")
    
    async def _record_extraction_history(self, 
                                       source: str, 
    ...
    async def get_facts_by_criteria(self, 
                                  source: Optional[str] = None,
                                  fact_type: Optional[FactType] = None,
                                  temporal_type: Optional[TemporalType] = None,
                                  min_confidence: Optional[float] = None,
                                  active_only: bool = True,
                                  limit: int = 100) -> List[AtomicFact]:
        """
        ...
        try:
            ...
            facts = []
            fact_ids_list = list(fact_ids)[:limit * 2]  # Get more IDs to account for filtering
        ...
        try:
            ...
            for history_json in recent_history:
                try:
                    ...
                    for fact_type, count in history.get("distributions", {}).get("fact_types", {}).items():
                        fact_type_totals[fact_type] = fact_type_totals.get(fact_type, 0) + count
                    
                    for temporal_type, count in history.get("distributions", {}).get("temporal_types", {}).items():
                        temporal_type_totals[temporal_type] = temporal_type_totals.get(temporal_type, 0) + count
                        
                except json.JSONDecodeError:
                    continue
            
            avg_processing_time = total_processing_time / total_extractions if total_extractions > 0 else 0
            avg_facts_per_extraction = total_facts_extracted / total_extractions if total_extractions > 0 else 0
            ...
            return {
                ...
                "extraction_success_rate": round(
                    (total_facts_stored / total_facts_extracted * 100) if total_facts_extracted > 0 else 0, 1
                )
            }
            ...
            for fact in new_facts:
                try:
                    ...
                    if result["status"] == "success" and result["facts_invalidated"] > 0:
                        logger.info(f"Invalidated {result['facts_invalidated']} contradictory facts for new fact {fact.fact_id}")
                    
                except Exception as e:
                    logger.error(f"Error checking contradictions for fact {fact.fact_id}: {e}")
                    continue
