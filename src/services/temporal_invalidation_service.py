...
class TemporalInvalidationService:
    ...
    async def initialize_rules(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Invalidation rules initialized: {len(existing_rules)} total, {rules_added} added")
    ...
    async def run_invalidation_sweep(self, 
        ...
        try:
            ...
            if not rules:
                return {
                    "status": "error",
                    "message": "No invalidation rules available"
                }
            ...
            logger.info(f"Invalidation sweep completed: {invalidated_count}/{len(facts_to_invalidate)} facts invalidated")
            return result
    ...
    async def _invalidate_facts(self, 
                               facts: List[AtomicFact], 
                               reasons: Dict[str, Dict[str, Any]]) -> int:
        """
    ...
    async def invalidate_contradictory_facts(self, new_fact: AtomicFact) -> Dict[str, Any]:
        ...
        try:
            if not self.fact_extraction_service:
                return {"status": "error", "message": "Fact extraction service not available"}
            
            logger.info(f"Checking for contradictions with new fact: {new_fact.fact_id}")
            ...
            for existing_fact in similar_facts:
                if (existing_fact.subject.lower() == new_fact.subject.lower() and
                    ...
                    existing_fact.object.lower() != new_fact.object.lower() and
                    existing_fact.fact_id != new_fact.fact_id):
            ...
            if contradictory_facts:
                high_confidence_contradictions = [
                    f for f in contradictory_facts 
                    if new_fact.confidence > f.confidence
                ]
    ...
    async def get_invalidation_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_processing_time = total_processing_time / total_sweeps if total_sweeps > 0 else 0
            invalidation_rate = (total_facts_invalidated / total_facts_processed * 100) if total_facts_processed > 0 else 0
    ...
    async def schedule_periodic_invalidation(self):
        ...
        logger.info(f"Scheduling periodic invalidation every {self.invalidation_interval_hours} hours")
        ...
        while True:
            try:
                ...
                if result["status"] == "success":
                    logger.info(f"Scheduled invalidation completed: {result['facts_invalidated']} facts invalidated")
                else:
