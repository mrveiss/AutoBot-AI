...
class VNCServerManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"VNC Server Manager initialized: VNC={self.vnc_available}, NoVNC={self.novnc_available}")
    
    def _check_vnc_availability(self) -> bool:
        """Check if VNC server components are available"""
        try:
            # Check for tigervnc or tightvnc
            result = subprocess.run(['which', 'vncserver'], 
    ...
    async def _start_novnc(self, vnc_port: int, novnc_port: int) -> Optional[subprocess.Popen]:
...
class DesktopStreamingManager:
    ...
    def __init__(self):
        ...
        self.session_clients: Dict[str, List[str]] = {}  # session_id -> list of client_ids
    ...
    async def handle_websocket_client(self, websocket, path: str):
        ...
        try:
            ...
            logger.info(f"WebSocket client connected: {client_id} to session {session_id}")
    ...
    async def _handle_control_request(self, session_id: str, control_data: Dict[str, Any]):
    ...
    async def _get_session_screenshot(self, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run([
                'import', '-window', 'root', '-display', f':{display}',
                'png:-'
            ], capture_output=True)
