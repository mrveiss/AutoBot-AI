...
class PromptKnowledgeSync:
    ...
    def should_import_prompt(self, prompt_key: str, file_path: str) -> tuple[bool, Optional[str], Optional[str]]:
        """
    ...
    def extract_prompt_metadata(self, prompt_key: str, content: str, file_path: str, category: str, collection_name: str) -> Dict[str, Any]:
        """
    ...
    def _determine_prompt_type(self, prompt_key: str, content: str) -> str:
        ...
        elif any(domain in prompt_key for domain in ['developer', 'hacker', 'researcher']):
            return 'domain_expertise'
        else:
            return 'operational_guidance'

    async def sync_prompts_to_knowledge(self, force_update: bool = False) -> Dict[str, Any]:
        """
        ...
        for prompt_key in available_prompts:
            try:
                ...
                should_import, category, collection_name = self.should_import_prompt(prompt_key, file_path)
                ...
                # Extract metadata
                metadata = self.extract_prompt_metadata(prompt_key, content, file_path, category, collection_name)
        ...
        logger.info(f"Prompt sync completed: {results['imported']} imported, {results['updated']} updated, {results['skipped']} skipped, {results['errors']} errors")
        return results

    async def _find_existing_prompt_entry(self, prompt_key: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for fact in all_facts:
                ...
                if isinstance(metadata, dict) and metadata.get('prompt_key') == prompt_key:
    ...
    async def remove_prompt_knowledge(self, prompt_key: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Removed {removed_count} knowledge entries for prompt: {prompt_key}")
        ...
        try:
            ...
            for category_name, category_info in self.IMPORT_CATEGORIES.items():
                ...
                auto_imported_count = sum(
                    ...
                    if (fact.get('metadata', {}).get('collection') == collection_name and
                        fact.get('metadata', {}).get('auto_imported', False))
                )
...
async def sync_prompts_to_knowledge(knowledge_base: KnowledgeBase, force_update: bool = False) -> Dict[str, Any]:
    """
