...
class AutoBotKnowledgeBase:
    ...
    def __init__(self):
        ...
        try:
            ...
            redis_config = global_config_manager.get_nested(
                "redis_databases.knowledge_vectors", {}
            )
            db_number = redis_config.get("db", 8)  # Database 8 for knowledge vectors

            logger.info(
                f"Connecting to Redis database {db_number} for knowledge vectors..."
            )
            ...
            if self.redis_client and self.redis_client.ping():
                ...
                logger.error(
                    "❌ Redis connection failed - knowledge base limited functionality"
                )
    ...
    async def add_documents_from_directory(
        ...
        logger.info(
            f"🚀 Starting GPU-optimized document processing from: {directory_path}"
        )
        ...
        try:
                ...
                try:
                    ...
                    if text_content and len(text_content.strip()) > 50:
                        ...
                        if self.index and documents:
                            ...
                            logger.debug(
                                f"✅ Processed {file_path}: {len(documents)} chunks"
                            )
                        ...
                        if processed_count % 10 == 0:
                            logger.info(
                                f"📊 Progress: {processed_count}/{len(files_to_process)} files processed"
                            )
            ...
            results.update(
                {
                    ...
                    "chunks_per_second": (
                        total_chunks / processing_time if processing_time > 0 else 0
                    ),
                    "files_per_second": (
            ...
            logger.info(
                f"   ⚡ Performance: {results['chunks_per_second']:.1f} chunks/sec"
            )
    ...
    async def search_documents(
        ...
        try:
            ...
            response = await asyncio.wait_for(
                asyncio.to_thread(self.query_engine.query, query), timeout=10.0
            )
