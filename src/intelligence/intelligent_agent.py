...
from src.intelligence.goal_processor import GoalProcessor, ProcessedGoal, GoalCategory
from src.intelligence.tool_selector import OSAwareToolSelector, ToolSelection
from src.intelligence.streaming_executor import StreamingCommandExecutor, StreamChunk, ChunkType
...
class IntelligentAgent:
        ...
        try:
            ...
            init_result = {
                ...
                "os_info": {
                    ...
                    "distro": self.state.os_info.distro.value if self.state.os_info.distro else None,
            ...
            logger.info(f"Total capabilities: {capabilities_info.get('total_count', 0)}")
    ...
    async def process_natural_language_goal(self,
                                          user_input: str,
                                          context: Optional[Dict[str, Any]] = None) -> AsyncGenerator[StreamChunk, None]:
        """
            ...
            if processed_goal.confidence > 0.5:
                ...
                if tool_selection.requires_install:
                    yield StreamChunk(
                        timestamp=self._get_timestamp(),
                        chunk_type=ChunkType.COMMENTARY,
                        content=f"📦 Need to install required tool first",
        ...
        try:
            ...
            if commands:
                ...
                yield StreamChunk(
                    timestamp=self._get_timestamp(),
                    chunk_type=ChunkType.COMMENTARY,
                    content="🤷 I couldn't determine specific commands for this request. Could you try rephrasing or being more specific?",
    ...
    async def _install_tool(self, install_command: str, goal: ProcessedGoal) -> AsyncGenerator[StreamChunk, None]:
        ...
        if not self.state.os_info.is_root and 'sudo' not in install_command:
            ...
            if any(pm in install_command for pm in ['apt', 'yum', 'dnf', 'pacman', 'zypper']):
...
- Distribution: {os_info.distro.value if os_info.distro else 'N/A'}
...
Your task is to analyze this request and provide specific, executable commands for this system.
...
If the request is unclear or potentially dangerous, explain why and suggest alternatives.
"""
...
- Distribution: {self.state.os_info.distro.value if self.state.os_info.distro else 'N/A'}
- Version: {self.state.os_info.version}
- Architecture: {self.state.os_info.architecture}
- User: {self.state.os_info.user}
...
I understand natural language goals and translate them into appropriate OS-specific commands.
"""
...
async def get_intelligent_agent(llm_interface: LLMInterface = None,
                               knowledge_base: KnowledgeBase = None,
                               worker_node: WorkerNode = None,
                               command_validator: CommandValidator = None) -> IntelligentAgent:
    """
