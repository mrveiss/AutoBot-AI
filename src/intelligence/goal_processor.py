...
class GoalProcessor:
    ...
    def _build_intent_patterns(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "install_package": {
                "patterns": [
                    r"install.{0,20}(docker|nginx|apache|mysql|postgresql|node|python|git)",
                    r"setup.{0,20}(docker|nginx|apache|mysql|postgresql|node|python|git)",
                    r"deploy.{0,20}(docker|nginx|apache|mysql|postgresql|node|python|git)"
                ],
    ...
    def _guess_category(self, user_input: str) -> GoalCategory:
        ...
        category_keywords = {
            GoalCategory.NETWORK: ["network", "ip", "connect", "ping", "url", "port", "scan"],
            GoalCategory.SYSTEM: ["system", "os", "update", "install", "service", "process"],
            GoalCategory.FILES: ["file", "directory", "folder", "backup", "copy", "move"],
            GoalCategory.DEVELOPMENT: ["code", "git", "build", "deploy", "docker", "npm"],
            GoalCategory.SECURITY: ["security", "firewall", "password", "encrypt", "permissions"],
    ...
    def add_custom_intent(self, intent_name: str, patterns: List[str], 
                         category: GoalCategory, explanation: str, 
