...
class OSDetector:
    ...
    async def detect_system(self) -> OSInfo:
        ...
        logger.info(f"System detected: {os_type.value} ({distro.value if distro else 'N/A'})")
    ...
    async def validate_installation_capability(self) -> Tuple[bool, str]:
        ...
        if (self._os_info.package_manager in root_required_managers and 
            not self._os_info.is_root and not shutil.which("sudo")):
            return False, f"Package manager {self._os_info.package_manager} requires root access"
    ...
    async def get_capabilities_info(self) -> Dict[str, any]:
        ...
        network_tools = [tool for tool in capabilities if tool in [
            "ping", "curl", "wget", "netstat", "ss", "nmap", "arp",
            "dig", "nslookup", "traceroute", "ifconfig", "ip"
        ]]
        
        system_tools = [tool for tool in capabilities if tool in [
            "ps", "top", "htop", "df", "du", "free", "uname", "whoami",
            "id", "groups", "sudo", "su", "crontab", "systemctl", "service"
        ]]
        
        file_tools = [tool for tool in capabilities if tool in [
            "ls", "find", "grep", "sed", "awk", "cat", "head", "tail",
            "less", "more", "wc", "sort", "uniq", "tar", "zip", "unzip"
        ]]
        
        dev_tools = [tool for tool in capabilities if tool in [
            "git", "python", "python3", "pip", "pip3", "node", "npm",
            "docker", "docker-compose", "vim", "nano", "emacs"
        ]]
