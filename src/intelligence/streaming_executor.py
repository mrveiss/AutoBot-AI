...
class StreamingCommandExecutor:
    ...
    def __init__(self, llm_interface: LLMInterface, command_validator: CommandValidator):
        """
    ...
    async def execute_with_streaming(self, 
                                   ...
                                   user_goal: str,
                                   timeout: int = 300,
                                   provide_commentary: bool = True) -> AsyncGenerator[StreamChunk, None]:
        """
        ...
        if len(self.active_processes) >= self._max_processes:
            yield StreamChunk(
                timestamp=self._get_timestamp(),
                chunk_type=ChunkType.ERROR,
                content=f"❌ Maximum number of concurrent processes ({self._max_processes}) reached",
    ...
    async def _stream_process_output(self,
        ...
        async def read_stdout():
            ...
            while True:
                try:
                    ...
                    if provide_commentary and commentary_counter > 500:  # Every ~500 chars
                        commentary_counter = 0
                        async for chunk in self._provide_progress_commentary(stdout_buffer[-200:], user_goal):
                            yield chunk
    ...
    async def _provide_progress_commentary(self, recent_output: str, user_goal: str) -> AsyncGenerator[StreamChunk, None]:
        ...
        try:
            ...
            prompt = f"""Analyze this command output and provide a brief, helpful comment about the progress:

User Goal: {user_goal}
Recent Output: {recent_output[-150:]}

Provide a single, concise comment (max 100 characters) about what's happening. 
Be helpful and informative, but don't repeat the obvious.
Use emojis when appropriate.
If the output is just routine/boring, respond with "SKIP" to avoid spam."""
    ...
    async def _provide_completion_commentary(self, command: str, user_goal: str, execution_time: float) -> AsyncGenerator[StreamChunk, None]:
        """Provide intelligent commentary on command completion."""
        try:
            prompt = f"""A command just completed successfully. Provide a brief, helpful summary:

User Goal: {user_goal}
Command: {command}
Execution Time: {execution_time:.2f}s

Provide a single, concise comment (max 150 characters) summarizing what was accomplished.
