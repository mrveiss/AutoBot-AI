...
class EnhancedCommandDetector:
    ...
    def _load_intent_patterns(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "disk_space": {
                "keywords": ["disk", "space", "storage", "filesystem", "df", "free space"],
    ...
    def get_command_info_from_kb(self, command_name: str) -> Optional[Dict]:
        ...
        if manual:
            return {
                ...
                "explanation": f"Use {manual.command_name} to {manual.description.lower()}",
    ...
    def search_commands_by_functionality(self, query: str) -> List[Tuple[str, str, str]]:
    ...
    async def detect_command_needed(self, message: str, llm_interface=None) -> Optional[Dict]:
        """Enhanced command detection with knowledge base integration.
        ...
        how_to_match = re.search(r'how\s+(?:do\s+i\s+|to\s+)?(?:use\s+)?(?:the\s+)?([a-zA-Z][a-zA-Z0-9_-]+)', 
    ...
    def explain_command_risk(self, command_info: Dict) -> str:
        ...
        explanations = {
            "LOW": f"The command '{command}' is considered safe - it only reads information and doesn't modify your system.",
            "MEDIUM": f"The command '{command}' may modify files or system settings. Please review what it will do before approving.",
            "HIGH": f"⚠️ The command '{command}' can make significant system changes or delete data. Use with extreme caution!"
        }
