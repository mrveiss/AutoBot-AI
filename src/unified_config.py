...
class UnifiedConfig:
    ...
    def get_timeout(
        self, category: str, timeout_type: str = "default", default: float = 60.0
    ) -> float:
    ...
    def get_redis_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "socket_timeout": redis_config.get("connection", {}).get(
                "socket_timeout", 2.0
            ),
            "socket_connect_timeout": redis_config.get("connection", {}).get(
                "socket_connect_timeout", 2.0
            ),
            "retry_on_timeout": redis_config.get("connection", {}).get(
                "retry_on_timeout", False
            ),
        }
...
logger.info(
    f"Unified config exports - Backend: {BACKEND_URL}, Redis: {REDIS_URL}, Ollama: {OLLAMA_URL}"
)
