    ...
    async def load_model(self, model_id: str, device: str = "CPU") -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                f"{self.npu_endpoint}/models/load", 
                json=payload
            ) as response:
                return await response.json()
        except Exception as e:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    return {"error": result.get("detail", "Unknown error"), "success": False}
        except Exception as e:
    ...
    async def offload_heavy_processing(
        ...
        try:
            ...
            if not self.available:
                return {"success": False, "error": "NPU worker not available", "fallback": True}
            
            # For now, use standard inference endpoint with task-specific prompts
            if task_type == "text_analysis":
                prompt = f"Analyze the following text and extract key insights:\n\n{data.get('text', '')}"
                ...
                prompt = f"Process and summarize this knowledge data:\n\n{json.dumps(data.get('knowledge_data', {}))}"
                return await self.run_inference(
                    model_id=data.get('model_id', 'default'),
                    input_text=prompt,
