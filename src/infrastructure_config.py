...
USAGE:
    from src.infrastructure_config import config, cfg
    ...
    from src.infrastructure_config import BACKEND_HOST_IP, OLLAMA_PORT
"""
...
class ConsolidatedConfigSettings(BaseSettings):
    ...
    complete_config_file: str = Field(
        default="complete.yaml", env="COMPLETE_CONFIG_FILE"
    )
...
class ConsolidatedConfigManager:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "frontend": {"server_host": "localhost", "server_port": 5173},
    ...
    def _load_complete_config(self):
        ...
        try:
            complete_path = (
                Path(self.settings.config_dir) / self.settings.complete_config_file
            )
    ...
    def get_port(self, service_name: str) -> int:
        ...
        return self.get(
            f"infrastructure.ports.{service_name}",
            default_ports.get(service_name, 8000),
        )
    ...
    def get_service_url(self, service_name: str, protocol: str = None) -> str:
        ...
        # Issue #694: Use SSOT config instead of hardcoded IP
        if _SSOT_AVAILABLE:
            try:
                ssot = get_ssot_config()
                backend_ip = ssot.vm.main
            except Exception:
                backend_ip = "172.16.168.20"  # Fallback only if SSOT fails
        else:
            backend_ip = "172.16.168.20"  # Fallback if SSOT unavailable

        if service_name.lower() == "ollama" and backend_ip in host:
            host = "localhost"
    ...
    async def async_get(
        self, path: str, default: Any = None, use_cache: bool = True
    ) -> Any:
        ...
        try:
            ...
            if value is not None:
                await redis_set(
                    cache_key, json.dumps(value), expire=self.settings.cache_ttl
                )
...
def get_standardized_service_address(
    service_name: str, port: int, protocol: str = "http"
) -> str:
