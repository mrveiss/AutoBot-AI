...
class TerminalWebSocketManager:
    ...
    async def _set_state(self, new_state: TerminalState):
        ...
        async with self._state_lock:
            ...
            logger.debug(f"Terminal state changed: {old_state.value} -> {new_state.value}")
    ...
    async def _create_pty(self):
        ...
        try:
            ...
            attrs[0] &= ~(termios.ICRNL | termios.IXON)  # Disable auto CR/LF and XON/XOFF
    ...
    async def _start_background_tasks(self):
        ...
        self.output_sender_task = asyncio.create_task(
            self._output_sender_loop(),
            name=f"OutputSender-{self.terminal_type}"
        )
        ...
        try:
            while True:
                ...
                try:
                    ...
                    ready, _, error = select.select([self.pty_fd], [], [self.pty_fd], 0.1)
