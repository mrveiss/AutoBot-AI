...
USAGE:
    from src.utils.redis_consolidated import get_redis_client, AsyncRedisManager
...
class ConsolidatedAsyncRedisManager:
    ...
    async def _load_configuration(self, config_path: Optional[str] = None):
        ...
        logger.info(f"Redis configuration loaded: {base_config.host}:{base_config.port}")
    ...
    @asynccontextmanager
    async def get_connection(self, database: Union[str, RedisDatabase] = "main") -> AsyncGenerator[AsyncRedis, None]:
        """
    ...
    async def get_client(self, database: Union[str, RedisDatabase] = "main") -> AsyncRedis:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.info(f"Redis reconnected successfully for database: {database}")
                except Exception as reconnect_error:
                    logger.error(f"Redis reconnection failed for {database}: {reconnect_error}")
...
async def redis_get(key: str, database: Union[str, RedisDatabase] = "main") -> Optional[Any]:
