...
class DistributedRedisClient:
    ...
    def _create_connection(self) -> Optional[redis.Redis]:
        ...
        try:
            ...
            logger.info(f"✅ Connected to Redis VM at {self.redis_host}:{self.redis_port}")
            self._connection_attempts = 0
            return client
            
        except redis.ConnectionError as e:
            if self._connection_attempts <= self._max_retries:
                logger.warning(f"Redis VM connection attempt {self._connection_attempts}/{self._max_retries} failed: {e}")
            else:
                logger.error(f"❌ Failed to connect to Redis VM after {self._max_retries} attempts")
            return None
