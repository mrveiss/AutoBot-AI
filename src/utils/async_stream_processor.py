        ...
        async for chunk_bytes in response.content:
            ...
            if is_complete:
                if processor.is_natural_completion(chunk_data, accumulated_content):
                    ...
                    logger.info(f"✅ Stream completed naturally after {chunk_count} chunks")
                else:
                    completion_signal = StreamCompletionSignal.PROVIDER_SPECIFIC
                    logger.info(f"✅ Stream completed (provider signal) after {chunk_count} chunks")
                break
...
async def process_stream_with_cancellation(
    ...
    try:
        ...
        return StreamProcessingResult(
            ...
            completion_signal=StreamCompletionSignal.DONE_CHUNK_RECEIVED if success else StreamCompletionSignal.ERROR_CONDITION,
