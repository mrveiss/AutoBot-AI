...
class PerformanceOptimizer:
    ...
    async def apply_all_optimizations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["optimizations"]["error"] = {"status": "failed", "error": str(e)}
            return results
    ...
    async def benchmark_improvements(self) -> Dict[str, Any]:
        ...
        benchmarks = {"timestamp": asyncio.get_event_loop().time(), "tests": {}}
        ...
        try:
            ...
            if self.llm_interface:
                ...
                content, success = await self.llm_interface.stream_ollama_request(
                    test_url, test_data
                )
            ...
            if self.memory_manager:
                ...
                for i in range(1500):  # Exceed max size
                    self.memory_manager.put_in_cache(
                        "benchmark_cache", f"key_{i}", f"data_{i}" * 100
                    )
                ...
                benchmarks["tests"]["memory_management"] = {
                    ...
                    "cache_hit_rate": cache_stats.get("benchmark_cache", {}).get(
                        "hit_rate", 0
                    ),
                    "memory_stable": abs(
                        memory_after["percent"] - memory_before["percent"]
                    )
                    < 2.0,
                    "lru_eviction_working": cache_stats.get("benchmark_cache", {}).get(
                        "size", 0
                    )
                    <= 1000,
                }
