...
class KnowledgeCache:
    ...
    async def _get_redis_client(self):
        ...
        if self._redis_client is None:
            try:
                ...
                self.logger.debug(f"Knowledge cache Redis client initialized for DB {self._cache_db}")
            except Exception as e:
                self.logger.error(f"Failed to initialize knowledge cache Redis client: {e}")
                self._redis_client = None
    ...
    async def get_cached_results(self, query: str, top_k: int, filters: Optional[Dict] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if results_json:
                ...
                cache_time = float(cached_data.get(b'timestamp') or cached_data.get('timestamp', 0))
    ...
    async def cache_results(self, query: str, top_k: int, results: List[Dict[str, Any]], filters: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Cached {len(results)} results for query: '{query}' (top_k={top_k})")
            return True
    ...
    async def _manage_cache_size(self, redis_client):
        ...
        try:
            ...
            if oldest_keys:
                ...
                keys_to_remove = oldest_keys[:len(oldest_keys) - self._max_cache_size + 100]  # Remove extra for buffer
                
                if keys_to_remove:
                    keys_only = [key for _, key in keys_to_remove]
                    await redis_client.delete(*keys_only)
                    self.logger.info(f"Removed {len(keys_only)} old cache entries to manage cache size")
    ...
    async def clear_cache(self, pattern: Optional[str] = None) -> int:
        ...
        try:
            ...
            async for key in redis_client.scan_iter(match=search_pattern, count=batch_size):
            ...
            self.logger.info(f"Cleared {deleted_count} cache entries with pattern: {search_pattern}")
        ...
        try:
            ...
            async for key in redis_client.scan_iter(match=f"{self._cache_prefix}*"):
                ...
                try:
                    ...
                    if key_info:
                        timestamp_data = key_info.get(b'timestamp') or key_info.get('timestamp')
                        if timestamp_data:
                            timestamp = float(timestamp_data)
            ...
            stats = {
                ...
                "cache_utilization": f"{cache_count / self._max_cache_size * 100:.1f}%" if self._max_cache_size > 0 else "N/A"
            }
