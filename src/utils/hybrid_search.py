...
class HybridSearchEngine:
    ...
    def __init__(self, knowledge_base=None):
        ...
        self.stop_words = set(cfg.get('search.hybrid.stop_words', [
    ...
    def calculate_keyword_score(self, query_keywords: List[str], document_text: str, 
    ...
    async def explain_search(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, result in enumerate(semantic_results[:top_k]):
                content = result.get('content', '')[:200] + "..."  # Truncate for readability
                semantic_score = result.get('score', 0.0)
                
                keyword_score = self.calculate_keyword_score(
                    query_keywords, result.get('content', ''), result.get('metadata', {})
                )
