...
class WebSocketManager:
    ...
    async def _check_connection_health(self, connection_id: str, current_time: float):
        ...
        last_heartbeat_received = self.last_heartbeat_received.get(connection_id, current_time)
    ...
    async def _send_heartbeat(self, connection_id: str, websocket: WebSocket, current_time: float):
    ...
    async def disconnect(self, connection_id: str):
        ...
        if connection_id in self.active_connections:
            ...
            try:
                ...
                if self.connection_states.get(connection_id) not in [ConnectionState.ERROR, ConnectionState.DISCONNECTED]:
                    await websocket.send_json({
                        "type": "disconnecting",
                        "connection_id": connection_id,
                        "timestamp": time.time()
    ...
    async def handle_message(self, connection_id: str, message: str) -> bool:
        ...
        try:
            ...
            if message_type == "heartbeat_response":
                await self._handle_heartbeat_response(connection_id, message_data, current_time)
                return True
    ...
    async def _handle_heartbeat_response(self, connection_id: str, message_data: Dict[str, Any], current_time: float):
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        for connection_id in self.active_connections.keys():
            ...
            last_heartbeat = self.last_heartbeat_received.get(connection_id, current_time)
