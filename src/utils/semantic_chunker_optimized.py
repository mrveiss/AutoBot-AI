...
class GPUOptimizedSemanticChunker:
    ...
    def __init__(
        ...
        self.cpu_workers = cpu_workers or min(
            16, os.cpu_count() or 4
        )  # Use more workers for 22-core CPU
                ...
                def load_optimized_model():
                    ...
                    if device.startswith("cuda"):
                        try:
                            ...
                            logger.info(
                                "✅ Enabled FP16 precision for RTX 4070 Tensor Cores"
                            )
                            ...
                            logger.info(
                                "✅ Enabled cuDNN optimization for consistent batching"
                            )
                    ...
                    logger.info(
                        f"✅ Model loaded in {load_time:.2f}s on {model.device}"
                    )
    ...
    async def _compute_embeddings_gpu_optimized(
        ...
        logger.debug(
            f"Computing embeddings for {total_sentences} sentences using GPU optimization"
        )
        ...
        try:
            ...
            if self._device_info and self._device_info["device"].startswith("cuda"):
                ...
                if total_sentences > 1000:
                    batch_size = min(
                        self.gpu_batch_size, 400
                    )  # Large batches for bulk processing
                elif total_sentences > 100:
                    batch_size = min(self.gpu_batch_size, 300)  # Medium batches
                else:
                    batch_size = min(
                        self.gpu_batch_size, total_sentences
                    )  # Small batches
            else:
            ...
            for i in range(0, total_sentences, batch_size):
                ...
                with ThreadPoolExecutor(
                    max_workers=2
                ) as executor:  # Limit to avoid GPU contention
                    batch_embeddings = await loop.run_in_executor(
                        executor, compute_batch
                    )
                ...
                if i % (batch_size * 3) == 0:  # Every 3 batches
                    ...
                    if total_sentences > 100:
                        progress = min(
                            100, int((i + batch_size) / total_sentences * 100)
                        )
            ...
            sentences_per_second = (
                total_sentences / processing_time if processing_time > 0 else 0
            )
            ...
            self.stats["avg_processing_time"] = (
                self.stats["avg_processing_time"]
                * (self.stats["total_chunks_created"] or 1)
                + processing_time
            ) / (self.stats["total_chunks_created"] + 1)

            logger.info(
                f"✅ GPU embeddings computed: {total_sentences} sentences in {processing_time:.2f}s ({sentences_per_second:.1f} sent/s)"
            )
    ...
    def _advanced_sentence_splitting(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if (
                len(sentence) > 10 and len(sentence.split()) >= 3
            ):  # Minimum viable sentence
                cleaned_sentences.append(sentence)

        return cleaned_sentences

    def _compute_semantic_distances_optimized(
    ...
    def _find_chunk_boundaries_optimized(self, distances: List[float]) -> List[int]:
        ...
        for i, distance in enumerate(distances):
            if (
                distance > threshold and i - last_boundary >= 3
            ):  # Avoid too frequent splits
        ...
        for boundary in all_boundaries:
            ...
            if len(chunk_content) < self.min_chunk_size and chunks:
                ...
                chunks[-1] = OptimizedSemanticChunk(
                    ...
                    semantic_score=self._calculate_coherence_from_embeddings(
                        merged_embeddings
                    ),
    ...
    async def chunk_document_optimized(
        ...
        for chunk in optimized_chunks:
            ...
            if chunk.embedding_vector is not None:
                ...
                doc_data["metadata"]["embedding_dimension"] = len(
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "optimization_level": (
                "gpu_maximum"
                if device_info.get("device", "").startswith("cuda")
                else "cpu_fallback"
            ),
        }
