...
from src.models.entity_mapping import (
    EntityMapping, EntityResolutionResult, EntityType, 
    SimilarityMethod, EntityRelationship
)
...
class EntityResolver:
    ...
    async def resolve_entities(self, 
                             ...
                             context: Optional[Dict[str, Any]] = None) -> EntityResolutionResult:
        """
    ...
    async def _resolve_entity_batch(self, 
                                  entity_names: List[str], 
                                  existing_mappings: Dict[str, EntityMapping]) -> Dict[str, EntityMapping]:
        """
    ...
    async def _find_exact_match(self, 
                               entity_name: str, 
                               existing_mappings: Dict[str, EntityMapping]) -> Optional[EntityMapping]:
    ...
    async def _find_similar_entity(self, 
                                  entity_name: str, 
                                  existing_mappings: Dict[str, EntityMapping]) -> Optional[EntityMapping]:
    ...
    async def _calculate_fuzzy_similarity(self, 
                                        entity_name: str, 
                                        mapping: EntityMapping) -> float:
        ...
        canonical_similarity = SequenceMatcher(
            ...
            mapping.canonical_name.lower().strip()
        ).ratio()
    ...
    def _classify_entity_type(self, entity_name: str) -> EntityType:
        ...
        tech_patterns = [
            ...
            'python', 'javascript', 'java', 'redis', 'chromadb', 'openai',
            'model', 'algorithm', 'neural', 'transformer', 'gpt', 'llm'
        ]
        ...
        elif any(pattern in name_lower for pattern in version_patterns) and any(c.isdigit() for c in entity_name):
    ...
    async def _load_entity_mappings(self) -> Dict[str, EntityMapping]:
        ...
        try:
            ...
            for mapping_id, mapping_json in mapping_data.items():
                try:
                    mapping_dict = eval(mapping_json)  # Safe eval for simple dict
    ...
    async def _record_resolution_history(self, 
                                       result: EntityResolutionResult, 
    ...
    async def get_resolution_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for history_str in recent_history:
                try:
                    history = eval(history_str)  # Safe eval for simple dict
            ...
            avg_processing_time = total_processing_time / total_resolutions if total_resolutions > 0 else 0
            avg_resolution_rate = (
                (total_entities_processed - total_entities_resolved) / total_entities_processed * 100
                if total_entities_processed > 0 else 0
            )
