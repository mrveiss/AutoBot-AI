...
class OptimizedRedisConnectionManager:
    ...
    @asynccontextmanager
    async def get_managed_client(self, host: str, port: int, db: int, password: Optional[str] = None):
        ...
        for pool_key, pool in self.connection_pools.items():
            try:
                ...
                if available_count_before != available_count_after:
                    logger.info(f"Cleaned {available_count_before - available_count_after} idle connections from pool {pool_key}")
                    
            except Exception as e:
                logger.error(f"Error cleaning idle connections for pool {pool_key}: {e}")
...
async def with_redis_client(host: str, port: int, db: int, password: Optional[str] = None):
