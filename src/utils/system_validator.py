...
class SystemValidator:
    ...
    def __init__(self):
        ...
        self.critical_thresholds = {
            ...
            'memory_usage_percent': cfg.get('validation.thresholds.memory_usage_percent', 90.0),
            'cpu_usage_percent': cfg.get('validation.thresholds.cpu_usage_percent', 95.0),
            'error_rate_percent': cfg.get('validation.thresholds.error_rate_percent', 5.0)
        }
    ...
    def _add_result(self, component: str, test_name: str, severity: ValidationSeverity, 
                    status: bool, message: str, details: Dict[str, Any] = None, 
                    duration_ms: float = 0.0):
    ...
    async def validate_knowledge_base_caching(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if cache:
                self._add_result(component, "Cache Initialization", ValidationSeverity.SUCCESS, 
                               True, "Cache system initialized successfully", 
                               {"init_time_ms": init_time}, init_time)
            else:
                self._add_result(component, "Cache Initialization", ValidationSeverity.CRITICAL,
                               False, "Failed to initialize cache system")
                return results
            ...
            if cache_success and retrieved_results:
                self._add_result(component, "Cache Operations", ValidationSeverity.SUCCESS,
                               True, "Cache store/retrieve operations working", 
                               {"cache_time_ms": cache_time}, cache_time)
            else:
                self._add_result(component, "Cache Operations", ValidationSeverity.CRITICAL,
                               False, "Cache operations failed")
            
            # Test 3: Cache statistics
            try:
                stats = await cache.get_cache_stats()
                if 'cache_entries' in stats:
                    self._add_result(component, "Cache Statistics", ValidationSeverity.SUCCESS,
                                   True, f"Cache stats available: {stats.get('cache_entries', 0)} entries",
                                   stats)
                else:
                    self._add_result(component, "Cache Statistics", ValidationSeverity.WARNING,
                                   False, "Cache statistics not available", stats)
            except Exception as e:
                self._add_result(component, "Cache Statistics", ValidationSeverity.WARNING,
                               False, f"Cache statistics error: {str(e)}")
            ...
            try:
                ...
                if kb.index:
                    ...
                    if search_results:
                        severity = ValidationSeverity.SUCCESS if search_time < self.critical_thresholds['response_time_ms'] else ValidationSeverity.WARNING
                        self._add_result(component, "KB Integration", severity, True,
                                       f"Knowledge base search working: {len(search_results)} results",
                                       {"search_time_ms": search_time, "result_count": len(search_results)},
                                       search_time)
                    else:
                        self._add_result(component, "KB Integration", ValidationSeverity.WARNING,
                                       True, "Knowledge base search returned no results")
                else:
                    self._add_result(component, "KB Integration", ValidationSeverity.WARNING,
                                   False, "Knowledge base index not available")
            except Exception as e:
                self._add_result(component, "KB Integration", ValidationSeverity.WARNING,
                               False, f"Knowledge base integration error: {str(e)}")
            
            # Cleanup test data
            await cache.clear_cache("*validation_test*")
            
        except Exception as e:
            self._add_result(component, "System Error", ValidationSeverity.CRITICAL,
                           False, f"Knowledge base cache validation failed: {str(e)}")
    ...
    async def validate_hybrid_search(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if not kb.index:
                self._add_result(component, "Prerequisites", ValidationSeverity.WARNING,
                               False, "Knowledge base not available for hybrid search testing")
                return [r for r in self.results if r.component == component]
            
            hybrid_engine = get_hybrid_search_engine(kb)
            init_time = (time.time() - start_time) * 1000
            
            self._add_result(component, "Engine Initialization", ValidationSeverity.SUCCESS,
                           True, "Hybrid search engine initialized", 
                           {"init_time_ms": init_time}, init_time)
            ...
            for query in test_queries:
                ...
                if keywords and len(keywords) >= 2:
                    self._add_result(component, "Keyword Extraction", ValidationSeverity.SUCCESS,
                                   True, f"Keywords extracted: {len(keywords)} from query",
                                   {"query": query[:50] + "...", "keywords": keywords})
                else:
                    self._add_result(component, "Keyword Extraction", ValidationSeverity.WARNING,
                                   False, f"Poor keyword extraction: {len(keywords)} keywords",
                                   {"query": query[:50] + "...", "keywords": keywords})
            ...
            if search_results:
                ...
                if has_hybrid_scores and has_keyword_scores:
                    severity = ValidationSeverity.SUCCESS if search_time < self.critical_thresholds['response_time_ms'] else ValidationSeverity.WARNING
                    self._add_result(component, "Hybrid Search", severity, True,
                    ...
                    self._add_result(component, "Hybrid Search", ValidationSeverity.WARNING,
                                   True, "Hybrid search working but missing enhanced scoring",
                                   {"search_time_ms": search_time, "result_count": len(search_results)})
            else:
                self._add_result(component, "Hybrid Search", ValidationSeverity.WARNING,
                               False, "Hybrid search returned no results")
            ...
            try:
                ...
                if 'extracted_keywords' in explanation:
                    self._add_result(component, "Search Explanation", ValidationSeverity.SUCCESS,
                                   True, "Search explanation functionality working",
                                   {"keywords_count": len(explanation.get('extracted_keywords', []))})
                else:
                    self._add_result(component, "Search Explanation", ValidationSeverity.WARNING,
                                   False, "Search explanation missing keywords")
            except Exception as e:
                self._add_result(component, "Search Explanation", ValidationSeverity.WARNING,
                               False, f"Search explanation error: {str(e)}")
            
        except Exception as e:
            self._add_result(component, "System Error", ValidationSeverity.CRITICAL,
                           False, f"Hybrid search validation failed: {str(e)}")
    ...
    async def validate_monitoring_system(self) -> List[ValidationResult]:
        ...
        try:
            ...
            self._add_result(component, "Collector Initialization", ValidationSeverity.SUCCESS,
                           True, "Metrics collector initialized", 
                           {"init_time_ms": init_time}, init_time)
            ...
            if len(found_metrics) == len(expected_metrics):
                ...
                self._add_result(component, "System Metrics", severity, True,
                ...
                self._add_result(component, "System Metrics", ValidationSeverity.WARNING,
                               False, f"Missing system metrics: {len(found_metrics)}/{len(expected_metrics)}",
            ...
            if service_count >= 2:  # At least backend and redis health
                self._add_result(component, "Service Health", ValidationSeverity.SUCCESS,
                               True, f"Service health metrics collected: {service_count} services",
                               {"collection_time_ms": health_time, "services": service_count},
                               health_time)
            else:
                self._add_result(component, "Service Health", ValidationSeverity.WARNING,
                               False, f"Limited service health coverage: {service_count} services")
            ...
            if storage_success:
                self._add_result(component, "Metrics Storage", ValidationSeverity.SUCCESS,
                               True, f"Metrics storage working: {len(all_metrics)} metrics stored",
                               {"storage_time_ms": storage_time, "metrics_count": len(all_metrics)},
                               storage_time)
            else:
                self._add_result(component, "Metrics Storage", ValidationSeverity.WARNING,
                               False, "Metrics storage failed")
            ...
            try:
                ...
                if 'overall_health' in summary:
                    ...
                    self._add_result(component, "Health Summary", severity, True,
                                   f"System health summary available: {health_score}% health",
                                   {"health_score": health_score, "status": summary['overall_health'].get('status')})
                else:
                    self._add_result(component, "Health Summary", ValidationSeverity.INFO,
                                   True, "Health summary available without overall score")
            except Exception as e:
                self._add_result(component, "Health Summary", ValidationSeverity.WARNING,
                               False, f"Health summary error: {str(e)}")
            
        except Exception as e:
            self._add_result(component, "System Error", ValidationSeverity.CRITICAL,
                           False, f"Monitoring system validation failed: {str(e)}")
    ...
    async def validate_model_optimization(self) -> List[ValidationResult]:
        ...
        try:
            ...
            self._add_result(component, "Optimizer Initialization", ValidationSeverity.SUCCESS,
                           True, "Model optimizer initialized", 
                           {"init_time_ms": init_time}, init_time)
            ...
            if models and len(models) >= 2:
                self._add_result(component, "Model Discovery", ValidationSeverity.SUCCESS,
                               True, f"Models discovered: {len(models)} available",
                               {"discovery_time_ms": discovery_time, "model_count": len(models),
                                "models": [m.name for m in models[:5]]}, discovery_time)
            elif models and len(models) == 1:
                self._add_result(component, "Model Discovery", ValidationSeverity.WARNING,
                               True, f"Limited models available: {len(models)}",
                               {"discovery_time_ms": discovery_time, "models": [m.name for m in models]})
            else:
                self._add_result(component, "Model Discovery", ValidationSeverity.CRITICAL,
                               False, "No models discovered - is Ollama running?")
                return [r for r in self.results if r.component == component]
            ...
            self._add_result(component, "Complexity Analysis", severity, accuracy >= 50,
            ...
            if selected_model:
                self._add_result(component, "Model Selection", ValidationSeverity.SUCCESS,
                               True, f"Model selection working: {selected_model}",
                               {"selection_time_ms": selection_time, "selected_model": selected_model},
                               selection_time)
            else:
                self._add_result(component, "Model Selection", ValidationSeverity.WARNING,
                               False, "Model selection returned no result")
            ...
            if selected_model:
                try:
                    ...
                    self._add_result(component, "Performance Tracking", ValidationSeverity.SUCCESS,
                                   True, "Model performance tracking working")
                except Exception as e:
                    self._add_result(component, "Performance Tracking", ValidationSeverity.WARNING,
                                   False, f"Performance tracking error: {str(e)}")
            
            # Test 6: Optimization suggestions
            try:
                suggestions = await optimizer.get_optimization_suggestions()
                self._add_result(component, "Optimization Suggestions", ValidationSeverity.SUCCESS,
                               True, f"Optimization suggestions generated: {len(suggestions)}",
                               {"suggestion_count": len(suggestions)})
            except Exception as e:
                self._add_result(component, "Optimization Suggestions", ValidationSeverity.WARNING,
                               False, f"Optimization suggestions error: {str(e)}")
            
        except Exception as e:
            self._add_result(component, "System Error", ValidationSeverity.CRITICAL,
                           False, f"Model optimization validation failed: {str(e)}")
                        ...
                        if method == "GET":
                            async with session.get(url) as response:
                                ...
                                if response.status == 200:
                                    ...
                                    severity = ValidationSeverity.SUCCESS if response_time < self.critical_thresholds['response_time_ms'] else ValidationSeverity.WARNING
                                    self._add_result(component, name, severity, True,
                                                   f"API endpoint responding: {response.status}",
                                                   {"response_time_ms": response_time, "status_code": response.status,
                                                    "response_size": len(str(data))}, response_time)
                                else:
                                    self._add_result(component, name, ValidationSeverity.WARNING,
                                                   False, f"API endpoint error: HTTP {response.status}",
                                                   {"response_time_ms": response_time, "status_code": response.status})
                    
                    except asyncio.TimeoutError:
                        self._add_result(component, name, ValidationSeverity.CRITICAL,
                                       False, "API endpoint timeout", {"timeout_seconds": 10})
                    except Exception as e:
                        self._add_result(component, name, ValidationSeverity.CRITICAL,
                                       False, f"API endpoint error: {str(e)}")
            
        except Exception as e:
            self._add_result(component, "System Error", ValidationSeverity.CRITICAL,
                           False, f"API endpoint validation failed: {str(e)}")
    ...
    async def validate_system_resources(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if cpu_percent < self.critical_thresholds['cpu_usage_percent']:
                ...
                self._add_result(component, "CPU Usage", severity, True,
                               f"CPU usage acceptable: {cpu_percent:.1f}%",
                               {"cpu_percent": cpu_percent, "threshold": self.critical_thresholds['cpu_usage_percent']})
            else:
                self._add_result(component, "CPU Usage", ValidationSeverity.WARNING,
                               False, f"High CPU usage: {cpu_percent:.1f}%",
                               {"cpu_percent": cpu_percent, "threshold": self.critical_thresholds['cpu_usage_percent']})
            ...
            if memory.percent < self.critical_thresholds['memory_usage_percent']:
                ...
                self._add_result(component, "Memory Usage", severity, True,
                ...
                self._add_result(component, "Memory Usage", ValidationSeverity.WARNING,
                               False, f"High memory usage: {memory.percent:.1f}%",
                               {"memory_percent": memory.percent, "available_gb": memory.available / (1024**3)})
            ...
            if disk_percent < 85:
                ...
                self._add_result(component, "Disk Usage", severity, True,
                ...
                self._add_result(component, "Disk Usage", severity, False,
            ...
            try:
                ...
                self._add_result(component, "Network Connectivity", ValidationSeverity.SUCCESS,
                               True, "Network connectivity available")
            except Exception:
                self._add_result(component, "Network Connectivity", ValidationSeverity.WARNING,
                               False, "Limited network connectivity")
            ...
            for host, port, service_name in critical_ports:
                try:
                    with socket.create_connection((host, port), timeout=3):
                        self._add_result(component, f"{service_name} Port", ValidationSeverity.SUCCESS,
                                       True, f"{service_name} service port accessible: {host}:{port}")
                except Exception:
                    self._add_result(component, f"{service_name} Port", ValidationSeverity.CRITICAL,
                                   False, f"{service_name} service port inaccessible: {host}:{port}")
            
        except Exception as e:
            self._add_result(component, "System Error", ValidationSeverity.CRITICAL,
                           False, f"System resource validation failed: {str(e)}")
    ...
    async def run_comprehensive_validation(self) -> SystemValidationReport:
        ...
        if total_tests > 0:
            ...
            overall_health_score = max(0, base_score - critical_penalty - warning_penalty)
        else:
        ...
        avg_response_time = sum(r.duration_ms for r in self.results if r.duration_ms > 0) / max(len([r for r in self.results if r.duration_ms > 0]), 1)
        
        performance_metrics = {
            "total_validation_time_seconds": round(total_duration, 2),
            "average_response_time_ms": round(avg_response_time, 2),
            "fastest_test_ms": min((r.duration_ms for r in self.results if r.duration_ms > 0), default=0),
            "slowest_test_ms": max((r.duration_ms for r in self.results if r.duration_ms > 0), default=0)
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if critical_results:
            recommendations.append(f"CRITICAL: Address {len(critical_results)} critical issues before production deployment")
        ...
        if slow_tests:
            recommendations.append(f"PERFORMANCE: {len(slow_tests)} operations are slower than {self.critical_thresholds['response_time_ms']}ms")
        ...
        if failed_by_component:
            ...
            recommendations.append(f"FOCUS: '{worst_component[0]}' has {worst_component[1]} failing tests - prioritize fixes here")
