...
class AsyncRedisManager:
    ...
    async def _wait_for_redis_ready(self, client: Redis, name: str, max_wait: int = 60) -> bool:
            ...
            try:
                ...
                if "loading" in error_msg or "busy" in error_msg:
                    if attempt % 10 == 0:  # Log every 10 seconds
                        logger.info(f"⏳ Redis '{name}' still loading dataset, waiting... ({attempt}s/{max_wait}s)")
        ...
        logger.error(f"❌ Redis '{name}' did not become ready within {max_wait} seconds")
        return False
    ...
    async def _create_connection(self, name: str, config: RedisConfig) -> None:
        ...
        try:
            ...
            if not await self._wait_for_redis_ready(client, name):
                raise aioredis.ConnectionError(f"Redis '{name}' did not become ready in time")
    ...
    async def _health_check_task(self, name: str, client: Redis, config: RedisConfig) -> None:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.error(f"Redis reconnection failed for {name}: {reconnect_error}")
    
    @asynccontextmanager
    async def get_client(self, database_name: str = "default") -> AsyncGenerator[Redis, None]:
