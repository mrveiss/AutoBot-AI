...
from src.utils.performance_monitor import autobot_monitor
...
class Phase9PerformanceMonitor:
            ...
            if result.returncode == 0:
                ...
                if len(parts) >= 8:
                    ...
                    return GPUMetrics(
                        ...
                        power_draw_watts=float(parts[5]) if parts[5] != "[Not Supported]" else 0.0,
                        gpu_clock_mhz=int(float(parts[6])) if parts[6] != "[Not Supported]" else 0,
                        memory_clock_mhz=int(float(parts[7])) if parts[7] != "[Not Supported]" else 0,
                        fan_speed_percent=int(float(parts[8])) if len(parts) > 8 and parts[8] != "[Not Supported]" else None,
                        encoder_utilization=int(float(parts[9])) if len(parts) > 9 and parts[9] != "[Not Supported]" else None,
                        decoder_utilization=int(float(parts[10])) if len(parts) > 10 and parts[10] != "[Not Supported]" else None,
    ...
    async def _get_npu_worker_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(f"http://{npu_host}:{npu_port}/stats") as response:
    ...
    async def collect_system_performance_metrics(self) -> SystemPerformanceMetrics:
        ...
        try:
            ...
            disk_write_speed = getattr(disk_io, 'write_bytes', 0) / (1024 * 1024)  # MB/s
            network_upload_speed = getattr(network_io, 'bytes_sent', 0) / (1024 * 1024)  # MB/s
            network_download_speed = getattr(network_io, 'bytes_recv', 0) / (1024 * 1024)  # MB/s
            ...
            return SystemPerformanceMetrics(
                timestamp=time.time(),
                cpu_usage_percent=0, cpu_cores_physical=0, cpu_cores_logical=0,
                cpu_frequency_mhz=0, cpu_load_1m=0, cpu_load_5m=0, cpu_load_15m=0,
                memory_total_gb=0, memory_used_gb=0, memory_available_gb=0,
                memory_usage_percent=0, swap_usage_percent=0, disk_read_mb_s=0,
                disk_write_mb_s=0, disk_usage_percent=0, disk_queue_depth=0,
                network_upload_mb_s=0, network_download_mb_s=0, network_latency_ms=0,
                network_packet_loss_percent=0, autobot_memory_usage_mb=0, autobot_cpu_usage_percent=0
            )
    ...
    async def _measure_network_latency(self) -> float:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(f"http://{backend_host}:8001/api/health") as response:
                    if response.status == 200:
    ...
    async def collect_service_performance_metrics(self) -> List[ServicePerformanceMetrics]:
        """Collect performance metrics for all distributed services"""
        services = []
        
        # Define services to monitor
        service_configs = [
            {"name": "Backend API", "host": cfg.get_host('backend'), "port": cfg.get_port('backend'), "path": "/api/health"},
            {"name": "Frontend", "host": cfg.get_host('frontend'), "port": cfg.get_port('frontend'), "path": "/"},
            {"name": "Redis", "host": cfg.get_host('redis'), "port": cfg.get_port('redis'), "path": None},
            {"name": "AI Stack", "host": cfg.get_host('ai_stack'), "port": cfg.get_port('ai_stack'), "path": "/health"},
            {"name": "NPU Worker", "host": cfg.get_host('npu_worker'), "port": cfg.get_port('npu_worker'), "path": "/health"},
            {"name": "Browser Service", "host": cfg.get_host('browser_service'), "port": cfg.get_port('browser_service'), "path": "/health"},
        ]
        ...
        for service_config in service_configs:
            try:
                ...
                self.logger.error(f"Error collecting metrics for {service_config['name']}: {e}")
        
        return services
    
    async def _collect_single_service_metrics(self, service_config: Dict[str, Any]) -> Optional[ServicePerformanceMetrics]:
    ...
    async def collect_all_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            gpu_metrics, npu_metrics, multimodal_metrics, system_metrics, service_metrics = results
        ...
        try:
            ...
            for category, metric_data in metrics.items():
                if metric_data:
                    ...
                    value = json.dumps(asdict(metric_data) if hasattr(metric_data, '__dict__') else metric_data, default=str)
    ...
    def _log_performance_summary(self, metrics: Dict[str, Any]):
        ...
        try:
            ...
            if metrics.get("system"):
                system = metrics["system"]
                summary_parts.append(f"CPU: {system['cpu_usage_percent']:.0f}%, MEM: {system['memory_usage_percent']:.0f}%")
        ...
        try:
            ...
            if len(self.system_metrics_buffer) >= 5:
                ...
                trends["cpu_load"] = {
                    ...
                    "trend": "increasing" if loads[-1] > loads[0] else "decreasing" if loads[-1] < loads[0] else "stable"
                }
            ...
            try:
                ...
                if phase9_monitor.redis_client:
                    try:
                        ...
                        phase9_monitor.redis_client.zadd(key, {json.dumps({
                ...
                execution_time = time.time() - start_time
                logger.error(f"PERFORMANCE [{category}]: {func.__name__} failed after {execution_time:.3f}s: {e}")
                raise
