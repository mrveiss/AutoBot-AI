...
class ModelOptimizer:
    ...
    def __init__(self):
        ...
        self._ollama_base_url = f"http://{cfg.get_host('ollama')}:{cfg.get_port('ollama')}"
        
        # Configuration
        self._performance_threshold = cfg.get('llm.optimization.performance_threshold', 0.8)
        ...
        self.model_classifications = {
            ...
            ModelPerformanceLevel.LIGHTWEIGHT: [
                'llama3.2:1b', 'gemma3:270m', 'gemma3:1b', 'nomic-embed-text'
            ],
            # Standard models (2-8B parameters)
            ModelPerformanceLevel.STANDARD: [
                'llama3.2:3b', 'gemma2:2b', 'gemma3:latest', 'artifish/llama3.2-uncensored'
            ],
            # Advanced models (8B+ parameters)
            ModelPerformanceLevel.ADVANCED: [
                'dolphin-llama3:8b', 'wizard-vicuna-uncensored:13b'
            ]
        }
        ...
        self.complexity_keywords = {
            ...
            TaskComplexity.SPECIALIZED: [
                'research paper', 'scientific', 'mathematical proof', 'legal analysis',
                'medical', 'financial modeling', 'advanced algorithms'
            ]
        }
        ...
        if self._redis_client is None:
            try:
                self._redis_client = get_redis_client(
                    async_client=True,
                    db=cfg.get('redis.databases.llm_cache.db', 5)
                )
                if asyncio.iscoroutine(self._redis_client):
                    self._redis_client = await self._redis_client
            except Exception as e:
                self.logger.error(f"Failed to initialize Redis client for model optimization: {e}")
                self._redis_client = None
                ...
                async with session.get(f"{self._ollama_base_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        for model_data in data.get('models', []):
                            ...
                            performance_level = self._classify_model_performance(name, parameter_size)
    ...
    def _classify_model_performance(self, name: str, parameter_size: str) -> ModelPerformanceLevel:
    ...
    async def _load_model_performance_history(self, model_info: ModelInfo):
        ...
        try:
            ...
            if perf_data:
                model_info.avg_tokens_per_second = float(perf_data.get(b'avg_tokens_per_second') or perf_data.get('avg_tokens_per_second', 0))
                model_info.avg_response_time = float(perf_data.get(b'avg_response_time') or perf_data.get('avg_response_time', 0))
            ...
            self.logger.error(f"Error loading performance history for {model_info.name}: {e}")
    
    async def _save_model_performance(self, model_name: str, response_time: float, 
    ...
    def analyze_task_complexity(self, task_request: TaskRequest) -> TaskComplexity:
        ...
        if task_type in ['code', 'programming', 'development']:
            if any(word in query_lower for word in ['complex', 'algorithm', 'optimize', 'architecture']):
    ...
    async def select_optimal_model(self, task_request: TaskRequest) -> Optional[str]:
        ...
        try:
            ...
            suitable_models = self._filter_models_by_complexity(complexity, list(self._models_cache.values()))
            
            if not suitable_models:
                self.logger.warning(f"No suitable models found for complexity: {complexity}")
                return list(self._models_cache.keys())[0]  # Fallback to first available
            
            # Filter by resource constraints
            resource_filtered = self._filter_models_by_resources(suitable_models, resources)
            
            if not resource_filtered:
                self.logger.warning("No models pass resource filtering, using least resource-intensive")
                resource_filtered = sorted(suitable_models, key=lambda m: m.size_gb)[:1]
            
            # Score and rank models
            ranked_models = self._rank_models_by_performance(resource_filtered, task_request)
    ...
    def _filter_models_by_complexity(self, complexity: TaskComplexity, models: List[ModelInfo]) -> List[ModelInfo]:
        ...
        if complexity == TaskComplexity.SIMPLE:
            ...
            return [m for m in models if m.performance_level in [
                ModelPerformanceLevel.STANDARD, ModelPerformanceLevel.ADVANCED, ModelPerformanceLevel.SPECIALIZED
            ]]
        elif complexity == TaskComplexity.COMPLEX:
            # Complex tasks need advanced models
            return [m for m in models if m.performance_level in [
                ModelPerformanceLevel.ADVANCED, ModelPerformanceLevel.SPECIALIZED
            ]]
        else:  # SPECIALIZED
            # Specialized tasks prefer the most capable models
            advanced_models = [m for m in models if m.performance_level == ModelPerformanceLevel.ADVANCED]
            return advanced_models if advanced_models else models
    
    def _filter_models_by_resources(self, models: List[ModelInfo], resources: Dict[str, float]) -> List[ModelInfo]:
    ...
    def _rank_models_by_performance(self, models: List[ModelInfo], task_request: TaskRequest) -> List[ModelInfo]:
        ...
        for model in models:
            ...
            if task_request.user_preference and task_request.user_preference in model.name:
                score += 10
    ...
    async def track_model_performance(self, model_name: str, response_time: float, 
                                    response_tokens: int, success: bool):
        """Track model performance for future optimization"""
        try:
            tokens_per_second = response_tokens / response_time if response_time > 0 else 0
            await self._save_model_performance(model_name, response_time, tokens_per_second, success)
            ...
            if model_name in self._models_cache:
                ...
                if model.use_count > 0:
                    ...
                    model.avg_response_time = (model.avg_response_time * model.use_count + response_time) / total_count
                    model.avg_tokens_per_second = (model.avg_tokens_per_second * model.use_count + tokens_per_second) / total_count
                    model.success_rate = (model.success_rate * model.use_count + (1.0 if success else 0.0)) / total_count
                else:
    ...
    async def get_optimization_suggestions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            models_with_history = [m for m in self._models_cache.values() if m.use_count > 0]
            ...
            underperforming = [m for m in models_with_history if m.success_rate < avg_success_rate * 0.8]
            ...
            if models_with_history:
                ...
                slow_models = [m for m in models_with_history if m.avg_response_time > avg_response_time * 1.5]
            ...
            lightweight_overused = [m for m in models_with_history 
                                  if m.performance_level == ModelPerformanceLevel.LIGHTWEIGHT and m.use_count > 50]
            ...
            if resources['memory_percent'] < 70 and resources['cpu_percent'] < 60:
                large_models = [m for m in self._models_cache.values() 
