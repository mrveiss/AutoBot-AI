...
class LongTermMemoryManager:
    ...
    def __init__(self, config_path="config/config.yaml"):
        ...
        self.max_entries_per_category = memory_config.get('max_entries_per_category', 10000)
    ...
    def _init_memory_db(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_task_type ON task_logs(task_type)")
    ...
    def store_memory(self, category: str, content: str, metadata: Optional[Dict[str, Any]] = None,
                    reference_path: Optional[str] = None, embedding: Optional[List[float]] = None) -> int:
        """
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (category, content, metadata_json, reference_path, embedding_blob, content_hash))
    ...
    def retrieve_memory(self, category: Optional[str] = None, limit: int = 100,
                       days_back: Optional[int] = None) -> List[MemoryEntry]:
        """
    ...
    def store_task_log(self, task_id: str, task_type: str, status: str, description: str,
                      result: Optional[str] = None, execution_time_ms: Optional[int] = None,
                      error_message: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> int:
    ...
    def store_agent_state(self, state_name: str, state_data: Dict[str, Any], phase: int,
                         active_tasks: List[str], system_status: str) -> int:
    ...
    def store_config_change(self, config_section: str, old_value: str, new_value: str,
                           changed_by: str, change_reason: str) -> int:
        ...
        try:
            ...
            logging.info(f"Stored config change {change_id} for section '{config_section}'")
            return change_id
    ...
    def store_conversation(self, session_id: str, role: str, message: str,
    ...
    def link_markdown_reference(self, memory_entry_id: int, file_path: str,
                               line_start: int, line_end: int, content_excerpt: str) -> int:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (file_path, line_start, line_end, content_excerpt, memory_entry_id, last_modified.isoformat()))
            ...
            logging.info(f"Linked markdown reference {ref_id} to memory entry {memory_entry_id}")
            return ref_id
    ...
    def search_memory(self, query: str, category: Optional[str] = None) -> List[MemoryEntry]:
        """
        ...
        try:
            ...
            logging.info(f"Found {len(memories)} memory entries matching query: '{query}'")
            return memories
    ...
    def cleanup_old_entries(self) -> int:
        ...
        try:
            ...
            cursor.execute("DELETE FROM task_logs WHERE completed_at < ?", (cutoff_date.isoformat(),))
            
            # Clean up old conversations (keep more recent ones)
            conversation_cutoff = datetime.now() - timedelta(days=self.retention_days // 2)
            cursor.execute("DELETE FROM conversations WHERE timestamp < ?", (conversation_cutoff.isoformat(),))
...
def log_task_completion(task_id: str, task_type: str, description: str, 
                       result: str, execution_time_ms: int) -> int:
