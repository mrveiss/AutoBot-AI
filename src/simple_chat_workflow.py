...
class SimpleChatWorkflow:
    ...
    async def process_message(self, user_message: str, chat_id: Optional[str] = None) -> SimpleWorkflowResult:
        """
        ...
        try:
            ...
            await self.send_workflow_message("thought", "🤔 I'm analyzing your message...")
            ...
            await self.send_workflow_message("planning", "📋 Planning my response approach...")
            await asyncio.sleep(0.5)  # Brief pause for realism
            
            # Step 3: Show classification
            await self.send_workflow_message("debug", "🔍 WORKFLOW: Classifying message type...")
            message_type = MessageType.GENERAL_QUERY
            await self.send_workflow_message("utility", f"✅ Classified as: {message_type.value}")
            ...
            await self.send_workflow_message("debug", "🔍 WORKFLOW: Searching knowledge base...")
            await asyncio.sleep(0.3)
            await self.send_workflow_message("utility", "📚 Knowledge base search completed")
            
            # Step 5: Show LLM interaction
            logger.info("DEBUG: Starting LLM interaction step")
            await self.send_workflow_message("thought", "🧠 Generating response using LLM...")
            await self.send_workflow_message("debug", "🔗 WORKFLOW: Connecting to Ollama...")
            ...
            await self.send_workflow_message("debug", "🏁 WORKFLOW: Response generation completed")
            ...
            await self.send_workflow_message("sources", f"📖 Sources used: {len(sources)}")
            
            processing_time = time.time() - start_time
            logger.info(f"DEBUG: About to return SimpleWorkflowResult, processing_time: {processing_time:.2f}s")
            ...
            try:
                if isinstance(response, dict):
                    ...
                    if 'message' in response and isinstance(response['message'], dict) and 'content' in response['message']:
                        ...
                        logger.info(f"DEBUG: Extracted content from message.content: {len(content)} chars")
                        return content
                    # Check for direct content key (fallback format)
                    elif 'content' in response:
                        content = response['content']
                        logger.info(f"DEBUG: Extracted content from direct content: {len(content)} chars")
                        return content
                    # Check for error format
                        ...
                        logger.warning(f"DEBUG: Unexpected dict format, converting to string")
                        return str(response)
                elif isinstance(response, str):
                    logger.info(f"DEBUG: Response is already string: {len(response)} chars")
                    return response
                else:
                    logger.warning(f"DEBUG: Unknown response type {type(response)}, converting to string")
                    return str(response)
...
async def process_chat_message_simple(user_message: str, 
                                    chat_id: Optional[str] = None,
                                    conversation = None) -> SimpleWorkflowResult:
    """
