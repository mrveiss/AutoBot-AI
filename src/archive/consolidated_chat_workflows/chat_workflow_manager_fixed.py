...
class ChatWorkflowManager:
    ...
    def __init__(self):
        ...
        self.web_research_enabled = self.settings.get('agents', {}).get('research', {}).get('enabled', True)
        
        # Also check legacy web_research setting for backward compatibility
        if not self.web_research_enabled:
            self.web_research_enabled = self.settings.get('web_research', {}).get('enabled', True)
        
        # User preference settings
        self.require_user_confirmation = self.settings.get('web_research', {}).get('require_user_confirmation', True)
        self.auto_research_threshold = self.settings.get('web_research', {}).get('auto_research_threshold', 0.3)
        ...
        self.min_confidence_threshold = self.settings.get('agents', {}).get('kb_librarian', {}).get('similarity_threshold', 0.3)
        
        # Track research sessions for user confirmation
        self.pending_research_queries = {}
        
        logger.info(f"ChatWorkflowManager initialized - KB/Librarian enabled: {self.kb_librarian_enabled}, "
    ...
    def _is_research_response(self, user_message: str) -> Optional[bool]:
        ...
        if message_lower in ['yes', 'y', 'yeah', 'yep', 'sure', 'ok', 'okay', 'please', 'do it']:
            return True
        elif message_lower in ['no', 'n', 'nope', 'nah', 'never mind', 'nevermind', 'skip', 'skip it']:
            return False
            
        # Phrase-based responses
        yes_phrases = ['yes please', 'yes, please', 'go ahead', 'do it', 'research it', 'look it up', 'search for it']
        no_phrases = ['no thanks', 'no, thanks', 'not needed', 'skip it', 'end workflow', 'stop here', 'dont research']
    ...
    async def _initialize_web_research(self):
        ...
        if self.web_research_integration is None and self.web_research_enabled:
            try:
                ...
                self.web_research_integration = get_web_research_integration(merged_config)
    ...
    async def process_message(self, 
        ...
        try:
            ...
            if research_decision is not None:
                return await self._handle_research_decision(user_message, research_decision, chat_id, start_time)
            ...
            try:
                ...
                logger.info(f"WORKFLOW: Classification completed! Message type: {message_type.value}")
            ...
            if needs_research:
                if should_ask_user and self.require_user_confirmation:
                    ...
                    return await self._ask_for_research_permission(
                        user_message, message_type, knowledge_status, kb_results, 
                        classification, start_time, chat_id
                    )
                else:
                    ...
                    research_results, research_conducted, librarian_engaged, mcp_used = await self._conduct_research(
        ...
        if research_decision:
            ...
            if self.web_research_integration:
                try:
                    ...
                    research_result = await asyncio.wait_for(research_task, timeout=self.research_timeout)
                    ...
                    if research_result.get("status") == "success":
                        response = await self._generate_research_response(research_result, original_query)
                        return ChatWorkflowResult(
                            response=response,
            ...
            if self.kb_librarian is None:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info("WORKFLOW: KB Librarian initialization task cancelled successfully")
                    return KnowledgeStatus.MISSING, []
                except Exception as e:
    ...
    def _should_conduct_research(self, 
                               ...
                               user_message: str) -> Tuple[bool, bool]:
        """
        ...
        auto_research_requested = any(trigger in user_message_lower for trigger in auto_research_triggers)
        ...
        if knowledge_status == KnowledgeStatus.PARTIAL:
            if kb_results:
                avg_confidence = sum(r.get('score', 0) for r in kb_results) / len(kb_results)
    ...
    async def _conduct_research(self, 
                              user_message: str, 
                              message_type: MessageType,
                              existing_kb_results: List[Dict[str, Any]]) -> Tuple[Optional[Dict[str, Any]], bool, bool, bool]:
        """
        ...
        try:
            ...
            if self.web_research_integration and message_type in [MessageType.RESEARCH_NEEDED, MessageType.GENERAL_QUERY]:
                try:
                    ...
                    web_research_result = await asyncio.wait_for(research_task, timeout=self.research_timeout)
                    ...
                    if web_research_result.get("status") == "success":
                        ...
                        logger.warning(f"Web research failed: {web_research_result.get('message', 'Unknown error')}")
                        
                except asyncio.TimeoutError:
                    logger.warning(f"Web research timed out after {self.research_timeout}s")
                except Exception as e:
            ...
            return research_results if research_results else None, research_conducted, librarian_engaged, mcp_used
    ...
    async def _generate_response(self, 
        ...
        try:
            ...
            if research_results:
                prompt = self._create_research_enhanced_prompt(user_message, context, research_results)
            ...
            try:
                ...
                return self._generate_fallback_response(knowledge_status, research_results)
            except Exception as e:
            ...
            if llm_response and hasattr(llm_response, 'content'):
                ...
                return str(llm_response) if llm_response else self._generate_fallback_response(knowledge_status, research_results)
                ...
                if web_res.get('status') == 'success':
                    ...
                    if results:
                        ...
                        for i, result in enumerate(results[:3], 1):
                            ...
                            content = result.get('snippet', result.get('content', ''))[:200]
...
Is there a more specific aspect of this topic I can help you with based on my available knowledge?"""
    ...
    async def _generate_research_response(self, research_result: Dict[str, Any], original_query: str) -> str:
        """Generate response based on research results"""
        ...
        response_parts = [f"Based on my web research for '{original_query}', here's what I found:"]
    ...
    def _generate_fallback_response(self, knowledge_status: KnowledgeStatus, research_results: Optional[Dict[str, Any]] = None) -> str:
                ...
                if web_res.get('status') == 'success':
                    for result in web_res.get('results', []):
                        sources.append({
                            ...
                            'content_preview': result.get('snippet', '')[:100] + '...' if result.get('snippet') else ''
                        })
            if 'mcp_manuals' in research_results:
                sources.append({
                    'type': 'manual_pages',
