...
class ChatWorkflowManager:
    ...
    def __init__(self):
        ...
        self.kb_librarian_enabled = self.settings.get('agents', {}).get('kb_librarian', {}).get('enabled', True)
        # Research agent is subordinate to librarian - called by librarian when needed
        self.web_research_enabled = self.settings.get('agents', {}).get('research', {}).get('enabled', False)
        
        # Also check legacy web_research setting for backward compatibility
        if not self.web_research_enabled:
            self.web_research_enabled = self.settings.get('web_research', {}).get('enabled', False)
        
        # Workflow configuration - use agent-specific timeouts from settings
        self.kb_search_timeout = self.settings.get('agents', {}).get('kb_librarian', {}).get('timeout_seconds', 15.0)
        self.research_timeout = self.settings.get('agents', {}).get('research', {}).get('timeout_seconds', 30.0)
        self.classification_timeout = self.settings.get('agents', {}).get('classification', {}).get('timeout_seconds', 10.0)
        self.max_kb_results = self.settings.get('agents', {}).get('kb_librarian', {}).get('max_results', 5)
        self.min_confidence_threshold = 0.3
        
        logger.info(f"ChatWorkflowManager initialized - KB/Librarian enabled: {self.kb_librarian_enabled}, Research agent available: {self.web_research_enabled}")
    ...
    def _is_research_response(self, user_message: str) -> Optional[bool]:
        ...
        yes_phrases = ['yes please', 'yes, please', 'go ahead', 'do it', 'research it', 'look it up']
        no_phrases = ['no thanks', 'no, thanks', 'not needed', 'skip it', 'end workflow', 'stop here']
    ...
    async def process_message(self, 
                            ...
                            conversation: Optional[Conversation] = None) -> ChatWorkflowResult:
        """
        ...
        try:
            ...
            if research_decision is not None:
                if research_decision:
                    logger.info("WORKFLOW: User agreed to research - proceeding with research workflow")
            ...
            try:
                ...
                classify_task = asyncio.create_task(self._classify_message(user_message))
                logger.info("WORKFLOW: Waiting for classification with timeout...")
                message_type, classification = await asyncio.wait_for(classify_task, timeout=self.classification_timeout)
                logger.info(f"WORKFLOW: Classification completed successfully! Message type: {message_type.value}")
                if classification:
                    logger.info(f"WORKFLOW: Classification details - complexity: {classification.complexity}, confidence: {classification.confidence}")
                else:
                    logger.info("WORKFLOW: No classification details returned")
            except asyncio.TimeoutError:
                logger.error(f"WORKFLOW: Classification timed out after {self.classification_timeout} seconds")
                message_type = MessageType.GENERAL_QUERY
                classification = None
            except Exception as e:
                logger.error(f"WORKFLOW: Classification failed: {e}")
                import traceback
                logger.error(f"WORKFLOW: Classification traceback: {traceback.format_exc()}")
                message_type = MessageType.GENERAL_QUERY
                classification = None
            ...
            logger.info(f"WORKFLOW: Knowledge status: {knowledge_status.value}, found {len(kb_results)} results")
            
            # Step 3: Determine if research is needed
            logger.info("WORKFLOW: Checking if research needed...")
            needs_research = self._needs_research(knowledge_status, kb_results, classification)
            ...
            if needs_research:
                ...
                try:
                    ...
                    research_results, librarian_engaged, mcp_used = await asyncio.wait_for(
                        research_task, timeout=10.0
                    )
                    logger.info(f"WORKFLOW: Research completed: librarian={librarian_engaged}, mcp={mcp_used}")
                except asyncio.TimeoutError:
                    logger.warning("WORKFLOW: Research timed out after 10 seconds, proceeding without research")
                    research_results = None
                    librarian_engaged = False
                    mcp_used = False
                except Exception as e:
                    logger.error(f"WORKFLOW: Research failed: {e}, proceeding without research")
                    research_results = None
                    librarian_engaged = False
                    mcp_used = False
            ...
            try:
                ...
                logger.info(f"WORKFLOW: Response generated successfully, length: {len(response)}")
            except asyncio.TimeoutError:
            ...
            logger.info(f"WORKFLOW: Returning result with response length: {len(result.response)}")
            return result
    ...
    async def _classify_message(self, user_message: str) -> Tuple[MessageType, Optional[ClassificationResult]]:
        """
        ...
        try:
            ...
            classification = await self.classification_agent.classify_request(user_message)
            logger.info(f"WORKFLOW: Classification agent returned: {type(classification)}")
            
            # Map classification to our workflow message types
            if classification:
                logger.info("WORKFLOW: Processing classification result...")
                try:
                    reasoning = classification.reasoning.lower() if hasattr(classification, 'reasoning') else ""
                    logger.info(f"WORKFLOW: Reasoning: {reasoning[:100]}...")
                    
                    # Check for terminal/system tasks based on reasoning and suggested agents
                    if any(keyword in reasoning for keyword in ['terminal', 'command', 'shell', 'bash', 'cli']):
                        logger.info("WORKFLOW: Classified as TERMINAL_TASK")
                        return MessageType.TERMINAL_TASK, classification
                    
                    if any(keyword in reasoning for keyword in ['desktop', 'gui', 'window', 'application', 'app']):
                        logger.info("WORKFLOW: Classified as DESKTOP_TASK")
                        return MessageType.DESKTOP_TASK, classification
                        
                    if any(keyword in reasoning for keyword in ['system', 'config', 'install', 'setup', 'administration']):
                    ...
                    logger.error(f"WORKFLOW: Error processing classification attributes: {attr_error}")
                ...
                try:
                    ...
                    logger.error(f"WORKFLOW: Error checking complexity: {complexity_error}")
    ...
    async def _search_knowledge(self, 
                               user_message: str, 
                               message_type: MessageType,
                               classification: Optional[ClassificationResult]) -> Tuple[KnowledgeStatus, List[Dict[str, Any]]]:
        """
        ...
        if not self.kb_librarian_enabled:
            logger.info("Knowledge Base/Librarian is disabled in settings - falling back to file search")
        ...
        try:
            ...
            if self.kb_librarian is None:
                ...
                init_task = asyncio.create_task(
                    asyncio.to_thread(get_kb_librarian)
                )
                try:
                    ...
                    logger.warning("WORKFLOW: KB Librarian initialization timed out after 2s, skipping KB search")
                    return KnowledgeStatus.MISSING, []
                except Exception as e:
                    logger.warning(f"WORKFLOW: KB Librarian initialization failed: {e}, skipping KB search")
                    return KnowledgeStatus.MISSING, []
            
            logger.info("WORKFLOW: KB Librarian ready, checking initialization status...")
            ...
            if hasattr(self.kb_librarian, 'knowledge_base') and self.kb_librarian.knowledge_base:
                kb = self.kb_librarian.knowledge_base
                if hasattr(kb, 'index') and kb.index is None:
                    logger.warning("WORKFLOW: KnowledgeBase not initialized yet, skipping search to prevent blocking")
                    return KnowledgeStatus.MISSING, []
            ...
            logger.info(f"WORKFLOW: Waiting for search results with {self.kb_search_timeout}s timeout...")
            # Use configured timeout to prevent hanging
            kb_results = await asyncio.wait_for(search_task, timeout=self.kb_search_timeout)
            logger.info(f"WORKFLOW: Search completed, got {len(kb_results) if kb_results else 0} results")
    ...
    def _needs_research(self, 
                       knowledge_status: KnowledgeStatus, 
                       kb_results: List[Dict[str, Any]],
                       classification: Optional[ClassificationResult]) -> bool:
        """
        ...
        if not self.web_research_enabled:
            logger.info("Research agent is disabled in settings - librarian will use KB/local sources only")
            return False
        ...
        if knowledge_status == KnowledgeStatus.MISSING:
            ...
            if classification and classification.complexity in [TaskComplexity.COMPLEX, TaskComplexity.VERY_COMPLEX]:
                # Check if it's a research-heavy request that needs web data
                if any(keyword in classification.reasoning.lower() for keyword in ['latest', 'current', 'news', 'search', 'browse', 'website', 'online']):
                    logger.info("Librarian determining research agent is needed for missing knowledge")
                    return True
    ...
    async def _conduct_research(self, 
                              user_message: str, 
                              message_type: MessageType,
                              existing_kb_results: List[Dict[str, Any]]) -> Tuple[Optional[Dict[str, Any]], bool, bool]:
        """
        ...
        try:
            ...
            if self.web_research_enabled and message_type in [MessageType.RESEARCH_NEEDED, MessageType.GENERAL_QUERY]:
                logger.info("Research capability available - will ask user before proceeding")
            ...
            return research_results if research_results else None, librarian_engaged, mcp_used
    ...
    async def _generate_response(self, 
        ...
        try:
            ...
            context = self._build_llm_context(
                user_message, message_type, knowledge_status, kb_results, research_results
            )
            ...
            elif llm_response and isinstance(llm_response, dict) and 'content' in llm_response:
                return llm_response['content']
            else:
                return str(llm_response) if llm_response else self._generate_fallback_response(knowledge_status)
    ...
    def _build_llm_context(self, 
                          ...
                          knowledge_status: KnowledgeStatus,
                          kb_results: List[Dict[str, Any]],
                          research_results: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if kb_results:
            ...
            for i, result in enumerate(kb_results[:3], 1):  # Top 3 results
                ...
                kb_context_parts.append(f"{i}. {title} (confidence: {confidence:.2f}):\n{content}...")
...
Currently, the research agent is not available. I can only provide information from my local knowledge base and documentation files."""
    ...
    async def _fallback_file_search(self, user_message: str, message_type: MessageType) -> Tuple[KnowledgeStatus, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            if message_type == MessageType.TERMINAL_TASK:
                search_paths.extend([
                    "docs/*terminal*.md", "docs/*command*.md", "docs/*bash*.md",
                    "data/system_knowledge/**/*.md", "data/system_knowledge/**/*.txt"
                ])
            elif message_type == MessageType.SYSTEM_TASK:
                search_paths.extend([
                    "docs/*system*.md", "docs/*admin*.md", "docs/*config*.md",
            ...
            results = []
            search_terms = user_message.lower().split()[:3]  # Use first 3 words as search terms
            
            for pattern in search_paths[:5]:  # Limit to 5 patterns to prevent slowness
                try:
                    files = glob.glob(pattern, recursive=True)[:10]  # Max 10 files per pattern
            ...
            results = sorted(results, key=lambda x: x['score'], reverse=True)[:self.max_kb_results]
...
async def process_chat_message(user_message: str, 
    ...
    Returns:
        ChatWorkflowResult with response and metadata
    """
    return await chat_workflow_manager.process_message(user_message, chat_id, conversation)