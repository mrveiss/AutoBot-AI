...
class LLMSettings(BaseSettings):
    ...
    connection_pool_size: int = Field(
        default=20, env="LLM_POOL_SIZE"
    )  # Increased for 22-core CPU + RTX 4070
    max_concurrent_requests: int = Field(default=8, env="LLM_MAX_CONCURRENT")
    connection_timeout: float = Field(
        default=15.0, env="LLM_CONNECTION_TIMEOUT"
    )  # Reduced from 30s
...
class AsyncLLMInterface:
    ...
    def __init__(self, settings: Optional[LLMSettings] = None):
        ...
        self._connector = aiohttp.TCPConnector(
            ...
            limit_per_host=min(
                8, self.settings.connection_pool_size
            ),  # Increased from 5
    ...
    async def _cache_response(self, cache_key: str, response: LLMResponse) -> None:
        ...
        try:
            ...
            await redis_set(cache_key, json.dumps(data), ex=self.settings.cache_ttl)
        except Exception as e:
    ...
    async def chat_completion(
        ...
        **kwargs,
        ...
        cache_key = await self._generate_cache_key(
            normalized_messages, model=model, temperature=temperature
        )
    ...
    async def _handle_json_response(
        ...
        async with session.post(
            ...
            logger.info(
                f"LLM response from {model}: {len(content)} chars in {processing_time:.2f}s"
            )
            return llm_response
    ...
    async def _handle_streaming_response(
        ...
        async with session.post(
            ...
            async for chunk_data in response.content.iter_chunked(
                8192
                ...
                if current_buffer_size > max_buffer_size:
                    logger.warning(
                        f"Response exceeds buffer limit ({max_buffer_size} bytes), truncating"
                    )
                    break
        ...
        logger.info(
            f"Streaming LLM response from {model}: {len(full_content)} chars, {chunk_count} chunks, {current_buffer_size/1024/1024:.1f}MB in {processing_time:.2f}s"
        )
        return llm_response
    ...
    async def get_available_models(self, force_refresh: bool = False) -> List[str]:
        ...
        if (
            ...
            and time.time() - self._models_cache_time < 300
        ):  # 5 minute cache
...
async def chat_completion(
    messages: List[Union[Dict[str, str], ChatMessage]], **kwargs
) -> LLMResponse:
