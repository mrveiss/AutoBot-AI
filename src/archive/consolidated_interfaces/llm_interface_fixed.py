...
class LLMInterfaceFixed:
    ...
    async def _stream_ollama_response(
        self, response: aiohttp.ClientResponse, request_id: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            async for chunk_bytes in response.content.iter_chunked(1024):
                ...
                try:
                    ...
                    logger.warning(
                        f"[{request_id}] Failed to decode chunk, skipping..."
                    )
                    continue
                ...
                for line in chunk_text.split("\n"):
                    ...
                    try:
                        ...
                        if (
                            ...
                            logger.debug(
                                f"[{request_id}] Chunk {chunk_count}: '{content[:50]}...'"
                            )
                        ...
                        if chunk_data.get("done", False):
                            ...
                            logger.info(
                                f"[{request_id}] Stream completed naturally: {chunk_count} chunks in {duration:.2f}s"
                            )
                        ...
                        logger.warning(
                            f"[{request_id}] JSON decode error: {e} - Line: '{line[:100]}'"
                        )
                        continue
                ...
                if chunk_count > 0 and chunk_count % 50 == 0:
                    duration = current_time - start_time
                    logger.info(
                        f"[{request_id}] Progress: {chunk_count} chunks, {len(full_content)} chars in {duration:.1f}s"
                    )

            # Stream ended without explicit done flag
            duration = time.time() - start_time
            logger.info(
                f"[{request_id}] Stream ended without done flag: {chunk_count} chunks in {duration:.2f}s"
            )
            ...
            duration = time.time() - start_time
            logger.info(
                f"[{request_id}] Stream cancelled by user after {duration:.2f}s"
            )
    ...
    async def _make_ollama_request(
        self, url: str, data: Dict[str, Any], request_id: str = None
        ...
        try:
            ...
            async with await self._create_session() as session:
                ...
                async with session.post(url, json=data) as response:
                    ...
                    logger.info(
                        f"[{request_id}] Request successful, status={response.status}"
                    )
...
async def test_fixed_llm():
    ...
    messages = [{"role": "user", "content": "Say hello and tell me what you are"}]
    ...
    try:
        ...
        if result:
            ...
            if "stats" in result:
                stats = result["stats"]
                print(
                    f"Stats: {stats['chunks']} chunks, {stats['duration']:.2f}s, {stats['content_length']} chars"
                )
        else:
