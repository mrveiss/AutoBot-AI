    ...
    async def chat_completion(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            if "prompt_eval_count" in result:
                ...
                usage["total_tokens"] = completion_tokens + result["prompt_eval_count"]
    ...
    async def chat_completion(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            if request.llm_type == LLMType.EXTRACTION:
                content = (
                    '{"facts": [{"subject": "Test", "predicate": "is", '
                    '"object": "example"}]}'
                )
...
class UnifiedLLMInterface:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Unified LLM Interface initialized with {len(self.providers)} "
            f"providers"
        )
        ...
        for provider_type in provider_order:
            ...
            try:
                ...
                if response.error:
                    ...
                    logger.warning(
                        f"Provider {provider_type.value} returned error: "
                        f"{response.error}"
                    )
                    continue
