...
try:
    from src.dependency_container import get_config, get_llm
    from src.llm_interface import ChatMessage
...
class ConsolidatedChatWorkflow:
    ...
    def __init__(self):
        ...
        if MODERN_ARCHITECTURE_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"Modern architecture not available, using fallback: {e}"
                )
        ...
        logger.info(
            "Consolidated chat workflow initialized with all available features"
        )
        ...
        **kwargs,
        ...
        try:
            await self.send_workflow_message(
                "status", "Starting message processing...", {"step": "initialization"}
            )
            ...
            await self.send_workflow_message(
                ...
                {"message_type": message_type.value, "classification": classification},
            )
            ...
            if enable_research and self._should_conduct_research(
                knowledge_status, message_type, classification
            ):
                await self.send_workflow_message(
    ...
    async def _classify_message(
    ...
    ) -> Tuple[MessageType, Optional[Any]]:
        ...
        if not self.classification_agent:
            ...
            if any(
                term in user_lower
                for term in ["terminal", "command", "bash", "shell", "cli"]
            ):
                return MessageType.TERMINAL_TASK, None
            elif any(
                term in user_lower for term in ["desktop", "gui", "window", "interface"]
            ):
                return MessageType.DESKTOP_TASK, None
            elif any(
                term in user_lower for term in ["system", "config", "settings", "admin"]
            ):
        ...
        try:
            ...
            if classification:
                ...
                if any(
                    term in reasoning
                    for term in ["terminal", "command", "bash", "shell"]
                ):
                    return MessageType.TERMINAL_TASK, classification
                elif any(
                    term in reasoning for term in ["desktop", "gui", "application"]
                ):
                    return MessageType.DESKTOP_TASK, classification
                elif any(
                    term in reasoning
                    for term in ["system", "configuration", "administration"]
                ):
                ...
                if (
                    complexity in [TaskComplexity.COMPLEX, TaskComplexity.VERY_COMPLEX]
                    if hasattr(TaskComplexity, "COMPLEX")
                    else False
                ):
            ...
            logger.warning(
                f"Classification timed out after {self.classification_timeout} seconds"
            )
            return MessageType.GENERAL_QUERY, None
        except Exception as e:
    ...
    async def _search_knowledge(
        ...
        try:
            ...
            logger.warning(
                f"Knowledge base search timed out after {self.kb_search_timeout} seconds"
            )
            return KnowledgeStatus.MISSING, []
        except Exception as e:
    ...
    def _should_conduct_research(
        ...
        if classification and hasattr(classification, "complexity"):
            complexity = getattr(classification, "complexity", None)
            if hasattr(TaskComplexity, "COMPLEX") and complexity in [
    ...
    async def _conduct_research(
        ...
        try:
            ...
            if self.mcp_integration and message_type in [
                MessageType.TERMINAL_TASK,
                MessageType.SYSTEM_TASK,
            ]:
    ...
    async def _query_librarian(self, user_message: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            research_task = asyncio.create_task(librarian.research_topic(user_message))
    ...
    async def _generate_response(
        ...
        if self.modern_architecture and self.llm:
            ...
            return await self._generate_basic_response(
                user_message, knowledge_status, research_results
            )
...
You provide accurate, helpful responses based on available knowledge and research."""
        ...
        if research_results and research_results.get("results"):
            prompt = self._create_research_enhanced_prompt(
                user_message, {}, research_results
            )
        elif knowledge_status == KnowledgeStatus.FOUND:
            prompt = self._create_knowledge_based_prompt(
                user_message, {"kb_results": kb_results}
            )
        elif knowledge_status == KnowledgeStatus.PARTIAL:
            prompt = self._create_partial_knowledge_prompt(
                user_message, {"kb_results": kb_results}
            )
        else:
...
async def process_chat_message_unified(
    user_message: str, chat_id: Optional[str] = None, **kwargs
) -> ConsolidatedWorkflowResult:
    """
...
logger.info(
    "Consolidated chat workflow system initialized with all features from all previous versions"
)
