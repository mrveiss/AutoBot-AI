 ...
 self.chromadb_path = self._resolve_path(kb_config['chromadb_path'])
    ...
    def add_file(self, file_path: str, file_type: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Store document metadata in SQLite
            is_markdown = file_type.lower() in ['md', 'markdown']
            
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()

            # Prepare metadata JSON string for storage, including markdown flag
            metadata_for_db = metadata if metadata is not None else {}
            metadata_for_db['is_markdown'] = is_markdown
            metadata_json_string = json.dumps(metadata_for_db)
            
            cursor.execute(
                "INSERT INTO documents (filename, file_type, path, num_chunks, metadata) VALUES (?, ?, ?, ?, ?)",
                (
                    os.path.basename(file_path),
                    file_type,
                    file_path,
                    len(chunks),
 metadata_json_string # Store metadata as JSON string
                )
            )
            doc_id = cursor.lastrowid
            conn.commit()
            ...
            for i, chunk in enumerate(chunks):
                ...
                chunk_metadata = {
                    ...
                    "file_type": file_type,
                    "chunk_index": i,
                    **(metadata_for_db if metadata_for_db else {}) # Add custom metadata including is_markdown
                }
                metadatas.append(chunk_metadata)
                ids.append(f"doc_{doc_id}_chunk_{i}")
