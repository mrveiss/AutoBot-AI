...
class CircuitBreaker:
    ...
    def _record_failure(self, duration: float, exception: Exception):
        ...
        with self._lock:
            ...
            logger.debug(f"Circuit breaker {self.name}: Failure recorded ({self.failure_count}/{self.config.failure_threshold})")
...
if __name__ == "__main__":
    ...
    async def example_usage():
        ...
        print(f"Success rate: {cb_state['recent_performance'].get('success_rate', 0):.1%}")
