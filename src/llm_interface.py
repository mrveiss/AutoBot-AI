...
class LLMInterface:
    ...
    def _load_composite_prompt(self, base_file_path: str) -> str:
        ...
        resolved_content = self._resolve_includes(initial_content, os.path.dirname(base_file_path))
    ...
    async def check_ollama_connection(self) -> bool:
        ...
        try:
            ...
            response = await asyncio.to_thread(requests.get, health_check_url, timeout=5)
            ...
            missing_models = [model for model in all_configured_ollama_models if model not in available_ollama_models]

            if not missing_models:
                logger.info(f"✅ Ollama server is reachable and all configured models ({', '.join(all_configured_ollama_models)}) are available.")
                return True
            else:
                logger.warning(f"⚠️ Ollama server is reachable, but the following configured models are not found: {', '.join(missing_models)}. Available models: {', '.join(available_ollama_models)}")
                return False
        except requests.exceptions.ConnectionError:
            logger.error(f"❌ Failed to connect to Ollama server at {self.ollama_host}. Is it running?")
            return False
        except requests.exceptions.Timeout:
            logger.error(f"❌ Ollama server at {self.ollama_host} timed out. It might be too busy or slow.")
            return False
    ...
    async def _ollama_chat_completion(self, model: str, messages: list, temperature: float = 0.7, structured_output: bool = False, **kwargs):
        ...
        try:
            response = await asyncio.to_thread(requests.post, url, headers=headers, json=data, timeout=600)
            ...
            print(f"HTTP Error communicating with Ollama: {e.response.status_code} - {e.response.text}")
            ...
            logger.error(f"An unexpected error occurred during Ollama chat completion: {e}")
            return None

    async def _openai_chat_completion(self, model: str, messages: list, temperature: float = 0.7, structured_output: bool = False, **kwargs):
        if not self.openai_api_key:
            print("OpenAI API key not found. Please set OPENAI_API_KEY in .env or config.yaml.")
            return None
...
class LLMInterface:
    ...
    async def chat_completion(self, messages: list, llm_type: str = "orchestrator", **kwargs):
        ...
        llm_type: "orchestrator" or "task"
        """
        fallback_strategy = self.config['llm_config'].get('fallback_strategy', ['orchestrator', 'task'])
        ...
        if llm_type == "orchestrator":
            ...
            system_prompt_content = self.task_system_prompt
        # Note: System prompt is applied only once before attempting the first LLM in the strategy
        # if it's not already present in the messages.

        # Apply system prompt if not already present and content is available
        if system_prompt_content and not any(m.get('role') == 'system' for m in messages):
            messages.insert(0, {"role": "system", "content": system_prompt_content})

        for llm_alias_attempt in fallback_strategy:
            model_alias = ""
            settings = {}
            
            if llm_alias_attempt == "orchestrator":
                model_alias = self.orchestrator_llm_alias
                settings = self.orchestrator_llm_settings
            elif llm_alias_attempt == "task":
                model_alias = self.task_llm_alias
                settings = self.task_llm_settings
            # Add more aliases here if needed (e.g., "openai" if configured directly)
            # elif llm_alias_attempt == "openai":
            #     model_alias = self.config['llm_config']['openai'].get('model', 'gpt-3.5-turbo') # Assuming openai model name is the alias
            #     settings = {} # Load specific OpenAI settings if needed

            if not model_alias:
                logger.warning(f"LLM alias '{llm_alias_attempt}' specified in fallback strategy but not configured or recognized. Skipping.")
                continue # Skip this alias if it's not recognized
        ...
        if model_alias.startswith("ollama_"):
            ...
            # Prepare common LLM parameters for this specific model attempt
            llm_params = {
                "temperature": settings.get('temperature', 0.7),
                # Add other common parameters from settings or kwargs
                **kwargs # Allow kwargs to override config settings
            }
            ...
            try:
                if model_alias.startswith("ollama_"):
                    response = await self._ollama_chat_completion(model_name, messages, **llm_params)
                elif model_alias.startswith("openai_"):
                    response = await self._openai_chat_completion(model_name, messages, **llm_params)
                elif model_alias.startswith("transformers_"):
                    response = await self._transformers_chat_completion(model_name, messages, **llm_params)
                # Add more backend calls here for other model types
                else:
                     logger.warning(f"Unsupported LLM model alias prefix: {model_alias}. Skipping.")
                     continue # Skip if the alias prefix is not recognized

                if response is not None and response.get('choices'):
                    logger.info(f"Successfully got response from {llm_alias_attempt} ({model_name}).")
                    return response # Return the first successful response

            except Exception as e:
                logger.warning(f"Call to LLM '{llm_alias_attempt}' ({model_name}) failed: {e}. Trying next in fallback strategy.")

        logger.error(f"All LLMs in the fallback strategy failed for type '{llm_type}'.")
        return None # Return None if all fallbacks fail
