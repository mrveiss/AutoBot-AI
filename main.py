...
def _validate_api_keys():
    """Checks for required API keys (e.g., OpenAI) in config or env vars."""
    openai_api_key = os.getenv("OPENAI_API_KEY") or config.get("openai", {}).get("api_key")
    if not openai_api_key:
        logging.warning("OPENAI_API_KEY not found in environment variables or config/config.yaml. OpenAI backend will be unavailable.")

_validate_api_keys() # Validate API keys after loading config

# Configure logging based on loaded config (if logging section exists)
# This assumes a basic logging configuration in the YAML file
...
system_integration = SystemIntegration() # Initialize SystemIntegration
...
def _scan_plugins(plugins_dir: str) -> List[str]:
    """Scans the specified directory for Python files (potential plugins)."""
    plugin_files = []
...
scanned_plugins = _scan_plugins(plugins_directory)
app.mount("/api", api_router)

def _load_plugins(plugin_files: List[str]) -> List[Any]:
    """Loads plugins from the specified Python files."""
    loaded_plugins = []
    for plugin_file in plugin_files:
        module_name = os.path.basename(plugin_file).replace(".py", "")
        spec = importlib.util.spec_from_file_location(module_name, plugin_file)
        if spec is None or spec.loader is None:
            logging.warning(f"Could not load module spec for plugin file: {plugin_file}")
            continue
        ...
        try:
            ...
            logging.error(f"Error loading plugin {module_name} from {plugin_file}: {e}")
...
async def startup_event_orchestrator():
    ...
    plugins_directory = os.path.join(os.path.dirname(__file__), "plugins")
    scanned_plugins = _scan_plugins(plugins_directory)
    logging.info(f"Found {len(scanned_plugins)} potential plugins.")
    loaded_plugins = _load_plugins(scanned_plugins)
