...
def _get_endpoint_list(base_url: str) -> list:
    ...
    return [
        ...
        (f"{base_url}/api/system/health", "Health Check (Detailed)",
         {"detailed": "true"}),
        (f"{base_url}/api/project/status", "Project Status (Fast)", None),
        (f"{base_url}/api/project/status", "Project Status (Detailed)",
         {"detailed": "true"}),
...
def _log_performance_summary(results: list, total_time: float,
                             successful_tests: int, total_endpoints: int):
    ...
    for desc, duration, success in results:
        ...
        logger.info(
            "[%s] %-25s %8.0fms %s", status, desc, duration, speed_rating
        )

    logger.info("Total test time: %.0fms", total_time)
    logger.info(
        "Successful tests: %s/%s", successful_tests, total_endpoints
    )
...
async def test_api_endpoints():
    ...
    for url, description, params in endpoints:
        duration, success = test_endpoint_performance(
    ...
    _log_performance_summary(
        results, total_time, successful_tests, len(endpoints)
    )
