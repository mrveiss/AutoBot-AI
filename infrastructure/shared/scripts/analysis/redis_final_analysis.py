...
class AutoBotVectorStoreAnalysis:
    ...
    def _identify_compatibility_issues(self, field_analysis: Dict) -> List[str]:
        ...
        if "_node_content" in field_analysis and "text" in field_analysis:
            issues.append(
                "LLAMAINDEX_FIELD_MISMATCH: Both '_node_content' and 'text' fields exist - may cause confusion"
            )

        if "text" not in field_analysis:
            issues.append(
                "LLAMAINDEX_MISSING_TEXT: No 'text' field found - required by current LlamaIndex"
            )

        if "_node_content" not in field_analysis:
            issues.append(
                "LLAMAINDEX_MISSING_NODE_CONTENT: No '_node_content' field - may be required"
            )

        # Check for LangChain compatibility
        if "content" not in field_analysis and "text" in field_analysis:
            issues.append(
                "LANGCHAIN_FIELD_MAPPING: Uses 'text' field instead of expected 'content'"
            )

        # Check metadata structure
        if "source_metadata" in field_analysis:
            issues.append(
                "METADATA_STRUCTURE: Complex nested metadata may need flattening for LangChain"
            )
    ...
    async def test_llamaindex_field_fix_basic(self) -> Tuple[bool, int, List[str]]:
        ...
        try:
            ...
            vector_store = RedisVectorStore(
                schema=schema, redis_url=ServiceURLs.REDIS_LOCAL, redis_kwargs={"db": 2}
            )
            ...
            nodes = await asyncio.to_thread(
                retriever.retrieve, "deployment configuration"
            )
            ...
            if result_count > 0:
                for i, node in enumerate(nodes[:2]):
                    logger.info(
                        "  Result %s: %s...",
                        i + 1,
                        node.text[:100] if hasattr(node, "text") else "No text",
                    )

            fixes_attempted.append(
                f"Successfully retrieved {result_count} results using retriever"
            )
    ...
    async def test_langchain_db0_workaround_basic(self) -> Tuple[bool, int, List[str]]:
        ...
        try:
            ...
            results = await asyncio.to_thread(
                vector_store.similarity_search, "deployment", k=3
            )
            ...
            fixes_attempted.append(
                f"Successfully searched and found {result_count} results"
            )
    ...
    def generate_final_recommendation(
        ...
        data_analysis.get("compatibility_issues", [])
    ...
    async def run_final_analysis(self):
        ...
        self._print_final_report(
            data_analysis, llamaindex_result, langchain_result, recommendation
        )
    ...
    def _print_final_report(
        ...
        print(
            f"  • Documents: {data_analysis.get('total_documents', 0):,} (HIGH QUALITY)"
        )
        print(f"  • Index Size: {data_analysis.get('index_size_mb', 0):.1f} MB")
        print(
            f"  • Vector Dimensions: {data_analysis.get('vector_config', {}).get('dimensions', 'unknown')}"
        )
        print(
            f"  • Algorithm: {data_analysis.get('vector_config', {}).get('algorithm', 'unknown')}"
        )
        ...
        print(
            f"  • LlamaIndex: {'✅ WORKING' if llamaindex_result[0] else '❌ FAILED'} ({llamaindex_result[1]} results)"
        )
        if llamaindex_result[2]:
            print(f"    Fixes: {', '.join(llamaindex_result[2][-2:])}")

        print(
            f"  • LangChain: {'✅ WORKING' if langchain_result[0] else '❌ FAILED'} ({langchain_result[1]} results)"
        )
    ...
    async def test_llamaindex_field_fix(self) -> Tuple[bool, int, List[str]]:
        ...
        try:
            ...
            fixes_attempted.append(
                "Connected to existing vector store without overwriting"
            )
            ...
            try:
                nodes = await asyncio.to_thread(
                    retriever.retrieve, "kubernetes deployment"
                )
                result_count = len(nodes)
                ...
                query_embedding = await asyncio.to_thread(
                    embed_model.get_text_embedding, "kubernetes deployment"
                )
                ...
                result_count = (
                    len(direct_result.nodes) if hasattr(direct_result, "nodes") else 0
                )

                fixes_attempted.append(
                    f"Direct vector store query: {result_count} results"
                )
    ...
    async def test_langchain_db0_workaround(self) -> Tuple[bool, int, List[str]]:
        ...
        try:
            ...
            # Test search
            results = await asyncio.to_thread(
                vector_store.similarity_search, "deployment configuration", k=3
            )
