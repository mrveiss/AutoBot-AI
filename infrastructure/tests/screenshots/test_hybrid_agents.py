...
async def test_local_agents():
    ...
    try:
        ...
        request = create_agent_request(
            agent_type="chat",
            action="chat",
            payload={"message": "Hello, how are you?", "chat_history": []},
        )
    ...
    try:
        ...
        request = create_agent_request(
            ...
            payload={"message": "What network scanning tools do we have available?"},
        )
...
async def test_agent_client():
    ...
    try:
        # Create agent client
        config = AgentClientConfig({"default_mode": "local", "request_timeout": 30.0})
