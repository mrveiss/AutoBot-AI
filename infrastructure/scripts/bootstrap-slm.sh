...
\"" 2>/dev/null || warn "Could not create admin user (may need manual setup)"
    ...
    cat "${PROJECT_ROOT}/infrastructure/templates/scripts/backend-start.sh" | \
        remote_exec_sudo "cat > ${REMOTE_BACKEND}/start.sh"
    cat "${PROJECT_ROOT}/infrastructure/templates/scripts/backend-stop.sh" | \
        remote_exec_sudo "cat > ${REMOTE_BACKEND}/stop.sh"
    cat "${PROJECT_ROOT}/infrastructure/templates/scripts/backend-status.sh" | \
...
frontend_setup() {
    ...
    cat "${PROJECT_ROOT}/infrastructure/templates/nginx/autobot-slm.conf" | \
    ...
    # Install helper scripts
    info "Installing helper scripts..."
    cat "${PROJECT_ROOT}/infrastructure/templates/scripts/frontend-start.sh" | \
        remote_exec_sudo "cat > ${REMOTE_FRONTEND}/start.sh"
    cat "${PROJECT_ROOT}/infrastructure/templates/scripts/frontend-stop.sh" | \
        remote_exec_sudo "cat > ${REMOTE_FRONTEND}/stop.sh"
    cat "${PROJECT_ROOT}/infrastructure/templates/scripts/frontend-status.sh" | \
...
service_installation() {
    ...
    cat "${PROJECT_ROOT}/infrastructure/templates/systemd/autobot-slm-backend.service" | \
