                    ...
                    if response.status == 200:
                        ...
                        async with session.get(f"{self.base_url}/api/workflow/workflows") as wf_response:
                            if wf_response.status == 200:
                                ...
                                print(f"❌ Workflow API not accessible: {wf_response.status}")
                                print("   Please restart backend to load workflow endpoints")
                                return False
            ...
            try:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        print(f"   Step {current_step}/{total_steps} ({progress:.0f}%) - {workflow_status}")
