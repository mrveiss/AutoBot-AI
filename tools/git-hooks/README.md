...
1. **pre-commit framework (preferred)** — `pip install pre-commit && pre-commit
   install` writes `.git/hooks/pre-commit`, which runs the FULL quality suite
   (black, flake8, mypy, function-length, hardcoded-values, mass-deletions) and
   the `target-branch-guard`. Use this when you have the `pre-commit` binary.
2. **Standalone installer (fallback)** — `bash scripts/install-git-hooks.sh`
   installs self-contained copied hooks with NO external dependency. It is the
   fallback for machines/clones where the `pre-commit` binary isn't installed.

They are complementary, not competing: the standalone installer **detects a
framework-managed `pre-commit` hook (via its `generated by pre-commit` marker)
and leaves it untouched** — it only installs the standalone branch-guard when no
framework hook exists. So running the installer never silently drops the
framework's other quality hooks (#11598).
...
#4113) and points you at the correct `issue-*` → `main` PR workflow.
...
- Phase 6: simulated a push to `issue-5128` (closed), correctly warned about
  the closed issue and the existing `main` commit citing it
