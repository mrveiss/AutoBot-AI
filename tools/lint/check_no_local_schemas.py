...
Scan target:
  ...
  (*_test.py / test_*.py: a class a test defines inline is a fixture, not an
  endpoint schema, so this endpoint-schema-placement hook does not apply to it;
  matching the same exemption independently made on #16298; this hook side, #16261)
...
def _is_target_file(path: Path, repo_root: Path) -> bool:
    ...
    if name.startswith("schemas_"):
        return False
    # Test files: a BaseModel a test method defines is a fixture, not an
    # endpoint schema (#16261). Scanning them made api_endpoint_migrations_test.py
    # unfixable — its four pre-existing fixtures pre-date this hook.
