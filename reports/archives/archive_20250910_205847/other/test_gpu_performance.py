...
async def performance_test():
    ...
    print("=== AutoBot Phase 9 GPU Performance Analysis ===\n")
    ...
    if gpu_before:
        ...
        print(f"- Memory Used: {gpu_before['memory_used']}/{gpu_before['memory_total']} MB")
    ...
    if gpu_after:
        ...
        print(f"- Memory Used: {gpu_after['memory_used']}/{gpu_after['memory_total']} MB")
    ...
    if gpu_after and gpu_after['gpu_util'] < 50:
        ...
        print("   - Increase batch sizes for embedding computation")
        print("   - Enable mixed precision (FP16) for faster inference")
    ...
    if gpu_after:
        ...
        print(f"GPU Memory Utilization: {memory_util:.1f}%")
        if memory_util < 30:
...
async def npu_assessment():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.get('http://172.16.168.22:8081/health', timeout=5) as resp:
                    health_data = await resp.json()
                    print(f"NPU Worker Status: {health_data.get('status', 'unknown')}")
