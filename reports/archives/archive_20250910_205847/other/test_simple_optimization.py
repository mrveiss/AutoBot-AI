...
async def test_direct_optimization():
    ...
    try:
        ...
        # Test text
        test_text = """
        AutoBot Phase 9 represents a significant advancement in Linux system automation.
...
async def test_performance_stats():
    ...
    try:
        ...
        if hasattr(chunker, 'get_performance_stats'):
            ...
            print(f"    - Total sentences processed: {stats.get('total_sentences_processed', 0)}")
            print(f"    - Total processing time: {stats.get('total_processing_time', 0):.2f}s")
            print(f"    - Average performance: {stats.get('average_sentences_per_second', 0):.1f} sent/sec")
            print(f"    - GPU memory pool: {stats.get('gpu_memory_pool_enabled', False)}")
            print(f"    - Optimization level: {stats.get('optimization_level', 'unknown')}")
            return True
        else:
...
if __name__ == "__main__":
    async def main():
        print("🎯 AutoBot Phase 9 - Simple GPU Optimization Test")
        print("=" * 60)
