...
from src.services.temporal_invalidation_service import TemporalInvalidationService, InvalidationRule, InvalidationReason
...
from src.models.entity_mapping import EntityType
...
class MockFactExtractionService:
    ...
    def add_test_fact(self, subject: str, predicate: str, object: str, 
                     fact_type: FactType = FactType.FACT,
                     temporal_type: TemporalType = TemporalType.DYNAMIC,
                     confidence: float = 0.8,
                     age_days: int = 0,
                     active: bool = True) -> AtomicFact:
    ...
    async def get_facts_by_criteria(self, source=None, fact_type=None, temporal_type=None, 
                                  min_confidence=None, active_only=True, limit=1000):
...
class TestTemporalInvalidation:
    ...
    async def test_dry_run_invalidation_sweep(self):
        ...
        print(f"  Facts identified for invalidation: {result['facts_identified_for_invalidation']}")
        ...
        if result["facts_identified_for_invalidation"] > 0:
            ...
            for sample_fact in result.get("sample_facts_to_invalidate", [])[:3]:
                print(f"    - {sample_fact['statement']} (age: {sample_fact['age_days']} days)")
    ...
    async def test_contradiction_detection(self):
        ...
        print(f"    Fact 1: {fact1.subject} {fact1.predicate} {fact1.object} (confidence: {fact1.confidence})")
        print(f"    Fact 2: {fact2.subject} {fact2.predicate} {fact2.object} (confidence: {fact2.confidence})")
    ...
    async def test_temporal_type_behavior(self):
        ...
        assert temporal_test_results.get("STATIC", 0) <= temporal_test_results.get("DYNAMIC", 0), \
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            rule_management_results = await self.test_rule_management()
            ...
            print(f"  - Dry run identified: {dry_run_result['facts_identified_for_invalidation']} facts")
            print(f"  - Actual sweep processed: {actual_result['facts_processed']} facts")
            print(f"  - Contradictions detected: {contradiction_result['contradictions_found']}")
            print(f"  - Statistics collected: {len(stats)} metrics")
            print(f"  - Temporal behavior verified: {len(temporal_behavior)} types tested")
