...
def test_is_unreachable_null_scope_organization_resource():
    ...
    assert is_unreachable(r, has_grant=False, owner_is_live=True)


def test_is_unreachable_false_once_owned_or_granted():
    r = ResourceDescriptor(owner_id=None, company_id=None, scope=ScopeLevel.ORGANIZATION)
    assert not is_unreachable(r, has_grant=True, owner_is_live=True)
    owned = ResourceDescriptor(owner_id="someone", company_id=None, scope=ScopeLevel.ORGANIZATION)
    assert not is_unreachable(owned, has_grant=False, owner_is_live=True)


def test_is_unreachable_a_dangling_owner_is_no_owner():
    """#16927: an owner_id naming a deleted user is #15779's own orphan case, not a reachable resource."""
    dangling = ResourceDescriptor(owner_id="deleted-user", company_id=None, scope=ScopeLevel.ORGANIZATION)
    assert is_unreachable(dangling, has_grant=False, owner_is_live=False)
    assert not is_unreachable(dangling, has_grant=True, owner_is_live=False), "a grant still reaches it"


def test_is_unreachable_an_empty_owner_is_no_owner():
    """Truthiness, as is_visible() uses: an empty owner id matches no principal."""
    r = ResourceDescriptor(owner_id="", company_id=None, scope=ScopeLevel.PRIVATE)
    assert is_unreachable(r, has_grant=False, owner_is_live=True)
...
def test_is_unreachable_organization_with_company_id_is_reachable():
    ...
    assert not is_unreachable(r, has_grant=False, owner_is_live=True)
...
def test_is_unreachable_group_scope_empty_groups():
    ...
    assert not is_unreachable(reachable, has_grant=False, owner_is_live=True)
