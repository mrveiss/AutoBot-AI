...
class RetryMechanism:
    ...
    def calculate_delay(self, attempt: int) -> float:
        ...
        if self.config.strategy == RetryStrategy.EXPONENTIAL_BACKOFF:
            ...
            jitter_factor = 1.0 + (random.random() - 0.5) * 0.4  # nosec B311 - backoff jitter to prevent thundering herd, not cryptographic
            delay *= jitter_factor
        else:
            delay = self.config.base_delay
        ...
        if self.config.jitter and self.config.strategy != RetryStrategy.JITTERED_BACKOFF:
            jitter_factor = 1.0 + (random.random() - 0.5) * 0.2  # nosec B311 - delay jitter to prevent thundering herd, not cryptographic
            delay *= jitter_factor
