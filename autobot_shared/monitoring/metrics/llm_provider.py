...
class LLMProviderMetricsRecorder(BaseMetricsRecorder):
    ...
    def record_tokens(
        ...
        self.tokens_total.labels(provider=provider, model=model, token_type="input").inc(input_tokens)  # nosec B106
        self.tokens_total.labels(provider=provider, model=model, token_type="output").inc(output_tokens)  # nosec B106
        self.tokens_per_request.labels(provider=provider, model=model, token_type="input").observe(  # nosec B106
            input_tokens
        )
        self.tokens_per_request.labels(provider=provider, model=model, token_type="output").observe(  # nosec B106
            output_tokens
        )
