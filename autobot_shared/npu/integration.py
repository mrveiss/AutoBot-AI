...
# Author: mrveiss
"""Canonical NPU worker pool, client, queue and fallback (#12656, part of #12645).

These seven symbols were forked between ``autobot-backend/npu_integration.py``
and ``autobot-npu-worker/core/npu_integration.py``. Six were byte-identical;
only ``NPUWorkerClient`` had drifted, and that drift concealed a live defect
(#12910 — the worker awaited a sync ``get_http_client``).

Both sides now re-export from here, so there is one definition to fix.

Backend-only couplings are imported lazily or behind ``try``/``except``, exactly
as the npu-worker copy already did for the PyInstaller standalone build, so this
module has no hard dependency on ``autobot-backend``:

* ``utils.service_registry.get_service_url`` — falls back to an env-var lookup.
* ``utils.service_client.create_service_client`` — imported inside the method
  that uses it; on ImportError the client drops to unauthenticated mode, which
  is the behaviour the backend already had.
"""
...
if TYPE_CHECKING:  # backend-only type, annotation-only usage
...
from autobot_shared.ssot_constants import CircuitState, LLMDefaults, TimingConstants

logger = get_logger(__name__)

# Issue #255: authenticated service-to-service client. False falls back to
# unauthenticated mode (development/testing, and the standalone worker build).
USE_AUTHENTICATED_CLIENT = True

try:  # backend context
    from utils.service_registry import get_service_url
except (ImportError, ModuleNotFoundError):  # standalone worker context

    def get_service_url(service_name: str) -> str:
        """Env-var service lookup for contexts without the backend registry."""
        env_key = f"AUTOBOT_{service_name.upper().replace('-', '_')}_URL"
        return os.getenv(env_key, "http://127.0.0.1:8080")
...
def _load_dispatch():
    """Return (get_inference_config, UnsupportedArchitectureError), or None.

    GH#7352 pre-validates ``architecture_family`` against the npu-worker's
    dispatch table. That table ships with the worker, not the backend, so the
    import is resolved lazily **at call time** rather than bound at module
    import: binding it early made validation depend on module import order, and
    a module-level no-op fallback silently disabled the check instead of
    skipping it honestly. ``None`` means "no table here" — the backend forwards
    the field to the worker, which validates it there.
    """
    for module in ("workers.openvino_dispatch", "openvino_dispatch"):
        try:
            mod = __import__(module, fromlist=["get_inference_config"])
            return mod.get_inference_config, mod.UnsupportedArchitectureError
...
class NPUWorkerClient:
    ...
    async def load_model(
        ...
        if architecture_family is not None and dispatch is not None:
            get_inference_config, UnsupportedArchitectureError = dispatch
            try:
...
async def process_with_npu_fallback(
    ...
    ``queue_getter`` is injected because the task queue is a per-process
    singleton owned by whichever side is calling — the backend and the worker
    each have their own. Sharing the logic without sharing the singleton keeps
    this module free of a back-dependency on either caller (#12656).
    """
    try:
