...
class TestValidatePath:
    ...
    def test_valid_path_under_default_root(self, tmp_path):
        """Path under /tmp (a default root) resolves successfully."""
        result = validate_path(str(tmp_path / "file.txt"))
        assert isinstance(result, Path)

    def test_valid_path_under_custom_root(self, tmp_path):
    ...
    def test_traversal_escapes_all_roots(self, tmp_path):
    ...
    def test_path_outside_all_roots_no_path_leakage(self, tmp_path):
    ...
    def test_default_allowed_roots_used_when_none(self):
...
class TestValidateRelativePath:
    ...
    def test_null_byte_in_segment_raises(self):
        """Null byte in segment raises ValueError."""
        with pytest.raises(ValueError, match="empty or contains null bytes"):
            validate_relative_path("file\x00.txt", "/tmp/base")

    def test_absolute_path_as_segment(self, tmp_path):
    ...
    def test_symlink_escape_from_base(self, tmp_path):
    ...
    def test_base_dir_as_path_object(self, tmp_path):
        """base_dir can be a Path object (not just str)."""
        result = validate_relative_path("file.txt", Path(tmp_path))
        assert isinstance(result, Path)

    def test_dot_segment_stays_in_base(self, tmp_path):
