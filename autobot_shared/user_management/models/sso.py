...
# Author: mrveiss
"""
SSO Provider and User SSO Link Models (#12647)
...
before this move, but the diff was cosmetic/documentation only, no schema or
behavior difference: an unused ``typing.Optional`` import and the
``Mapped[Optional[...]]`` vs ``Mapped["... | None"]`` style (SLM's form,
already this repo's canonical style — see ``models/base.py``), plus SLM's
``SSOProvider``/``config`` docstrings and comments describing the
``SystemSecret``-table encryption design (added once that design landed in
SLM) versus the backend's older, less specific "encrypted at rest" wording.
This file keeps SLM's more accurate comments; no column or behavior changed.
"""
