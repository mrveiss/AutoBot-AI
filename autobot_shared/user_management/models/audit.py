...
# Author: mrveiss
"""
Audit Log Model (#12647)

Tracks all security-relevant actions for compliance.

Not byte-identical between ``autobot-backend`` and ``autobot-slm-backend``
before this move, but the diff was comment-wording only, no schema, value, or
behavior difference: the backend's ``# nosec``/``# nosemgrep`` suppression
comments were more verbose, and its ``PERMISSION_DENIED`` constant carried a
cross-reference note tying its string value to SLM's copy — a note that
stops being meaningful once both files are one canonical module. This file
keeps SLM's shorter comment wording; every enum/constant value is unchanged.
"""
