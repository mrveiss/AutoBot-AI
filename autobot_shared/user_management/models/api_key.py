...
Not byte-identical between ``autobot-backend`` and ``autobot-slm-backend``
before this move, but the diff was cosmetic only, no behavior difference:
the backend carried a module-level ``from __future__ import annotations``
(PEP 563) that SLM's copy did not have, plus the same unused
``typing.Optional`` import / ``Mapped[Optional[...]]`` style already noted on
the other model files. Every forward reference to a not-yet-imported class in
this module is already hand-quoted (e.g. ``Mapped[Optional["Team"]]``) or
uses a native ``X | None`` union (Python 3.10+), so PEP 563's deferred
evaluation changes nothing observable here; dropping it matches this repo's
canonical style (see ``models/base.py``, which also has no future-annotations
import) with no semantic change.
"""
