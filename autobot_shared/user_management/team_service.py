...
# Author: mrveiss
"""Canonical Team Service shared by both backends (#12647).

`team_service.py` was byte-identical across `autobot-backend` and
`autobot-slm-backend` — 701 duplicated lines, so any fix had to be applied
twice or it silently reached only one service.

It is the first `user_management` *service* that could move: its entire
runtime dependency set already lives in `autobot_shared` — `Team` /
`TeamMembership` (#13130), the audit vocabulary (#13130), and `BaseService` /
`TenantContext` (#12972). Its only mentions of `User` and `Organization` are
in docstrings and error strings, never as classes, so it does not need the
concrete per-backend models.

`organization_service.py` is also byte-identical but cannot follow yet: it
imports the concrete `User` and `Organization` at runtime, and those stay
backend-local by design (see `autobot_shared/user_management/models/user.py`).

Business logic for team management operations including CRUD,
membership management, and role assignment within teams.

Both backends keep a re-export shim, so every existing
`from user_management.services.team_service import TeamService` importer is
untouched — the fork is removed, not the callers.
"""
...
from autobot_shared.user_management.models.team import Team, TeamMembership
