...
# Author: mrveiss
"""The shared ``aiohttp`` session manager (#15641).

Split out of :mod:`autobot_shared.http_client`, which now holds only the
process-wide singleton that hands this class out. Everything about the
transport itself lives here: session creation, the adaptive connection-pool
sizing loop, request execution, and the in-flight request accounting that
deferred pool recreation depends on.

``autobot_shared.http_client`` re-binds ``HTTPClientManager`` by importing it,
so ``from autobot_shared.http_client import HTTPClientManager`` still resolves.
"""
...
# #15522: a launch whose handle is discarded can be collected before it runs —
# the loop holds only a weak reference. ``fire_and_forget`` retains the task
# until completion and logs a failure that would otherwise vanish. The import
# is what #15641 could not afford until this file existed.
from autobot_shared.async_compat import fire_and_forget
from autobot_shared.http_egress_guard import _assert_egress_allowed
...
class HTTPClientManager:
    ...
    async def request(
        ...
        **kwargs,
        ...
        # Check if pool adjustment needed (non-blocking). #15522: retained by
        # ``fire_and_forget`` — a discarded task can be collected before it
        # runs, and this one sits on the hot path of every guarded request.
        fire_and_forget(self._adjust_pool_size(), name="http-client-pool-adjust")
