...
class NPUWorkerManager(AsyncInitializable):
    ...
    async def _pulse_check_worker(self, worker_id: str) -> None:
        ...
        try:
            ...
            model_id = random.choice(models_list)  # nosec B311 - random probe selection for health check, not cryptographic
        except Exception as e:
    ...
    async def _pulse_probe_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                jitter = base_interval * 0.2 * (random.random() * 2 - 1)  # nosec B311 - interval jitter to prevent thundering herd, not cryptographic
