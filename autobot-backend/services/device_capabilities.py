...
async def _load_device(device_id: str):
    """Read the paired-device row, or ``None`` when it cannot be read.

    A lookup failure — no row, a malformed id, a database that will not answer
    — denies. Returning ``None`` rather than raising keeps the failure on the
    same path as an ordinary refusal, so no caller can grant access by
    forgetting a ``try``.
    """
