...
async def get_orchestrator_instance(request: Request = None):
    ...
    if request is not None:
        app_orchestrator = getattr(request.app.state, "advanced_workflow_orchestrator", None)
        if app_orchestrator is not None:
            logger.debug("Using pre-initialized advanced workflow orchestrator from app.state")
    ...
    logger.info("Creating new AdvancedWorkflowOrchestrator instance (expensive operation)")
