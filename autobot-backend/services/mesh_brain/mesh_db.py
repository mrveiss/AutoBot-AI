...
class MeshDB:
    ...
    async def update_edge(
        ...
        sql = text(f"UPDATE mesh_edges SET {', '.join(updates)} WHERE id = :edge_id::uuid")  # nosec B608 - column names are hardcoded string literals; all values use SQLAlchemy named params
        async with self.engine.begin() as conn:
