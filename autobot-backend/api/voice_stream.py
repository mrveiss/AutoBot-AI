...
async def _process_tts_chunk(
    ...
    try:
        ...
        if i + 1 < total and not cancel_event.is_set():
            prefetch = asyncio.create_task(tts.synthesize(chunks[i + 1], voice_id=voice_id, language=language))
...
async def _stream_chunks_pipelined(
    ...
    if not cancel_event.is_set():
        next_task = asyncio.create_task(tts.synthesize(chunks[0], voice_id=voice_id, language=language))

    for i in range(total):
        ok, next_task = await _process_tts_chunk(ws, i, total, next_task, chunks, cancel_event, tts, voice_id, language)
        if not ok:
            break
