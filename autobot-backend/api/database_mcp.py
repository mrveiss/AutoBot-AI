...
def _list_tables_sync(db_path: Path) -> list[dict]:
    ...
    try:
        ...
        for (table_name,) in tables:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM [{table_name}]")  # nosec B608  # nosemgrep: autobot-sql-string-format  # nosemgrep
...
def _describe_schema_sync(db_path: Path, table: str | None) -> dict:
    ...
    try:
        ...
        if table:
            ...
            cursor.execute(f"PRAGMA table_info([{table}])")  # nosec B608  # nosemgrep: autobot-sql-string-format  # nosemgrep
            ...
            for (table_name,) in tables:
                cursor.execute(f"PRAGMA table_info([{table_name}])")  # nosemgrep: autobot-sql-string-format  # nosemgrep
                columns = cursor.fetchall()
                schemas[table_name] = [
                    {
                        "cid": col[0],
