...
async def test_org_audit_log_admin_allowed():
    ...
    with patch(
        ...
        result = await get_organization_audit_log(request=_make_request(kb), current_user=user)
...
async def test_compliance_summary_org_admin_allowed():
    ...
    with patch(
        ...
        result = await get_compliance_summary(request=_make_request(kb), current_user=user)
