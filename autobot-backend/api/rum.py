...
async def log_rum_event(event: RumEvent):
    ...
    try:
        ...
        logger.error("Error logging RUM event: %s", "Internal server error")
    ...
    try:
        async with _rum_lock:
            ...
            for session_data in rum_sessions.values():
                ...
                if (datetime.now(tz=timezone.utc) - last_activity.replace(tzinfo=None)) < timedelta(minutes=30):
                    active_sessions += 1
        ...
        raise HTTPException(status_code=500, detail="Error getting RUM status")
...
def _process_user_actions(metrics_manager, user_actions: List[UserActionMetric]) -> None:
...
def _process_websocket_events(metrics_manager, websocket_events: List[WebSocketMetric]) -> None:
...
def _process_critical_issues(metrics_manager, critical_issues: List[CriticalIssueMetric]) -> None:
...
async def receive_rum_metrics(metrics: RumMetrics):
    ...
    try:
        ...
        logger.error("Error recording RUM metrics: %s", "Internal server error")
        # Don't raise HTTP exception to avoid disrupting user experience
        return {
            "status": "error",
            "message": "Failed to record RUM metrics",
            "session_id": metrics.session_id,
        }
