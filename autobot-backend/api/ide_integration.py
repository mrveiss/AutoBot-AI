...
from api.ide_pattern_rules import PATTERN_RULES
...
# PUT /config also needs admin: it changes the one engine every client on this
# worker shares, and can switch off the security rules for all of them.
...
async def update_config(config: IDEConfigurationUpdate, _: bool = Depends(check_admin_permission)) -> Dict[str, Any]:
