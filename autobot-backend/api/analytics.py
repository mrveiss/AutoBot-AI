    ...
    realtime_data: Dict[str, Any] = {
        ...
        "recent_api_calls": len(
            [
                ...
                if _parse_timestamp(call["timestamp"]) > datetime.now(tz=timezone.utc) - timedelta(minutes=1)
            ]
        ),
        # Issue #596: Fixed key names to match hardware_monitor.get_system_resources() output
...
def _get_recent_api_calls(cutoff_seconds: int = 10) -> list:
    ...
    return [call for call in analytics_state["api_call_patterns"] if _parse_timestamp(call["timestamp"]) > cutoff]
