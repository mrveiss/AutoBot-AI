...
_PING_TIMEOUT = 10  # seconds to wait for pong before closing
...
def _verify_token(token: str) -> dict | None:
    ...
    Returns None if token is missing, invalid, or auth is disabled.
    Uses auth_middleware singleton for consistent JWT verification.
    """
    if not token:
        return None
    try:
...
def _auth_required() -> bool:
    """Return True when JWT auth is enabled in the app config."""
    try:
        from auth_middleware import auth_middleware
...
async def _handle_message(ws: WebSocket, raw: str, user_payload: dict | None) -> bool:
    """
    ...
    if action == "subscribe":
        ...
        channel = data.get("channel", "")
        await _handle_unsubscribe(ws, channel)
...
async def live_events_endpoint(websocket: WebSocket):
    ...
    Query params:
        token (str, optional): JWT auth token. Required when auth is enabled.
    """
    # --- Auth ---
    ...
    await websocket.send_json(
        {
            ...
            "message": "Connected to live events. Send subscribe actions to receive events.",
        }
    )
