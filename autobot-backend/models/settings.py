...
class DataSettings(BaseSettings):
    ...
    chat_history_file: str = Field(default="data/chat_history.json", description="Chat history file path")
    chats_directory: str = Field(default="data/chats", description="Individual chat files directory")
    long_term_db_path: str = Field(default="data/agent_memory.db", description="Long-term memory database path")
    reliability_stats_file: str = Field(
        default="data/reliability_stats.json", description="Reliability statistics file"
    )
    knowledge_base_db: str = Field(default="data/knowledge_base.db", description="Knowledge base SQLite database")
...
class BackendSettings(BaseSettings):
    ...
    server_port: int = Field(default=NetworkConstants.BACKEND_PORT, description="Server port")
    api_endpoint: str = Field(default=ServiceURLs.BACKEND_LOCAL, description="API endpoint URL")
...
class SecuritySettings(BaseSettings):
    ...
    audit_log_file: str = Field(default="data/audit.log", description="Audit log file path")
    allowed_users: Dict[str, str] = Field(default={}, description="Allowed users (username: password)")
    roles: Dict[str, Metadata] = Field(default={}, description="User roles and permissions")
...
class DiagnosticsSettings(BaseSettings):
    ...
    use_llm_for_analysis: bool = Field(default=True, description="Use LLM for failure analysis")
    use_web_search_for_analysis: bool = Field(default=False, description="Use web search for analysis")
    auto_apply_fixes: bool = Field(default=False, description="Automatically apply suggested fixes")
...
class MemorySettings(BaseSettings):
    ...
    retention_days: int = Field(default=90, description="Memory retention period in days")
    max_entries_per_category: int = Field(default=10000, description="Maximum entries per category")
...
class OrchestratorSettings(BaseSettings):
    ...
    task_transport: str = Field(default="local", description="Task transport mechanism (local/redis)")
    ...
    def validate_transport(cls, v):
        ...
        if v not in _VALID_TASK_TRANSPORTS:
            raise ValueError(f"Task transport must be one of: {list(_VALID_TASK_TRANSPORTS)}")
        return v
    ...
    def to_dict(self) -> Metadata:
        ...
        return {
            ...
            "diagnostics": {
                ...
                "use_web_search_for_analysis": (self.diagnostics.use_web_search_for_analysis),
