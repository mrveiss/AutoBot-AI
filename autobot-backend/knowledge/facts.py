...
async def _try_npu_batch_embeddings(texts: List[str], client: Any) -> Optional[List[List[float]]]:
...
async def _generate_embeddings_fallback(texts: List[str]) -> List[List[float]]:
    ...
    async def embed_one(text: str) -> List[float]:
        async with semaphore:
            return await asyncio.to_thread(Settings.embed_model.get_text_embedding, text)
