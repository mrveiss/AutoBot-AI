...
class TestRAGQueryBenchmarks:
    ...
    def mock_embeddings(self):
        ...
        return [[random.random() for _ in range(384)] for _ in range(100)]  # nosec B311 - mock embedding vectors for benchmarks, not cryptographic
        ...
        return [
            {
                ...
                "embedding": [random.random() for _ in range(384)],  # nosec B311 - mock embedding for benchmark, not cryptographic
            }
            for i in range(1000)
        ]
    ...
    def test_vector_similarity_computation_benchmark(self, runner, mock_embeddings):
        ...
        query_vector = np.array([random.random() for _ in range(384)])  # nosec B311 - mock query vector for benchmark, not cryptographic
    ...
    def test_top_k_retrieval_benchmark(self, runner, mock_documents):
        ...
        query_vector = [random.random() for _ in range(384)]  # nosec B311 - mock query vector for benchmark, not cryptographic
