...
class TestConnectorSchedulerRedis:
    ...
    async def test_no_dual_leader_after_atomic_race(self):
        ...
        Scenario: worker A held the lock; the lock expired during a GC pause;
        worker B acquired it atomically; worker A then tries to refresh.  With
        the old GET+PEXPIRE approach, PEXPIRE would blindly extend B's TTL and
        both workers would believe they were leader.  The Lua script must return
        ...
        with patch(
            ...
            worker_a = ConnectorScheduler()
            worker_b = ConnectorScheduler()
            worker_a._worker_id = "worker-A"
            worker_b._worker_id = "worker-B"

            # Worker A wins the initial election
            ...
            store[_LEADER_KEY] = worker_b._worker_id
