...
def touches_ownership(metadata_update: Optional[Dict[str, Any]]) -> bool:
    """Whether a metadata update sets any field the ownership indexes depend on."""
    return bool(metadata_update) and any(key in metadata_update for key in OWNERSHIP_KEYS + _OWNER_KEYS)
