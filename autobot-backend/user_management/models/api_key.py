...
This file's only diff from SLM's copy was cosmetic (a redundant
``from __future__ import annotations`` plus the unused ``Optional`` import /
``Mapped[Optional[...]]`` style already noted on the other model files) —
see ``autobot_shared/user_management/models/api_key.py`` for the
