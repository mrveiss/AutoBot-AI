...
This file's only diff from SLM's copy was comment wording (verbose
``# nosec``/``# nosemgrep`` suppression comments and a cross-file note tying
``PERMISSION_DENIED`` to SLM's copy, now meaningless once both are one
module) — see ``autobot_shared/user_management/models/audit.py`` for the
