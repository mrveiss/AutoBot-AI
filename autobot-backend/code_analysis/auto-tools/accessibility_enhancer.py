...
class AccessibilityFixAgent:
    @staticmethod
    def _build_icon_labels() -> dict:
        """Return icon-class to ARIA label mapping. Issue #1183."""
        return {
            "fa-close": "Close",
            "fa-times": "Close",
            "fa-x": "Close",
    ...
    @staticmethod
    def _build_context_labels() -> dict:
        """Return context-keyword to ARIA label mapping. Issue #1183."""
        return {
            "submit": "Submit form",
            "send": "Send message",
            "login": "Sign in",
    ...
    def __init__(self, root_path: str = _DEFAULT_VUE_ROOT):
        ...
        self.icon_labels = self._build_icon_labels()
        self.context_labels = self._build_context_labels()
    ...
    def _apply_component_fixes(self, content: str) -> Tuple[str, List[str]]:
        """Apply all accessibility fixes to component content. Issue #1183."""
        modified = content
        all_fixes: List[str] = []
        modified, button_fixes = self.fix_button_accessibility(modified)
        all_fixes.extend(button_fixes)
        modified, image_fixes = self.fix_image_accessibility(modified)
        all_fixes.extend(image_fixes)
        modified, keyboard_fixes = self.fix_keyboard_navigation(modified)
        all_fixes.extend(keyboard_fixes)
        return modified, all_fixes

    def _record_component_fix(
        self,
        file_path: Path,
        ...
        """Write fixed file, record result, and print summary. Issue #1183."""
    ...
    @staticmethod
    def _build_wcag_section() -> str:
        """Return the static WCAG compliance and implementation details section. Issue #1183."""
        return _WCAG_REPORT_SECTION

    def _build_markdown_report(self) -> str:
        """Build the full markdown report content. Issue #1183."""
        s = self.report_data["summary"]
        content = (
            f"# Accessibility Fix Report - AutoBot Frontend\n\n"
            f"**Generated**: {self.report_data['timestamp']}\n"
            f"**Agent**: Accessibility Fix Agent v1.0\n\n"
            f"## Summary\n\n"
            f"- **Files Scanned**: {s['total_files_scanned']}\n"
            f"- **Files Modified**: {s['files_modified']}\n"
            f"- **Total Fixes Applied**: {s['total_fixes_applied']}\n"
            f"- **Accessibility Score Improvement**: +{s['accessibility_improvements']} points\n\n"
            f"## Fixes Applied\n\n"
        )
        for fix_record in self.report_data["fixes_applied"]:
            content += (
                f"### {fix_record['file']}\n"
                f"**Accessibility Score**: {fix_record['initial_score']} → "
                f"{fix_record['final_score']} (+{fix_record['accessibility_score_improvement']})\n"
                f"**Backup**: {fix_record['backup']}\n\n**Fixes Applied**:\n"
            )
            for fix in fix_record["fixes"]:
                content += f"- {fix}\n"
            content += "\n"
        content += self._build_wcag_section()
        return content
...
if __name__ == "__main__":
    ...
    root_path = sys.argv[1] if len(sys.argv) > 1 else _DEFAULT_VUE_ROOT
