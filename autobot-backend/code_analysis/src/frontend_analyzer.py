...
class FrontendAnalyzer:
    ...
    def _analyze_framework_usage(self, components: List[FrontendComponent]) -> Dict[str, Any]:
        ...
        for component in components:
            ...
            if framework not in usage:
                usage[framework] = {"count": 0, "components": [], "common_patterns": []}
    ...
    async def _analyze_frontend_performance(
        self, components: List[FrontendComponent], issues: List[FrontendIssue]
    ) -> Dict[str, Any]:
    ...
    def _generate_frontend_recommendations(
        self, issues: List[FrontendIssue], framework_usage: Dict[str, Any]
    ) -> List[str]:
    ...
    def _calculate_frontend_quality_score(
        self, issues: List[FrontendIssue], components: List[FrontendComponent]
    ) -> float:
...
async def main():
    ...
    for framework, usage in results["framework_usage"].items():
        print(f"{framework}: {usage['count']} components ({usage['percentage']:.1f}%)")  # noqa: print
    ...
    for category in categories:
        ...
        if category_issues:
            print(f"\n=== {category.replace('_', ' ').title()} Issues ===")  # noqa: print
            for issue in category_issues[:5]:  # Show first 5
                print(f"• {issue['description']} ({issue['file_path']}:{issue['line_number']})")  # noqa: print
