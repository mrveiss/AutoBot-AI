...
def _run_openssl_command(cmd: List[str], operation: str, context: str = "") -> Tuple[bool, str | None]:
    ...
    """
    try:
        subprocess.run(cmd, check=True, capture_output=True)  # nosec B603 - cmd is openssl argv built by callers using hardcoded arg lists
        return True, None
...
class CertificateGenerator:
    ...
    def _generate_ca_private_key(self, ca_key: Path) -> bool:
        ...
        try:
            subprocess.run(key_cmd, check=True, capture_output=True)  # nosec B603 - key_cmd is fixed openssl argv built above
    ...
    def _generate_ca_certificate(self, ca_key: Path, ca_cert: Path) -> bool:
        ...
        try:
            subprocess.run(cert_cmd, check=True, capture_output=True)  # nosec B603 - cert_cmd is fixed openssl argv built above
    ...
    def _verify_certificate(self, cert_path: Path) -> bool:
        ...
        verify_result = subprocess.run(verify_cmd, capture_output=True)  # nosec B603 - verify_cmd is fixed openssl argv built above
    ...
    def get_certificate_status(self, cert_path: Path) -> CertificateStatus:
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, check=True)  # nosec B603 - cmd is fixed openssl argv built above
