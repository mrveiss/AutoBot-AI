...
def test_an_admin_writes_settings(client):
    ...
    stub's ``get_current_user``; authorship is ``TestAnAdmittedWriteRecordsItsAuthor``
    in ``api/settings_config_test.py`` and ``api/settings_sync_hardware_priority_created_by_test.py``.
    """
    test_client, identity = client
