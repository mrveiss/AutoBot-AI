...
class SSOIntegrationFramework:
            ...
            **self.stats,
            ...
            "session_statistics": {
                ...
                "sessions_expiring_soon": len(
                    [s for s in self.active_sessions.values() if s.expires_at - datetime.utcnow() < timedelta(hours=1)]
                ),
            },
        }
    ...
    def _calculate_average_session_age(self) -> float:
        ...
        total_age = sum(
            (datetime.utcnow() - session.created_at).total_seconds() / 60 for session in self.active_sessions.values()
        )
