...
class APIConsistencyAnalyzer:
    ...
    def __init__(self, redis_client=None):
        ...
        self.framework_patterns = {
            ...
            'flask': [
                (r'@app\.route\s*\([\'"]([^\'"]+)[\'"].*?methods\s*=\s*\[[\'"](.*?)[\'"]\]', 'Flask route'),
                (r'@bp\.route\s*\([\'"]([^\'"]+)[\'"].*?methods\s*=\s*\[[\'"](.*?)[\'"]\]', 'Flask blueprint'),
            ],
            'django': [
    ...
    def _analyze_decorator_for_endpoint(self, decorator: ast.AST, func_node: ast.FunctionDef, 
                                      file_path: str, lines: List[str]) -> Optional[APIEndpoint]:
        ...
        for resource, resource_endpoints in path_patterns.items():
            if len(resource_endpoints) > 1:
                ...
                if len(function_patterns) > 1:
                    inconsistencies.append(APIInconsistency(
                ...
                if len(formats) > 1 and 'unknown' in formats:
                    ...
                    if len(known_formats) > 1:
                        inconsistencies.append(APIInconsistency(
                            ...
                            affected_endpoints=[ep for ep in method_endpoints if ep.response_format in known_formats],
...
async def main():
    ...
    for endpoint in results['endpoints']:
        ...
        print(f"  Auth: {endpoint['has_auth']}, Validation: {endpoint['has_validation']}, "
