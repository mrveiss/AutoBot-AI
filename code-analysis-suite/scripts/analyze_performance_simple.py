...
class SimplePerformanceAnalyzer:
    ...
    def __init__(self):
        ...
        self.patterns = {
            'memory_leaks': [
                (r'open\s*\([^)]+\)(?!\s*(?:with|\.close\(\)|as\s+))', 'Unclosed file handle'),
                (r'requests\.[a-z]+\([^)]+\)(?!\s*\.close\(\))', 'Unclosed HTTP connection'),
            ],
            'blocking_calls': [
                (r'async\s+def\s+[^:]+:.*?time\.sleep\s*\(', 'time.sleep in async function'),
                (r'async\s+def\s+[^:]+:.*?requests\.[a-z]+\s*\(', 'requests in async function'),
            ],
            'inefficient_loops': [
                (r'for\s+[^:]+:\s*\n\s*for\s+[^:]+:\s*\n\s*for\s+', 'Triple nested loop'),
                (r'for\s+[^:]+:.*?\+=\s*[\'"]', 'String concatenation in loop'),
            ],
            'database_issues': [
                (r'for\s+[^:]+:.*?\.execute\s*\(', 'Query in loop (N+1 problem)'),
                (r'SELECT\s+\*\s+FROM\s+\w+(?!\s+(?:WHERE|LIMIT))', 'Unbounded SELECT *'),
            ]
        }
