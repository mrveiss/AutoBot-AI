...
from src.utils.semantic_chunker import AutoBotSemanticChunker, SemanticChunk
...
class TestSemanticChunking:
    ...
    async def test_large_text_splitting(self):
        ...
        for chunk in chunks:
            assert len(chunk.content) <= self.chunker.max_chunk_size + 100, \
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            # Test edge cases
            small_chunks = await self.test_small_text_handling()
            print()
            
            large_chunks = await self.test_large_text_splitting()
            print()
