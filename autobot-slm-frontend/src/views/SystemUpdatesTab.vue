 ...
 *
 * Extracted from UpdatesView into a tab component.
...
const logger = createLogger('SystemUpdatesTab')
...
async function fetchJobs(): Promise<void> {
  ...
  const result = await apiFetch<{ jobs: UpdateJob[]; total: number }>(
    '/api/updates/jobs',
  )
...
<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-semibold text-gray-900">System Updates</h2>
        <p class="text-sm text-gray-500 mt-1">
          Check for updates, track jobs, manage fleet-wide updates
        </p>
      </div>
      <button
    ...
    >
      {{ errorMessage }}
      <button @click="errorMessage = null" class="ml-2 underline">
        Dismiss
      </button>
    </div>
    <div
    ...
    <div v-if="checkResult" class="bg-white rounded-lg border mb-6">
      ...
      >
        <h2 class="font-medium text-gray-900">
          Available Updates ({{ checkResult.total }})
        </h2>
      </div>
      <div v-if="checkResult.updates.length" class="divide-y">
        <div
          v-for="upd in checkResult.updates"
          :key="upd.package_name"
        ...
        >
          <div>
            ...
            <p class="text-xs text-gray-500">
              {{ upd.current_version }} &rarr; {{ upd.available_version }}
            </p>
          </div>
          <span
            :class="[
    ...
    <div class="bg-white rounded-lg border">
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                ...
                >
                  <div
                    ...
                    :style="{
                      width: `${
                        job.total_updates
                          ? (job.applied_count / job.total_updates) * 100
                          : 0
                      }%`,
                    }"
                  ></div>
                ...
                <span class="text-xs text-gray-500">
                  {{ job.applied_count }}/{{ job.total_updates }}
                </span>
              </div>
            </td>
      ...
      <div v-if="!jobs.length" class="p-8 text-center text-gray-500">
