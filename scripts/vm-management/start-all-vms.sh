...
start_redis_vm() {
    log "Starting Redis service on VM (172.16.168.23)..."
    ...
    ssh -T -i "$SSH_KEY" "$SSH_USER@172.16.168.21" << 'EOF'
        ...
        if ! command -v nc >/dev/null 2>&1 || ! command -v node >/dev/null 2>&1; then
            ...
            if sudo -n true 2>/dev/null; then
                ...
                echo "⚠️  Cannot install packages: passwordless sudo not configured"
...
main() {
    ...
    echo -e "${CYAN}  Frontend:   http://172.16.168.21:5173${NC}"
    echo -e "${CYAN}  Redis:      172.16.168.23:6379${NC}"
    echo -e "${CYAN}  AI Stack:   http://172.16.168.24:8080/health${NC}"
    echo -e "${CYAN}  NPU Worker: http://172.16.168.22:8081/health${NC}"
    echo -e "${CYAN}  Browser:    http://172.16.168.25:3000/health${NC}"
    echo ""
