    ...
    def scan_hardcoded_timeouts(self) -> List[TimeoutIssue]:
        ...
        for scan_path in scan_paths:
            ...
            if full_scan_path.exists():
                self._scan_directory_recursive(full_scan_path, timeout_patterns, "hardcoded_timeout", exclude_patterns)

        return self.issues

    def _scan_directory_recursive(self, directory: Path, patterns: List[str], issue_type: str, exclude_patterns: List[str]):
        ...
        for file_path in directory.rglob('*'):
            if file_path.is_file():
                ...
                if file_path.suffix in ['.py', '.js', '.ts', '.vue', '.yaml', '.yml', '.json', '.env']:
    ...
    def _analyze_timeout_context(self, file_path: str, line: str, value: str, issue_type: str) -> Tuple[str, str, str]:
        ...
        if 'redis' in issue_type.lower() or 'redis' in file_path.lower():
            if numeric_value > 30:
                ...
                return "medium", "5.0", f"Redis timeout too low: {value}s (recommend 5s)"
            elif numeric_value != 5.0:
                return "medium", "5.0", f"Redis timeout inconsistent: {value}s (standard: 5s)"
        ...
        if 'api' in line.lower() or 'http' in line.lower():
            if numeric_value > 60:
                ...
                return "high", "30", f"API timeout too low: {value}s (may cause failures)"
...
async def main():
    ...
    parser.add_argument('--scan-hardcoded', action='store_true', help='Scan for hardcoded timeout values')
    parser.add_argument('--generate-config', action='store_true', help='Generate unified timeout configuration module')
    ...
    # Generate unified config if requested
