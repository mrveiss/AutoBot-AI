...
async def vectorize_facts_batch(
    ...
    for i in range(0, total, batch_size):
        ...
        logger.info(
            ...
            processed, total, pct, success_count, error_count, skipped_count
        )
...
async def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Count facts without vectorizing"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    # Vectorize
    result = await vectorize_facts_batch(
        kb,
        doc_fact_ids,
