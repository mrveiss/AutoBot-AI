...
secure_vm_exec() {
    ...
    if ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$SSH_USER@$vm_ip" "$command" 2>/dev/null; then
        return 0
    fi
    ...
    if [[ "$command" == *"sudo"* ]]; then
        ...
        if [ -n "$password" ]; then
            # Try with password
            echo "$password" | ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$SSH_USER@$vm_ip" "sudo -S $command" 2>/dev/null
            return $?
        else
        ...
        ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$SSH_USER@$vm_ip" "$command"
        return $?
    fi
}
...
secure_package_install() {
    ...
    for pkg in $packages; do
        if ! ssh -i "$SSH_KEY" -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$SSH_USER@$vm_ip" "dpkg -l | grep -q '^ii.*$pkg '" 2>/dev/null; then
    ...
    if ! check_passwordless_sudo "$vm_ip" "$vm_name"; then
        ...
        if [ -n "$password" ]; then
            echo "$password" | ssh -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$SSH_USER@$vm_ip" "sudo -S apt update && sudo -S apt install -y$missing_packages" 2>/dev/null
        else
            warning "Skipping package installation on $vm_name"
            return 1
        fi
    else
        ssh -i "$SSH_KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$SSH_USER@$vm_ip" "sudo apt update && sudo apt install -y$missing_packages"
    fi
}
