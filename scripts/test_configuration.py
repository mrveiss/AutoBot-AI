...
def test_config_values():
    ...
    try:
        from src.config import API_BASE_URL, REDIS_URL, OLLAMA_URL, API_TIMEOUT
...
def test_environment_overrides():
    ...
    try:
        ...
        if API_BASE_URL == test_value:
            ...
            print(f"   ⚠️  Environment override partial: got {API_BASE_URL}, expected {test_value}")
...
def test_config_validation():
    ...
    try:
        ...
        print(f"   ✅ Validation status: {validation_result.get('config_loaded', 'unknown')}")
        
        if validation_result.get('issues'):
            print(f"   ⚠️  Configuration issues found: {len(validation_result['issues'])}")
...
def test_config_performance():
    ...
    try:
        ...
        if avg_time_ms < 1.0:  # Less than 1ms average
            print(f"   ✅ Configuration access performance good: {avg_time_ms:.2f}ms avg")
            return True
        else:
            print(f"   ⚠️  Configuration access slower than expected: {avg_time_ms:.2f}ms avg")
