...
from src.security.domain_security import DomainSecurityManager, DomainSecurityConfig
...
class WebResearchSecurityTester:
    ...
    async def _test_input_validator(self):
        ...
        try:
            ...
            if not suspicious_result["safe"] or suspicious_result["risk_level"] in ["medium", "high"]:
        ...
        try:
            async with SecureWebResearch() as secure_research:
                ...
                if query_result["safe"]:
                    ...
                    test_details["subtests"]["query_validation"] = f"FAILED - {query_result.get('threats_detected', [])}"
                    test_details["failed"] += 1
                ...
                if component_test["overall_status"] == "passed":
                    ...
                    test_details["subtests"]["component_tests"] = f"FAILED - {component_test.get('overall_status', 'unknown')}"
                    test_details["failed"] += 1
    ...
    async def _test_malicious_detection(self):
        ...
        try:
            ...
            test_cases = [
                ...
                ("data:text/html,<script>alert(1)</script>", True, "data_uri_injection"),
            ...
            for query, should_block, test_case_name in test_cases:
                ...
                if should_block:
                    ...
                    if result["safe"]:
                        ...
                        test_details["subtests"][test_case_name] = f"FAILED - Incorrectly blocked: {result.get('threats_detected', [])}"
                        test_details["failed"] += 1
