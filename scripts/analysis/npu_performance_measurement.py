...
class NPUPerformanceMeasurement:
    ...
    async def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            benchmark_results["hardware_baseline"] = await self._measure_hardware_baseline()
            ...
            benchmark_results["semantic_search_tests"] = await self._test_semantic_search_engine()
            ...
            benchmark_results["performance_comparison"] = await self._analyze_performance_comparison(benchmark_results)

            # 6. Generate Recommendations
            logger.info("💡 Step 6: Generating optimization recommendations")
            benchmark_results["recommendations"] = await self._generate_recommendations(benchmark_results)
    ...
    async def _measure_hardware_baseline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                baseline_results["gpu_info"] = {"gpu_available": False, "torch_not_available": True}
    ...
    async def _test_cpu_embedding_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_text = "This is a test sentence for CPU embedding performance measurement."

            for i in range(5):
                start_time = time.time()
                embeddings = chunker._compute_sentence_embeddings([test_text])
                end_time = time.time()
            ...
            for batch_size in batch_sizes:
                ...
                start_time = time.time()
                embeddings = chunker._compute_sentence_embeddings(batch_texts)
                end_time = time.time()
    ...
    async def _test_gpu_embedding_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_text = "This is a test sentence for GPU embedding performance measurement."

            for i in range(5):
                start_time = time.time()
                embeddings = await chunker._compute_sentence_embeddings_async([test_text])
                end_time = time.time()
            ...
            for batch_size in batch_sizes:
                ...
                start_time = time.time()
                embeddings = await chunker._compute_sentence_embeddings_async(batch_texts)
                end_time = time.time()
    ...
    async def _test_npu_embedding_generation(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        embedding_results["single_embeddings"].append({
                            ...
                            "embedding_dimension": len(result.get("embeddings", [])),
                            "success": True
                        })
                    else:
                        embedding_results["single_embeddings"].append({
            ...
            successful_single = [r for r in embedding_results["single_embeddings"] if r["success"]]
            if successful_single:
                ...
                embedding_results["performance_metrics"] = {
                    ...
                    "success_rate": len(successful_single) / len(embedding_results["single_embeddings"]) * 100
                }
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    for query in test_queries:
                        ...
                        async with session.post(
                            ...
                            json={
                                ...
                                "document_metadata": [{"text": doc} for doc in test_documents],
                                "top_k": 3,
                    ...
                    # Calculate performance metrics
                    successful_searches = [r for r in search_results["search_tests"] if r["success"]]
                    if successful_searches:
                        ...
                        search_results["performance_metrics"] = {
                            ...
                            "success_rate": len(successful_searches) / len(search_results["search_tests"]) * 100
                        }
        ...
        try:
            ...
            for i, config in enumerate(search_configs):
                ...
                if successful_results:
                    ...
                    embedding_times = [r["embedding_time_ms"] for r in successful_results]
    ...
    async def _test_api_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
                # Test health check
                start_time = time.time()
                async with session.get(f"{self.backend_url}/api/search/health") as response:
                    if response.status == 200:
                        health_data = await response.json()
                ...
                if api_results["health_check"]["success"]:
                        ...
                        async with session.post(
                            ...
                            if response.status == 200:
                                ...
                                search_tests.append({
                                    "query": query,
                                    "response_time_ms": (time.time() - start_time) * 1000,
                                    "api_search_time_ms": search_data.get("search_time_ms", 0),
                                    "results_count": search_data.get("total_results", 0),
                                    "device_used": search_data.get("device_used", "unknown"),
                                    "success": True
                                })
                            else:
                                search_tests.append({
                    ...
                    api_results["semantic_search"] = {
                        "tests": search_tests,
                        "avg_response_time_ms": np.mean([t["response_time_ms"] for t in search_tests if t["success"]]) if any(t["success"] for t in search_tests) else 0
                    }
        ...
        try:
            ...
            if search_performance:
                ...
                comparison["efficiency_metrics"] = {
                    ...
                    "best_configuration": min(config_times.keys(), key=lambda k: config_times[k]) if config_times else None,
                    "worst_configuration": max(config_times.keys(), key=lambda k: config_times[k]) if config_times else None
                }
    ...
    async def _generate_recommendations(self, benchmark_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if not npu_connected:
                recommendations.append("CRITICAL: NPU Worker not accessible - verify VM2 (172.16.168.22) is running and NPU Worker service is started")
            elif not npu_available:
                recommendations.append("NPU hardware not available - install Intel NPU drivers and OpenVINO for optimal performance")
            else:
                recommendations.append("✅ NPU Worker is operational - NPU acceleration is available")
            ...
            if speedup:
                ...
                if npu_vs_cpu > 5:
                    recommendations.append(f"Excellent NPU performance: {npu_vs_cpu:.1f}x faster than CPU - route lightweight tasks to NPU")
                elif npu_vs_cpu > 2:
                    recommendations.append(f"Good NPU performance: {npu_vs_cpu:.1f}x faster than CPU - optimize NPU utilization")
                elif npu_vs_cpu < 1:
                    recommendations.append("NPU performance suboptimal - check NPU optimization settings and model quantization")

                if npu_vs_gpu > 1.5:
                    recommendations.append(f"NPU outperforms GPU for embedding tasks ({npu_vs_gpu:.1f}x) - prioritize NPU for semantic search")
                elif npu_vs_gpu < 0.7:
                    recommendations.append("GPU outperforms NPU for these tasks - consider GPU-first routing strategy")
            ...
            if hardware_util:
                ...
                if cpu_cores >= 16:
                    recommendations.append(f"High-core CPU detected ({cpu_cores} cores) - optimize parallel processing and batch sizes")

                if gpu_available:
                    gpu_memory = hardware_util.get("gpu_memory_total_gb", 0)
                    if gpu_memory >= 8:
                        recommendations.append(f"High-memory GPU available ({gpu_memory}GB) - optimize for large batch processing")
            ...
            if efficiency:
                ...
                if best_config:
                    recommendations.append(f"Optimal configuration identified: {best_config} - use this for production workloads")

            # General recommendations
            if len(recommendations) == 0:
                recommendations.append("System performance analysis incomplete - run full benchmark for detailed recommendations")

            recommendations.append("Monitor hardware utilization during production workloads for ongoing optimization")
