...
class AutoBotInventory:
    ...
    def connect_redis(self):
        ...
        try:
            ...
            print(f"ERROR: Cannot connect to Redis at {self.redis_host}:{self.redis_port}", file=sys.stderr)
    ...
    def build_inventory_from_redis(self, redis_client):
        ...
        if not hosts:
            print("WARNING: No hosts found in Redis, using fallback static inventory", file=sys.stderr)
    ...
    def build_fallback_inventory(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'frontend': {
                'hosts': ['autobot-frontend'],
                'vars': {}
            },
            'database': {
                'hosts': ['autobot-database'],
                'vars': {}
            },
            'npu': {
                'hosts': ['autobot-npu'],
