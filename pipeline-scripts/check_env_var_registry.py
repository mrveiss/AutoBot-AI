...
Also checks that the autogenerated section of docs/developer/ENV_VARS.md
...
# The expected table comes from the generator itself, so the check and the file
# it writes cannot drift apart (#16317). pipeline-scripts/ is not a package.
sys.path.insert(0, str(Path(__file__).resolve().parent))

from generate_env_docs import BEGIN_MARKER, DOCS_PATH, END_MARKER, build_table  # noqa: E402
...
def check_docs_freshness() -> list[str]:
    ...
    if current_section != expected_section:
        violations.append(
            ...
            "  Then: git add docs/developer/ENV_VARS.md"
        )
