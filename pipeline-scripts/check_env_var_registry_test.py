...
def test_the_generated_table_carries_no_count_line(checker):
    """A count line that every env-var PR rewrote made any two of them conflict (#16317)."""
    table = checker.build_table(checker.REGISTRY)

    assert "variables registered" not in table
    assert table.rstrip("\n").splitlines()[-1].startswith("| `AUTOBOT_")


def test_the_rules_doc_points_at_the_table_instead_of_carrying_it(checker):
    """The table lives in its own generated doc, so env-var PRs stop colliding in
    CLAUDE_RULES.md, the rules doc every agent loads (#16317)."""
    rules = (checker.REPO_ROOT / "docs" / "developer" / "CLAUDE_RULES.md").read_text(encoding="utf-8")

    assert checker.DOCS_PATH.name == "ENV_VARS.md"
    assert checker.BEGIN_MARKER not in rules
    assert "ENV_VARS.md" in rules
...
def test_a_pipe_in_a_description_is_escaped(checker):
    ...
    table = checker.build_table({"AUTOBOT_PIPE": _Piped()})
