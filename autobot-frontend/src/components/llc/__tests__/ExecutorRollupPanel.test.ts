...
describe('ExecutorRollupPanel (#13942, #14222)', () => {
  it('renders the exact per-class totals and per-cell counts the matrix carries', () => {
    ...
    expect(wrapper.get('[data-testid="executor-rollup-total-orphaned"]').text()).toBe('5')
    expect(wrapper.get('[data-testid="executor-rollup-total-unassigned"]').text()).toBe('3')
    expect(wrapper.get('[data-testid="executor-rollup-total"]').text()).toContain('15')
    ...
    expect(wrapper.get('[data-testid="executor-rollup-cell-orphaned-in_progress"]').text()).toBe('5')
    ...
    // The orphaned legend badge resolves a real translation, not a raw i18n key (#14222).
    expect(wrapper.get('[data-testid="executor-rollup-legend-orphaned"]').text()).toContain('Orphaned')
  })

  it('shows the unassigned AND orphaned buckets even when zero — neither is hidden as a non-finding', () => {
    ...
    expect(wrapper.find('[data-testid="executor-rollup-legend-orphaned"]').exists()).toBe(true)
    expect(wrapper.get('[data-testid="executor-rollup-total-orphaned"]').text()).toBe('0')
  })
