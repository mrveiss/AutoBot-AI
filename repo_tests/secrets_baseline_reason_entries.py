...
# Author: mrveiss
"""The (filename, type, hashed_secret) -> reason table for the baseline-reasons guard.

Split out of ``secrets_baseline_reasons.py`` at #17134, when that module reached 604
lines against the 600-line ratchet. The table is the part that grows -- every new
baseline entry adds one -- so it is the part that moves, leaving the module that
owns the guard's logic and its frozen legacy-key hash at a stable size.

``BaselineKey`` is defined HERE rather than imported, so this module depends on
nothing and the parent can re-export both names without a cycle.
"""
