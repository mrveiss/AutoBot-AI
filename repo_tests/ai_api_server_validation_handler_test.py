...
# Author: mrveiss
"""ai_api_server.py's duplicated 422 handler never echoes the payload
(#16428 security review).

This container has no `agents` package on this checkout (it's assembled into
the Docker image separately, outside this repo layout) and no autobot_shared
either, so `ai_api_server.py` itself cannot be imported here. Extracts the
handler function's real source by AST line range instead of retyping it --
this fails the moment the duplicate drifts from what the file actually
contains, which a hand-copied mirror function would not catch.
"""
...
_SERVER_PATH = repo_root() / "autobot-infrastructure/shared/docker/ai-stack/ai_api_server.py"
...
def _extract_handler_source() -> str:
    ...
    raise AssertionError("_validation_error_without_input not found in ai_api_server.py -- duplicate was removed?")
