...
class AutoBotMonitor:
    ...
    def get_npu_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                result = subprocess.run(['intel_gpu_top', '-l', '-n', '1'], 
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                services['redis'] = {'status': 'api_error', 'error': f"HTTP {response.status_code}"}
        except Exception as e:
    ...
    def test_inference_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(f"{self.api_base}/api/chat/message", 
                json={
                    'message': test_prompt,
                    'agent_type': 'chat'
                }, timeout=30)
    ...
    def print_status_dashboard(self):
        ...
        print("\n" + "="*80)
        print(f"🚀 AutoBot System Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("="*80)
        ...
        if health.get('status') == 'healthy':
            ...
            print(f"   Embedding Model: {health.get('current_embedding_model', 'unknown')}")
        ...
        if npu.get('hardware_detected'):
            ...
            if npu.get('wsl_limitation'):
                ...
                print(f"   OpenVINO: {'✅' if npu.get('openvino_support') else '❌'} {'Supported' if npu.get('openvino_support') else 'Not detected'}")
        ...
        for service_name, service_info in services.items():
            ...
            print(f"   {status_icon} {service_display}: {status.replace('_', ' ').title()} {extra_info}")
            ...
            if service_name == 'openvino' and service_info.get('devices'):
                ...
                print(f"      RediSearch: {'✅ Enabled' if service_info['search_module'] else '❌ Disabled'}")
            elif service_name == 'playwright' and 'browsers_available' in service_info:
                print(f"      Browsers: {'✅ Installed' if service_info['browsers_available'] else '❌ Missing'}")
