...
| | `autobot-frontend/src/config/ServiceDiscovery.js` | ~488 | Dynamic URL resolution | Complex fallback logic |
| | `autobot-frontend/src/constants/network.ts` | ~204 | TypeScript network constants | Mirrors Python |
...
| | `autobot-frontend/.env.example` | ~20 | Frontend env vars | Separate from backend |
...
6. **Environment File Fragmentation**: `.env` and `autobot-frontend/.env` not synchronized
...
master-config-schema.json
    ...
    +-> generate-typescript.ts -> autobot-frontend/src/config/generated/types.ts
...
MASTER_ENV=".env"
FRONTEND_ENV="autobot-frontend/.env"
...
| `autobot-frontend/.env.example` | Auto-generated from master |
...
2. Create `autobot-frontend/src/config/ssot-config.ts` with TypeScript types
...
**Files to Deprecate**:
- `autobot-frontend/src/config/defaults.js` (merge into ssot-config.ts)
