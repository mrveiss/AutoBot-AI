...
### Service-auth pre-deployment check

Run before `deploy-service-auth.yml`. It verifies Ansible can reach the fleet, that all six
`service:key:*` entries exist in Redis, and that a service-key backup is present — and it exits
non-zero rather than printing a completion banner when any of those fails (#14867, #15079):

```bash
bash autobot-infrastructure/shared/scripts/test-service-auth-deployment.sh
```

Keys are generated by `autobot-infrastructure/shared/scripts/generate_service_keys.py`.
