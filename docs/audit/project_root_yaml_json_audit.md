...
1. `compliance.yaml` — the only confirmed bug. Drop the placeholder so the
   ...
   through `autobot_shared.paths.project_root()`. Tracked as #13658, done.
...
5. **Superseded by #15044.** `threat_detection.yaml`'s `model_storage_path`
   (with siblings `save_trained_models`, `load_existing_models`) was removed
   rather than left: the file advertised trained-model persistence the engine
   never implemented, which is the same defect as a dead key one layer up.
   `logging.yml` needed a fix after all — "no fix" undersold it. It is
   regenerated by a live `--setup` path, so the checked-in copy is now
   documented as a generated artefact with no `${…}` placeholder, rather than
   deleted; see `repo_tests/no_inert_config_placeholder_15044_test.py`. The
   orphaned `logging/promtail/promtail-config.yml` copy (never deployed —
   Ansible renders its own template) is removed outright.
