...
All `AUTOBOT_*` environment variables must be registered before use, and the
pre-commit hook `env-vars-documented` enforces this.

The generated reference table, along with how to add a variable, is in
[`ENV_VARS.md`](ENV_VARS.md). It has its own file so that env-var PRs don't
conflict here (#16317).
