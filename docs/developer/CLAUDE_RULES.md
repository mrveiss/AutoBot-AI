...
| `AUTOBOT_API_KEY_SCOPES_ENFORCED_FROM` | slm | str | `'2026-09-11'` | ISO date from which API-key scopes are enforced; keys created before it get the legacy grace period. Set it to the day enforcement reached this deployment if that was later (#16040 AC5). |
...
| `AUTOBOT_BRANCH_INTEREST_TTL_S` | orchestration | int | `1209600` | Backstop lifetime for a branch's recorded interest in a scope (#15987). This is NOT how long the interest is meant to last: an interest ends when its branch merges or closes, which is a fact someone looks up, not a timer. The TTL exists only so a record nobody pruned cannot outlive the repository, and a record reaching it means prune() was never called rather than that the work was abandoned. Two weeks by default, which no healthy branch should reach. Range: 3600–7776000. |
| `AUTOBOT_BROWSER_SERVICE_HOST` | network | str | `'127.0.0.1'` | Hostname or IP of the Playwright/browser automation service (services/playwright_service.py). |
| `AUTOBOT_BROWSER_SERVICE_PORT` | network | int | `9001` | Port of the Playwright/browser automation service. 9001, not 3000 -- that's Grafana's (#4052). |
...
| `AUTOBOT_GIT_BRANCH` | system | str | `'main'` | Git branch that the running instance was built from. |
...
| `AUTOBOT_IDEMPOTENCY_CLAIM_ATTEMPTS` | backend | int | `3` | How many times an idempotency claim re-runs its atomic SET NX after the record it lost to turns out to have expired. Reporting 'unseen' at that point would let every loser of the race create (autobot_shared/idempotency.py, #15778). |
| `AUTOBOT_IDEMPOTENCY_CLAIM_TTL_SECONDS` | backend | int | `300` | How long an in-flight idempotency claim is held before another caller may retry it. A request that dies between claiming and completing would otherwise wedge the key for the full replay TTL (autobot_shared/idempotency.py, #15778). |
| `AUTOBOT_IDEMPOTENCY_TTL_SECONDS` | backend | int | `86400` | How long a completed creation stays replayable for its Idempotency-Key. Long enough to outlive any retry an agent or proxy makes, short enough that the keyspace stays bounded (autobot_shared/idempotency.py, #15778). |
...
| `AUTOBOT_PASSWORD_CHANGE_SESSION_MAX_ATTEMPTS` | backend | int | `5` | Password-change attempts allowed per CLIENT on the session auth surface before a 429 (autobot-backend/api/auth.py). Deliberately looser than the targeted limiter below: this one keys on the caller's client id and guards a self-service form, where a legitimate user mistyping their current password is the common case (#15757). |
| `AUTOBOT_PASSWORD_CHANGE_SESSION_WINDOW_SECONDS` | backend | int | `300` | Sliding window for AUTOBOT_PASSWORD_CHANGE_SESSION_MAX_ATTEMPTS, in seconds (#15757). |
| `AUTOBOT_PASSWORD_CHANGE_TARGETED_MAX_ATTEMPTS` | backend | int | `3` | Password-change attempts allowed per TARGET USER, and per calling actor when the actor differs from the target, on the user-management surface (autobot-backend/user_management/middleware/rate_limit.py). Stricter than the session limiter because this path includes admin reset of another user's password, where repeated attempts against one victim -- or one caller walking many target ids -- is the threat rather than a typo (#15743, #15757). |
| `AUTOBOT_PASSWORD_CHANGE_TARGETED_WINDOW_SECONDS` | backend | int | `1800` | Sliding window for AUTOBOT_PASSWORD_CHANGE_TARGETED_MAX_ATTEMPTS, in seconds. Six times the session window: a slower, stricter limit for the higher-value surface (#15757). |
...
| `AUTOBOT_STEWARDSHIP_MAX_HANDOFFS` | orchestration | int | `3` | How many times one branch may change stewards before it must land (#15987). An unbounded chain recreates, inside a single branch, the pile of unlanded work the worktree ceiling exists to prevent — and it is less visible there, because one growing branch looks like progress while four stalled ones look like a queue. Raising it buys more accretion, not more throughput. Range: 1–20. |
...
| `AUTOBOT_VNC_PORT` | network | int | `6080` | Port of the VNC web interface (novnc), used by the desktop-control/VNC proxy surface. |
...
| `AUTOBOT_WORKSPACE_BASE_REF` | orchestration | str | `'origin/main'` | Ref that new task workspaces branch from (#15938). Distinct from AUTOBOT_GIT_BRANCH, which records the branch the running instance was built from: one describes this deployment, the other where new work starts. |
...
| `AUTOBOT_WORK_CLAIM_STALL_INTERVALS` | orchestration | int | `3` | How many renewal intervals a run may go without reporting progress before its work claims stop being renewed and lapse (#15950). Progress is reported by every LLM attempt, tool SDK executions and write sites, so a slow run that keeps working keeps its claim and only silence lets one lapse. The effective window is never shorter than twice AUTOBOT_LLM_REQUEST_TIMEOUT plus the longest LLM backoff wait and rate-limit token wait, the longest a working call can go before an attempt ends. Range: 2–100. |
| `AUTOBOT_WORK_CLAIM_TTL_S` | orchestration | int | `300` | How long an agent's work claim survives without a renew (#15947). This is a coordination policy, not a tuning knob: the value is how long a crashed agent's scope stays blocked to every other agent, so the floor stops a claim expiring mid-write and the ceiling bounds a stale hold at one hour. A running task renews for its own lifetime, so raising this does not make long tasks safer -- it only makes a dead holder's scope take longer to free. Range: 10–3600. |
...
*232 variables registered as of last generation.*
