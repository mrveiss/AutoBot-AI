...
| `AUTOBOT_WORK_CLAIM_STALL_INTERVALS` | orchestration | int | `3` | How many renewal intervals a run may go without reporting progress before its work claims stop being renewed and lapse (#15950). Progress is reported by LLM completions, tool SDK executions and write sites, so a slow run that keeps working keeps its claim and only silence lets one lapse. The effective window is never shorter than twice AUTOBOT_LLM_REQUEST_TIMEOUT, because one model call reports progress only when it finishes. Range: 2–100. |
