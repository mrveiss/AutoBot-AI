...
This guide documents the migration from scattered `logging.basicConfig()` calls to the centralized `LoggingManager` system in `autobot-backend/utils/logging_manager.py`.
...
1. ✅ **Single Configuration Point** - `autobot-backend/utils/logging_manager.py`
...
- ✅ `autobot-backend/agents/research_agent.py`
- ✅ `autobot-backend/utils/system_context.py`
...
|----------|----------|-----------------|
| `backend` | Backend API, services, utilities | `autobot-backend/api/*.py`, `autobot-backend/utils/*.py` |
...
- **Centralized Logging Implementation:** `autobot-backend/utils/logging_manager.py`
