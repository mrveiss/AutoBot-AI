...
### 1. VNC WebSocket Proxy (`autobot-backend/api/vnc_proxy.py`)
...
### 2. VNC MCP Bridge (`autobot-backend/api/vnc_mcp.py`)
...
### 3. Frontend Integration (`autobot-frontend/src/config/AppConfig.js`)
...
2. **AutoBot's MCP** (`autobot-backend/api/*_mcp.py`) - Tools that AutoBot's LLM agents use
...
- `autobot-backend/api/knowledge_mcp.py` - Similar MCP bridge example
- `src/langchain_agent_orchestrator.py` - Where to integrate VNC MCP tools

**Files Modified**:
- `autobot-backend/api/vnc_proxy.py` - WebSocket proxy with observation recording
- `autobot-backend/api/vnc_mcp.py` - MCP bridge for agent access (NEW)
- `backend/app_factory.py` - Router registration
- `autobot-frontend/src/config/AppConfig.js` - Fixed duplicate path, routes via backend
