...
AutoBot Phase 5 implements defense-in-depth security across its distributed, role-based architecture — Docker, a single VM, or however many machines a deployment scales to — with specialized security controls for multi-modal AI processing, system automation, and sensitive data handling.
...
# NPU worker role (<npu-ip>) - Internal only
iptables -A INPUT -s <backend-ip> -p tcp --dport 8081 -j ACCEPT  # Backend → NPU

# Database role - Redis Stack (<database-ip>) - Internal only
...
# AI/ML role - AI Stack (<aiml-ip>) - Internal only
iptables -A INPUT -s <backend-ip> -p tcp --dport 8080 -j ACCEPT  # Backend → AI Stack

# Browser role (<browser-ip>) - Internal only
...
class CertificateManager:
    ...
    async def ensure_certificates(self):
        ...
        domains = [
            "autobot-frontend.yourdomain.com",     # Frontend role
