...
# 5. Full test suite
pytest tests/ -v --cov=autobot-backend/api/files --cov=src/auth_middleware
```
...
cp autobot-backend/api/files.py autobot-backend/api/files.py.backup.$(date +%Y%m%d)
cp autobot-frontend/autobot-backend/utils/ApiClient.ts autobot-frontend/autobot-backend/utils/ApiClient.ts.backup.$(date +%Y%m%d)
...
# Create file: autobot-frontend/src/services/AuthTokenService.ts
# (Use code from section 3.2)

# Step 4.2: Update ApiClient
# Modify: autobot-frontend/autobot-backend/utils/ApiClient.ts
# (Use code from section 3.3)

# Step 4.3: Create/Update Login component
# Create: autobot-frontend/src/components/auth/LoginForm.vue
...
grep -r "check_file_permissions" autobot-backend/api/files.py
