# Research: provider quota headroom and multi-account failover

**Subject:** the multi-provider AI quota-manager pattern, as implemented by an external
reference implementation (a macOS menu-bar app, MIT, Swift 6 / SwiftUI, macOS 14+, ~8 months
old, single maintainer, no automated test suite). The product and vendor are deliberately not
named here; what is analysed is the pattern, and file citations are to the reference
implementation's own source tree.
...
## Source Analysis: the reference implementation

### What It Is

The reference implementation is a native macOS menu-bar GUI that fronts a separate
third-party Go proxy (named in `Services/Proxy/CLIProxyManager.swift:13`) that multiplexes many AI provider accounts behind one local
OpenAI/Anthropic-compatible endpoint. The GUI itself does **not** route traffic. It does four
...
Maturity: ~8 months old, several thousand stars, ~130 open issues, actively pushed.
Single-maintainer, no automated test suite (`AGENTS.md` states this
outright; its test target holds 17 files but the doc says none is a real suite). ~217 source
...
- **Wrapper-over-daemon.** GUI (Swift) + a downloaded Go proxy binary. The GUI talks to it
...
- **The advertised failover isn't in the GUI at all.** "Round Robin / Fill First" belongs to the
  Go proxy; the GUI writes `RoutingConfig.strategy` and `QuotaExceededConfig
...
| Second-project coupling | Real routing lives in the Go proxy. Adopting failover = adopting a Go daemon this analysis has not reviewed. |
...
## AutoBot Comparison: reference implementation → AutoBot
...
mark the provider degraded, fall to the next model. The reference implementation's contribution is **proactive and
...
- **The hook already exists and is unbuilt:** [`llc/api/costs.py:209-244`](autobot-backend/llc/api/costs.py#L209-L244)
  ...
  `["5h_output_tokens", "7d_output_tokens"]` for Anthropic — the exact windows the reference implementation reads. It
...
undocumented-endpoint scraping that makes the reference implementation work is the single largest hidden cost in the
...
server platform; the hidden cost scales with AutoBot's deployment model, not the reference implementation's.
...
The honest summary: AutoBot's *plumbing* is better everywhere. What the reference implementation has is a **signal**
...
| 7 | No hardware/KMS-rooted vault key | Low-Medium | Significant | Different shape than the reference implementation's; own issue |
