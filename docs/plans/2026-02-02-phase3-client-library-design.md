...
Add to `autobot-frontend/src/config/ssot-config.ts`:
...
- 29 agents defined in `autobot-backend/api/agent_config.py`
...
2. **Update Backend API**: Modify `autobot-backend/api/agent_config.py`
...
- [ ] Update `autobot-backend/api/agent_config.py` to use SLM
...
| `autobot-backend/api/agent_config.py` | Use SLM client instead of unified_config_manager |
| `autobot-frontend/src/config/ssot-config.ts` | Add `discoverService()` |
| `autobot-frontend/src/composables/useServiceDiscovery.ts` | New: Vue composable |
