...
1. **Implementation Complete**
   - Bridge file created: `autobot-backend/api/{bridge_name}_mcp.py`
...
3. **MCP Registry Entry**
   - Bridge added to `MCP_BRIDGES` list in `autobot-backend/api/mcp_registry.py`
   - Entry format:
     ```python
     (
         "{bridge_name}_mcp",
...
ls -la autobot-backend/api/{bridge_name}_mcp.py
...
grep -n "{bridge_name}_mcp" autobot-backend/api/mcp_registry.py
```
...
2. **Missing MCP Registry Entry**
   ...
   grep '"{bridge_name}_mcp"' autobot-backend/api/mcp_registry.py
   ```
...
- **MCP Registry API:** `autobot-backend/api/mcp_registry.py`
