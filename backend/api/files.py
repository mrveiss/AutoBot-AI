...
ALLOWED_EXTENSIONS = {
    '.txt', '.md', '.json', '.yaml', '.yml', '.py', '.js', '.ts', '.html', '.css',
    '.xml', '.csv', '.log', '.conf', '.cfg', '.ini', '.sh', '.bat', '.sql',
...
async def upload_file(
    ...
    try:
        ...
        if target_file.exists() and not overwrite:
            raise HTTPException(status_code=409, detail="File already exists. Use overwrite=true to replace it.")
        ...
        # Log the upload
        security_layer = get_security_layer(request)
        security_layer.audit_log("file_upload", "system", "success", {
...
async def download_file(request: Request, path: str):
    ...
    try:
        ...
        # Log the download
        security_layer = get_security_layer(request)
        security_layer.audit_log("file_download", "system", "success", {
            "path": path,
...
async def delete_file(request: Request, file_operation: FileOperation):
    ...
    try:
        ...
        if target_path.is_file():
            ...
            target_path.unlink()
            security_layer.audit_log("file_delete", "system", "success", {
...
async def create_directory(request: Request, path: str = Form(...), name: str = Form(...)):
    ...
    """
    if not check_file_permissions(request, "upload"):
        raise HTTPException(status_code=403, detail="Insufficient permissions for directory creation")
    ...
    try:
        ...
        # Log the creation
        security_layer = get_security_layer(request)
        security_layer.audit_log("directory_create", "system", "success", {
            "path": relative_path,
            "name": name
        })
