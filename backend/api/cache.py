...
def get_redis_connection(db_number: int = 0):
    ...
    try:
        ...
        try:
            logger.warning(f"Falling back to config-based Redis connection for database {db_number}")
            return redis.Redis(
            ...
            logger.error(f"Config fallback also failed for Redis database {db_number}: {str(fallback_error)}")
            raise
    ...
    try:
        ...
        for db_name, db_number in REDIS_DATABASES.items():
            try:
                ...
                logger.warning(f"Could not get stats for Redis database {db_name} ({db_number}): {str(e)}")
                stats["redis_databases"][db_name] = {
                    "database": db_number,
                    "key_count": 0,
                    "memory_usage": "0B",
        ...
        if database == "all":
            ...
            for db_name, db_number in REDIS_DATABASES.items():
                try:
                    ...
                    logger.info(f"Cleared Redis database {db_name} ({db_number}) - {keys_before} keys removed")
            ...
            logger.info(f"Cleared Redis database {database} ({db_number}) - {keys_before} keys removed")
...
async def save_cache_config(config_data: Dict[str, Any]):
    ...
    try:
        ...
        required_fields = ["defaultTTLMinutes", "settingsTTLMinutes", "autoCleanupEnabled", "maxCacheSizeMB"]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error saving cache configuration: {str(e)}"
        )
...
async def get_cache_config():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting cache configuration: {str(e)}"
        )
...
async def warmup_caches():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error during cache warmup: {str(e)}"
        )