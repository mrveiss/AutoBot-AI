...
class WorkflowAutomationManager:
    ...
    def _extract_command_from_step(self, step) -> str:
        ...
        if "update" in action and "package" in action:
            ...
            return "find . -name '*' -type f"  # Generic search
        else:
    ...
    async def start_workflow_execution(self, workflow_id: str) -> bool:
        ...
        await self._send_workflow_message(workflow.session_id, {
            "type": "start_workflow",
            "workflow": {
                "id": workflow_id,
                "name": workflow.name,
    ...
    async def _send_step_confirmation_request(self, workflow_id: str, step: WorkflowStep):
        """Send step confirmation request to frontend"""
        workflow = self.active_workflows[workflow_id]
    ...
    async def _approve_and_execute_step(self, workflow_id: str, step_id: str):
        ...
        try:
            ...
            result = await self._execute_workflow_command(workflow.session_id, current_step.command)
    ...
    async def _execute_workflow_command(self, session_id: str, command: str) -> Dict[str, Any]:
    ...
    async def _check_step_dependencies(self, workflow_id: str, step: WorkflowStep) -> bool:
    ...
    def _create_system_update_workflow(self, session_id: str) -> List[WorkflowStep]:
        ...
        return [
            WorkflowStep("update_repos", "sudo apt update", "Update package repositories"),
            WorkflowStep("upgrade_packages", "sudo apt upgrade -y", "Upgrade installed packages"),
            WorkflowStep("autoremove", "sudo apt autoremove -y", "Remove unnecessary packages"),
            WorkflowStep("verify", "apt list --upgradable", "Check for remaining updates")
        ]
    ...
    def _create_dev_environment_workflow(self, session_id: str) -> List[WorkflowStep]:
        ...
        return [
            ...
            WorkflowStep("install_git", "sudo apt install -y git", "Install Git version control"),
            WorkflowStep("install_nodejs", "curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash -", "Setup Node.js repository"),
            WorkflowStep("install_nodejs_pkg", "sudo apt install -y nodejs", "Install Node.js and npm"),
            WorkflowStep("install_python", "sudo apt install -y python3 python3-pip", "Install Python 3 and pip"),
            WorkflowStep("verify_installs", "git --version && node --version && python3 --version", "Verify installations")
        ]
...
async def create_workflow_from_chat(request: dict):
    ...
    try:
        ...
        workflow_id = await workflow_manager.create_workflow_from_chat_request(user_request, session_id)
        ...
        if workflow_id:
            ...
            return {
                "success": True,
                "workflow_id": workflow_id,
                "message": f"Workflow created and started from chat request"
            }
        else:
            return {
                "success": False,
