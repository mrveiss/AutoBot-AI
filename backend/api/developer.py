...
async def enhanced_404_handler(request: Request, exc: HTTPException):
    ...
    if not similar_endpoints:
        response_content["developer_info"]["suggestion"] = f"Check available endpoints at /api/developer/endpoints"
...
async def enhanced_405_handler(request: Request, exc: HTTPException):
    ...
    response_content = {
        ...
        "developer_info": {
            ...
            "suggestion": f"Try one of: {', '.join(allowed_methods)}" if allowed_methods else "No methods available for this path"
        }
    }
