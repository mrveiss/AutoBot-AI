...
async def authorize_elevation(auth: ElevationAuthorization):
    ...
    try:
        ...
        session_data = {
            ...
            "expires": datetime.now() + timedelta(minutes=15 if auth.remember_session else 5),
            "request_id": request_id
        }
        ...
        return {
            ...
            "expires_in": int((session_data["expires"] - datetime.now()).total_seconds()),
...
async def get_pending_requests():
    ...
    active_requests = {
        req_id: req_data for req_id, req_data in pending_requests.items()
