...
class AsyncRedisDatabase:
    ...
    def __init__(
        self, name: str, config: DatabaseConfig, host: str = None, port: int = None
    ):
        self.name = name
        self.config = config
        ...
        self.port = (
            port
            if port is not None
            else int(os.getenv("AUTOBOT_REDIS_PORT", str(NetworkConstants.REDIS_PORT)))
        )
        ...
        async with self._lock:
            ...
            try:
                ...
                await asyncio.wait_for(
                    self._redis.ping(), timeout=self.config.timeout_seconds
                )
                ...
                logger.info(
                    f"Redis database '{self.name}' (DB {self.config.db}) initialized successfully"
                )
                return True
    ...
    def _record_failure(self):
        ...
        if self._failure_count >= self.config.circuit_breaker_threshold:
            ...
            self._state = ConnectionState.FAILED
            logger.warning(
                f"Circuit breaker opened for Redis database '{self.name}' after {self._failure_count} failures"
            )
    ...
    async def _execute_with_retry(self, operation, *args, **kwargs):
        ...
        if self._is_circuit_open():
            raise ConnectionError(
                f"Circuit breaker is open for Redis database '{self.name}'"
            )
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                if attempt < self.config.retry_attempts - 1:
                    ...
                    logger.warning(
                        f"Redis operation failed (attempt {attempt + 1}/{self.config.retry_attempts}) for '{self.name}': {e}"
                    )
...
class AsyncRedisManager:
    ...
    def __init__(
        self,
        config_file: str = "config/redis-databases.yaml",
        host: str = None,
        port: int = None,
    ):
        ...
        try:
            ...
            for name, db_config in databases_config.items():
                ...
                config = DatabaseConfig(
                    ...
                    circuit_breaker_threshold=db_config.get(
                        "circuit_breaker_threshold", 5
                    ),
                    circuit_breaker_timeout=db_config.get(
                        "circuit_breaker_timeout", 60
                    ),
                )

                self._databases[name] = AsyncRedisDatabase(
                    name, config, self.host, self.port
                )
                logger.info(
                    f"Configured Redis database '{name}' (DB {config.db}): {config.description}"
                )
    ...
    def _create_default_config(self, config_path: Path):
        ...
        default_config = {
            "redis_databases": {
                ...
                "sessions": {
                    "db": 2,
                    "description": "User sessions and temporary data",
                },
                "vectors": {
                    "db": 8,
                    "description": "Vector embeddings and search indexes",
                },
                "cache": {"db": 3, "description": "Application cache"},
                "metrics": {"db": 4, "description": "System metrics and monitoring"},
                "workflows": {
                    "db": 7,
                    "description": "Workflow coordination and state",
                },
    ...
    async def initialize_all(self) -> Dict[str, bool]:
        ...
        logger.info(
            f"Redis initialization complete: {successful}/{len(results)} databases ready"
        )
        ...
        if _redis_manager_instance is None:
            ...
            try:
                ...
                if successful == 0:
                    ...
                    logger.info(
                        f"Redis manager ready with {successful}/{len(results)} databases"
                    )
...
if __name__ == "__main__":
    ...
    async def test_redis_manager():
        ...
        try:
            ...
            print(
                f"Overall health: {'✅ Healthy' if health['overall_healthy'] else '❌ Degraded'}"
            )
            ...
            print(
                f"Healthy databases: {stats.healthy_databases}/{stats.total_databases}"
            )
