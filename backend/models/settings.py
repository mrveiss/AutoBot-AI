...
from src.constants.model_constants import Models
...
class LLMSettings(BaseSettings):
    ...
    task_llm: str = Field(
        default="ollama", description="LLM provider for tasks")
    ...
    openai_model: str = Field(
        default="gpt-3.5-turbo", description="OpenAI model")
...
class RedisSettings(BaseSettings):
    ...
    enabled: bool = Field(
        default=True,
        description="Enable Redis for caching and task transport")
...
class DataSettings(BaseSettings):
    ...
    base_directory: str = Field(
        default="data", description="Base data directory")
    chat_history_file: str = Field(
        default="data/chat_history.json",
        description="Chat history file path")
    chats_directory: str = Field(
        default="data/chats",
        description="Individual chat files directory")
    long_term_db_path: str = Field(
        default="data/agent_memory.db",
        description="Long-term memory database path")
    reliability_stats_file: str = Field(
        default="data/reliability_stats.json",
        description="Reliability statistics file")
    knowledge_base_db: str = Field(
        default="data/knowledge_base.db",
        description="Knowledge base SQLite database")
...
class BackendSettings(BaseSettings):
    ...
    cors_origins: List[str] = Field(
        default=["http://localhost:{NetworkConstants.AI_STACK_PORT}", ServiceURLs.FRONTEND_LOCAL],
...
class OrchestratorSettings(BaseSettings):
    ...
    use_langchain: bool = Field(
        default=False, description="Use LangChain orchestrator")
    task_transport: str = Field(
        default="local",
        description="Task transport mechanism (local/redis)")
        ...
        return {
            ...
            'memory': {
                ...
                'max_entries_per_category': (
                    self.memory.max_entries_per_category),
            ...
            'diagnostics': {
                ...
                'use_llm_for_analysis': (
                    self.diagnostics.use_llm_for_analysis),
            ...
            'orchestrator': {
                ...
                'max_concurrent_tasks': (
                    self.orchestrator.max_concurrent_tasks)
            }
        }
...
def yaml_config_settings_source(settings: BaseSettings) -> Metadata:
    ...
    try:
        ...
        logger.warning(
            "Could not load YAML config from %s: %s", config_file, e)
        return {}
