            ...
            if agent.error_rate > 20:
                recommendations.append(
                    MaintenanceRecommendation(
                        ...
                        metadata={"error_rate": agent.error_rate, "agent_type": agent.agent_type},
                    )
                )
                ...
                if timeout_rate > 5:
                    recommendations.append(
                        MaintenanceRecommendation(
                            ...
                            metadata={"timeout_tasks": agent.timeout_tasks, "timeout_rate": timeout_rate},
                        )
                    )
        ...
        if growth_rate > 20:
            recommendations.append(
                MaintenanceRecommendation(
                    ...
                    estimated_impact=f"Projected cost increase of ${trends.get('total_cost_usd', 0) * growth_rate / 100:.2f}",
                ...
                if memory_pct > 80:
                    recommendations.append(
                        MaintenanceRecommendation(
                            ...
                            confidence=0.9,
                            recommended_action="Clean up old data, increase memory allocation, review retention policies",
                            estimated_impact="May affect caching and real-time features",
                            metadata={"used_memory_mb": used_memory / 1024 / 1024, "memory_percent": memory_pct},
                        )
                    )
            ...
            if agent.avg_duration_ms > 5000 and agent.total_tasks > 20:
                recommendations.append(
                    ResourceOptimization(
                        ...
                        details=f"Reducing {agent.avg_duration_ms}ms to 2000ms would save {(agent.avg_duration_ms - 2000) * agent.total_tasks / 1000}s",
                    )
                )
