...
class BackgroundVectorizer:
    ...
    async def vectorize_pending_facts(self, kb):
        ...
        try:
            ...
            logger.info(f"Background vectorization complete: {success_count} vectorized, {skipped_count} skipped, {failed_count} failed")
    ...
    async def periodic_check(self, kb):
        ...
        while True:
            try:
                ...
                if self.last_run and (datetime.now() - self.last_run).seconds < self.check_interval:
                    continue
