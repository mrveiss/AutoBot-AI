...
async def create_new_chat():
    try:
        return await chat_api.create_new_chat()
    except Exception as e:
...
async def get_chat_list():
    try:
        return await chat_api.get_chat_list()
    except Exception as e:
        logger.error(f"Error getting chat list: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error getting chat list: {str(e)}")

@app.post("/api/chats/{chat_id}")
async def save_chat_messages(chat_id: str, messages: list):
    try:
        return await chat_api.save_chat_messages(chat_id, messages)
    except Exception as e:
...
async def get_chat_messages(chat_id: str):
    try:
        return await chat_api.get_chat_messages(chat_id)
    except Exception as e:
...
async def reset_chat(chat_id: str):
    try:
        return await chat_api.reset_chat(chat_id)
    except Exception as e:
...
async def health_check():
    ...
    try:
        ...
        return {
            "status": "healthy", 
            "backend": "connected",
            "llm_status": "connected" if ollama_healthy else "disconnected",
            "ollama": "connected" if ollama_healthy else "disconnected",  # Keep for backward compatibility
        ...
        return JSONResponse(
            ...
            content={
                ...
                "backend": "connected",
                "llm_status": "unknown",
                "ollama": "unknown",  # Keep for backward compatibility
