...
class ChatAPI:
    ...
    async def get_chat_messages(self, chat_id: str) -> List[Dict]:
        ...
        try:
            ...
            if chat_file.exists():
                ...
                logger.info(f"Chat file {chat_file} does not exist, returning empty list")
                return []
        except Exception as e:
            logger.error(f"Error loading chat messages for {chat_id}: {str(e)}")
            return []
    
    async def save_chat_messages(self, chat_id: str, messages: List[Dict]) -> Dict[str, str]:
        ...
        try:
            ...
            for i, msg in enumerate(messages):
                try:
                    ...
                    logger.warning(f"Skipping invalid message at index {i}: {str(msg_error)}")
                    continue
            ...
            logger.info(f"Saved {len(cleaned_messages)} cleaned messages for chat {chat_id}")
        ...
        for session_dir in self.messages_dir.iterdir():
            if session_dir.is_dir():
                try:
                    ...
                    logger.warning(f"Failed to cleanup message directory {session_dir}: {cleanup_error}")
    
    async def cleanup_all_message_files(self) -> Dict[str, Any]:
        """Clean up all leftover message files"""
        try:
            if not self.messages_dir.exists():
                return {"status": "success", "message": "No message directory to clean up", "cleaned_count": 0}
            ...
            for session_dir in self.messages_dir.iterdir():
                if session_dir.is_dir():
                    try:
                        ...
                        logger.error(f"Failed to cleanup message directory {session_dir}: {cleanup_error}")
            ...
            try:
                if self.messages_dir.exists() and not any(self.messages_dir.iterdir()):
                    ...
                    logger.info(f"Removed empty messages directory: {self.messages_dir}")
            except Exception as e:
            ...
            raise HTTPException(status_code=500, detail=f"Error cleaning up message files: {str(e)}")
    ...
    async def cleanup_all_chat_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.chat_data_dir.exists():
                for chat_file in self.chat_data_dir.glob("chat_*.json"):
                    try:
                        ...
                        logger.error(f"Failed to delete chat file {chat_file.name}: {e}")
            ...
            raise HTTPException(status_code=500, detail=f"Error in comprehensive cleanup: {str(e)}")
