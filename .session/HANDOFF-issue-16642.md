...
- Post-hoc audit (end-of-session checklist) found and filed three more, none blocking: #16687 (`AccessLevel.SYSTEM`/`ScopeLevel.SYSTEM` naming collision — the actual root cause of round-3's review confusion; related to closed #11290 which didn't cover `AccessLevel`), #16688 (four parallel secret/PII redaction modules with no canonical owner — `secret_redaction.py`/`pii_pipeline.py`/`chat_message_safety.py`/`credential_redaction.py`), #16690 (pii_pipeline's 5ms p99 perf budget flaked once after the round-2 detector widening, passed on retry — margin worth re-measuring)
- Posted a full AC-by-AC status comment on #16642 itself — it had zero comments despite three PR review rounds; the issue was invisible from that side
