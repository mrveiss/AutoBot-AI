...
class TestUnifiedLLMInterface:
    ...
    def __init__(self):
        ...
        self.test_messages = {
            ...
            "extraction": [
                ...
                {"role": "user", "content": "AutoBot is an AI automation platform built with Python."}
            ],
            "classification": [
                {"role": "system", "content": "Classify the sentiment of this text as POSITIVE, NEGATIVE, or NEUTRAL."},
    ...
    async def test_llm_type_configurations(self):
        ...
        for llm_type in llm_types_to_test:
            try:
                ...
                print(f"  ✓ {llm_type.value}: {len(response.content)} chars, {response.response_time:.3f}s")
        ...
        assert len(responses) >= len(llm_types_to_test) // 2, "Should support most LLM types"
    ...
    async def test_provider_selection_and_fallback(self):
        ...
        assert response_mock.provider == ProviderType.MOCK, "Should use requested provider"
    ...
    async def test_parameter_handling(self):
        ...
        response_structured = await self.interface.chat_completion(
            messages=self.test_messages["extraction"],
    ...
    async def test_monitoring_and_statistics(self):
        ...
        for provider, status in health["providers"].items():
            print(f"    {provider}: available={status['available']}, enabled={status['enabled']}")
    ...
    async def test_concurrent_requests(self):
        ...
        successful = sum(1 for r in responses if isinstance(r, LLMResponse) and not r.error)
        total_time = sum(r.response_time for r in responses if isinstance(r, LLMResponse))
        avg_time = total_time / len([r for r in responses if isinstance(r, LLMResponse)]) if responses else 0
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            provider_results = await self.test_provider_selection_and_fallback()
            
            # Test message types
            message_results = await self.test_different_message_types()
            
            # Test parameter handling
            param_results = await self.test_parameter_handling()
            
            # Test error handling
            error_result = await self.test_error_handling()
            
            # Test backward compatibility
            compat_results = await self.test_backward_compatibility()
            
            # Test monitoring and statistics
            monitoring_results = await self.test_monitoring_and_statistics()
            
            # Test concurrent requests
            concurrent_responses = await self.test_concurrent_requests()
            
            # Test provider isolation
            isolation_results = await self.test_provider_isolation()
