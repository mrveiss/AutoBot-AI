...
class FileUploadAPITester:
    ...
    def _test_upload_endpoint_availability(self):
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                self._add_result("Upload Endpoint Available", "PASS", 
                               "API endpoint accepts OPTIONS requests")
            else:
                self._add_result("Upload Endpoint Available", "FAIL", 
                               f"OPTIONS returned {response.status_code}")
        except requests.ConnectionError:
            self._add_result("Upload Endpoint Available", "SKIP", 
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('success'):
                    self._add_result("Valid File Upload", "PASS", 
                                   f"File uploaded successfully: {result.get('filename')}")
                else:
                    self._add_result("Valid File Upload", "FAIL", 
    ...
    def _test_invalid_file_types(self):
        ...
        if not self._is_backend_available():
            self._add_result("Invalid File Type Rejection", "SKIP", "Backend not available")
            return
        ...
        try:
            ...
            if response.status_code == 400:
                self._add_result("Invalid File Type Rejection", "PASS", 
                               "Executable file properly rejected")
            elif response.status_code == 200:
                self._add_result("Invalid File Type Rejection", "FAIL", 
                               "Executable file was accepted - SECURITY RISK!")
            else:
                self._add_result("Invalid File Type Rejection", "PARTIAL", 
    ...
    def _test_file_size_limits(self):
        ...
        try:
            ...
            if response.status_code == 413:
                self._add_result("File Size Limit", "PASS", 
                ...
                if "too large" in result.get('message', '').lower():
                    ...
                    self._add_result("File Size Limit", "PARTIAL", 
                                   f"File rejected but unclear reason: {result.get('message')}")
            elif response.status_code == 200:
                self._add_result("File Size Limit", "INFO", 
    ...
    def _test_security_validation(self):
        ...
        try:
            ...
            if response.status_code == 400:
                self._add_result("Security Validation", "PASS", 
                ...
                if "passwd" not in result.get('filename', '').lower():
                    ...
                    self._add_result("Security Validation", "FAIL", 
                                   "Path traversal filename not sanitized - SECURITY RISK!")
            else:
                self._add_result("Security Validation", "PARTIAL", 
                               f"Unexpected response: {response.status_code}")
...
if __name__ == "__main__":
    ...
    results_file = Path(__file__).parent.parent / "results" / "file_upload_api_results.json"
