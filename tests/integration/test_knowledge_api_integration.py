...
def sample_knowledge_data():
    ...
    return {
        ...
        "facts": {
            "tools": [
                {"id": "t1", "content": "nmap network scanner", "category": "tools", "vectorized": True},
                {"id": "t2", "content": "wireshark packet analyzer", "category": "tools", "vectorized": False},
            ],
            "security": [
                {"id": "s1", "content": "Use strong passwords", "category": "security", "vectorized": True},
                {"id": "s2", "content": "Enable 2FA authentication", "category": "security", "vectorized": True},
            ]
        }
    }
...
class TestVectorizationStatusFlow:
    ...
    async def test_vectorization_status_badge_update(self, api_client):
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            async with api_client.post(
                '/api/knowledge_base/vectorization_status',
    ...
    async def test_real_time_status_updates(self, api_client):
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            mock_response.json = AsyncMock(return_value={
                "statuses": {
                    fact_id: {"vectorized": False, "timestamp": None}
                }
            })
            mock_post.return_value.__aenter__.return_value = mock_response
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            mock_response.json = AsyncMock(return_value={
                "statuses": {
                    fact_id: {"vectorized": True, "timestamp": "2025-11-28T10:10:00"}
                }
            })
            mock_post.return_value.__aenter__.return_value = mock_response
    ...
    async def test_partial_retry_success(self, api_client):
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            mock_response.json = AsyncMock(return_value={
                ...
                "failed_facts": [
                    {"id": "fact_3", "error": "Persistent error"}
                ]
            })
            mock_post.return_value.__aenter__.return_value = mock_response
    ...
    async def test_concurrent_api_requests(self, api_client):
        ...
        with patch('aiohttp.ClientSession.get') as mock_get:
            ...
            responses = await asyncio.gather(*[
                task.__aenter__() for task in tasks
            ])
