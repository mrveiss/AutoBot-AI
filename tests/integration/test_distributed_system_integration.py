            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        ...
                        return (
                            False,
                            f"Backend status check failed: {response.status}",
                            response_time,
                        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        return (
                            True,
                            "Backend-Ollama communication successful",
                            response_time,
                        )
                    else:
                        return (
                            False,
                            f"LLM status check failed: {response.status}",
                            response_time,
                        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(ai_url) as response:
                    ...
                    if response.status == 200:
                        return (
                            True,
                            "Backend-AI Stack communication successful",
                            response_time,
                        )
                    else:
                        return (
                            False,
                            f"AI Stack health check failed: {response.status}",
                            response_time,
                        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        return (
                            True,
                            f"{source}-{target} communication successful",
                            response_time,
                        )
                    else:
                        return (
                            False,
                            f"{target} returned status {response.status}",
                            response_time,
                        )
...
async def test_network_partition_recovery():
    ...
    assert (
        partition_result.recovery_time < 30
    ), f"Partition recovery took {partition_result.recovery_time:.1f}s, expected < 30s"
