...
async def test_security_endpoints():
    ...
    try:
        ...
        try:
            response = requests.get('http://localhost:8001/api/security/status', timeout=5)
            ...
            if response.status_code == 200:
                ...
                print(f"   - Command security enabled: {data.get('command_security_enabled')}")
                print(f"   - Docker sandbox enabled: {data.get('docker_sandbox_enabled')}")
        ...
        try:
            response = requests.get('http://localhost:8001/api/security/command-history', timeout=5)
            print(f"   Status code: {response.status_code}")
        ...
        try:
            response = requests.get('http://localhost:8001/api/security/pending-approvals', timeout=5)
            print(f"   Status code: {response.status_code}")
        ...
        try:
            response = requests.get('http://localhost:8001/api/security/audit-log', timeout=5)
            print(f"   Status code: {response.status_code}")
