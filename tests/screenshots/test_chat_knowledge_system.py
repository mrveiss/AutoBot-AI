...
class ChatKnowledgeSystemTester:
    ...
    async def test_chat_context_creation(self):
        ...
        async with self.session.post(
            f"{BASE_URL}/api/chat_knowledge/context/create",
            json=context_data
        ) as response:
            if response.status == 200:
                data = await response.json()
...
""")
    ...
    async def test_chat_search(self):
        ...
        for query in search_queries:
            ...
            async with self.session.post(
                f"{BASE_URL}/api/chat_knowledge/search",
                json=search_data
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def test_chat_compilation(self):
        ...
        for message in chat_messages:
            ...
            async with self.session.post(
                f"{BASE_URL}/api/chats/{self.test_chat_id}/message",
                json=message_data
            ) as response:
                if response.status == 200:
        ...
        async with self.session.post(
            f"{BASE_URL}/api/chat_knowledge/compile",
            json=compile_data
        ) as response:
            if response.status == 200:
                data = await response.json()
