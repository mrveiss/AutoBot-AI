...
from backend.api.security import router, CommandApprovalRequest
...
class TestSecurityAPI:
    ...
    def test_get_security_status_on_demand_initialization(self):
        ...
        with patch('backend.api.security.EnhancedSecurityLayer') as MockSecurityLayer:
...
class TestSecurityAPIIntegration:
    ...
    def test_api_error_consistency(self):
        ...
        with patch('backend.api.security.EnhancedSecurityLayer', side_effect=Exception("Init error")):
            response = self.client.get("/api/security/status")
            assert response.status_code == 500
