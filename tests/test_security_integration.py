...
class TestSecuritySystemIntegration:
    ...
    async def test_end_to_end_forbidden_command_blocking(self, security_layer):
        ...
        history = security_layer.get_command_history(limit=10)
        blocked_logged = any(
            entry.get("action") == "command_execution_attempt" and
            "rm -rf /" in entry.get("details", {}).get("command", "")
            for entry in history
        )
...
class TestTerminalSecurityIntegration:
    ...
    async def test_terminal_risk_assessment_integration(self):
        ...
        # Should log both command and risk assessment
        assert mock_security.audit_log.call_count == 2
        
        # Should send security warning
        session.websocket.send_text.assert_called()
        warning_call = session.websocket.send_text.call_args[0][0]
        warning_data = json.loads(warning_call)
        assert warning_data["type"] == "security_warning"
        assert "high risk" in warning_data["message"]
...
class TestSecuritySystemResilience:
    ...
    async def test_approval_timeout_handling(self, security_layer):
        ...
        with patch.object(security_layer, '_command_approval_callback', side_effect=asyncio.TimeoutError()):
            result = await security_layer.execute_command(
