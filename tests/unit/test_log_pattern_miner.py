...
from src.code_intelligence.log_pattern_miner import (
    Anomaly,
    AnomalyType,
    LogEntry,
    LogLevel,
...
def sample_mixed_logs():
    ...
    for i in range(5):
        ...
        lines.append(
            f"{ts} - DB - ERROR - Connection failed: timeout after 30s"
        )
...
class TestPatternExtraction:
    ...
    def test_extract_api_patterns(self, sample_standard_logs):
        ...
        api_patterns = [
            p for p in result.patterns
            if p.pattern_type == PatternType.API_USAGE
        ]
...
class TestAnomalyDetection:
    ...
    def test_detect_duration_anomalies(self):
        ...
        lines.append(
            f"{ts} - API - INFO - "
            f"API_CALL: GET /api/test DURATION=500ms STATUS=200"
        )
