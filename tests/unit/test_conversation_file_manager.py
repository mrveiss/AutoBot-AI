...
class TestDatabaseInitialization:
    ...
    async def test_idempotent_initialization(self, temp_db_manager):
        ...
        await temp_db_manager.initialize()
        version1 = await temp_db_manager._get_schema_version()
        ...
        await temp_db_manager.initialize()
        version2 = await temp_db_manager._get_schema_version()
    ...
    async def test_schema_version_tracking(self, temp_db_manager):
        ...
        version = await temp_db_manager._get_schema_version()

        # Should have version "001" from migration
        assert version == "001", f"Expected version '001', got '{version}'"

    @pytest.mark.asyncio
    async def test_get_schema_version_no_migrations(self, temp_db_manager):
        """Test _get_schema_version returns 'unknown' for uninitialized DB."""
