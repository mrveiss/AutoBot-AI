...
from src.code_intelligence.bug_predictor import (
    BugPredictor,
    FileRiskAssessment,
    PredictionResult,
    RiskFactor,
...
class TestFileRiskAssessment:
    ...
    def test_to_dict(self):
        ...
        assessment = FileRiskAssessment(
            ...
            factor_scores=[
                RiskFactorScore(
                    factor=RiskFactor.COMPLEXITY, score=80.0, weight=0.15
                )
            ],
            bug_count_history=5,
...
class TestPreventionTips:
    ...
    def test_generates_tips_for_high_scores(self, predictor):
        ...
        scores = [
            RiskFactorScore(
                factor=RiskFactor.COMPLEXITY, score=75.0, weight=0.15
            ),
            RiskFactorScore(
                factor=RiskFactor.BUG_HISTORY, score=80.0, weight=0.25
            ),
        ]
    ...
    def test_no_tips_for_low_scores(self, predictor):
        ...
        scores = [
            RiskFactorScore(
                factor=RiskFactor.COMPLEXITY, score=20.0, weight=0.15
            ),
            RiskFactorScore(
                factor=RiskFactor.BUG_HISTORY, score=30.0, weight=0.25
            ),
        ]
        tips = predictor._generate_prevention_tips(scores)
...
class TestSuggestions:
    ...
    def test_suggestions_for_high_complexity(self, predictor):
        ...
        scores = [
            RiskFactorScore(
                factor=RiskFactor.COMPLEXITY, score=70.0, weight=0.15
            ),
        ]
