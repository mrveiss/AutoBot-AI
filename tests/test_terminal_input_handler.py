...
from src.utils.terminal_input_handler import (
    TerminalInputHandler, safe_input, safe_input_async,
    mock_terminal_input, configure_testing_defaults,
    patch_builtin_input, InputTimeoutError
)
...
class TestTerminalInputHandler:
        ...
        try:
            with patch('builtins.input', side_effect=lambda x: time.sleep(2)):
                ...
                try:
                    ...
                    duration = time.time() - start_time
                    print(f"  Interactive timeout test: {duration:.3f}s (result: '{result}')")
                except InputTimeoutError:
                    duration = time.time() - start_time
                    print(f"  Interactive timeout raised exception after: {duration:.3f}s")
    ...
    def test_builtin_patch(self):
        ...
        patched_input = patch_builtin_input()
    ...
    async def test_concurrent_safety(self):
        ...
        async def async_worker(worker_id):
            return await self.handler.get_input_async(
                f"Async worker {worker_id}: ",
                default=f"async_result_{worker_id}"
            )
        
        async_results = await asyncio.gather(*[
            async_worker(i) for i in range(3)
        ])
