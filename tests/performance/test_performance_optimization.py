...
class PerformanceOptimizationTester:
    ...
    async def _test_npu_ai_performance(
        self, npu_url: str, metrics: List[PerformanceMetric], recommendations: List[str]
    ):
            ...
            for prompt in test_prompts:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        metrics.append(
                            PerformanceMetric(
                                f"npu_confidence_{len(processing_times)}",
                                confidence,
                                "score",
                                "NPU",
                            )
            ...
            metrics.extend(
                [
                    ...
                    PerformanceMetric(
                        ...
                        (successful_requests / concurrent_requests) * 100,
                        "%",
                        "CPU",
                    ),
                    PerformanceMetric(
    ...
    async def _test_thread_pool_optimization(
        ...
        try:
            ...
            for thread_count in thread_counts:
                ...
                metrics.append(
                    PerformanceMetric(
                        f"thread_pool_{thread_count}_performance",
                        performance,
                        "tasks/s",
                        "CPU",
                    )
            ...
            for endpoint in cache_endpoints:
                ...
                if response1.status_code == 200 and response2.status_code == 200:
                    try:
                        ...
                        metrics.append(
                            PerformanceMetric(
                                f"cache_effectiveness_{endpoint.replace('/', '_')}",
                                cache_effectiveness,
                                "boolean",
                                "Caching",
                            )
            ...
            for endpoint in hot_reload_endpoints:
                try:
                    ...
                    if response.status_code in [200, 204]:
                        ...
                        metrics.append(
                            PerformanceMetric(
                                f"hot_reload_time_{endpoint.replace('/', '_')}",
                                response_time,
                                "seconds",
                                "Hot Reload",
                            )
