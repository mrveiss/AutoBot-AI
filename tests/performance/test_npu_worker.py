...
Note: This is a comprehensive test suite. For startup testing, use the
simplified test_npu_worker.py in the project root.
"""
...
try:
    ...
    print(f"⚠️  Advanced NPU testing dependencies not available: {e}")
    print("💡 Using simplified testing mode...")
    ASYNC_DEPS_AVAILABLE = False

    # Fallback to simple synchronous testing
    import urllib.error
...
def simple_fallback_test():
    """Simple fallback test when async dependencies not available"""
    print("🧪 AutoBot NPU Worker Fallback Test")
    print("=" * 40)
    print("💡 Using simple HTTP test (no async dependencies)")
