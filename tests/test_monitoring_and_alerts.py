...
class MonitoringAndAlertingTester:
    ...
    def __init__(self):
        ...
        self.services = {
            "backend": {"host": "172.16.168.20", "port": 8001, "metrics_endpoint": "/api/metrics"},
            "frontend": {"host": "172.16.168.21", "port": 5173, "metrics_endpoint": "/metrics"},
            "redis": {"host": "172.16.168.23", "port": 6379, "metrics_endpoint": None},
            "ai_stack": {"host": "172.16.168.24", "port": 8080, "metrics_endpoint": "/metrics"},
            "npu_worker": {"host": "172.16.168.22", "port": 8081, "metrics_endpoint": "/metrics"},
            "ollama": {"host": "127.0.0.1", "port": 11434, "metrics_endpoint": "/api/version"},
        }
        ...
        self.critical_metrics = [
            ...
            {"name": "api_response_time", "warning": 2.0, "critical": 5.0, "unit": "seconds"},
            {"name": "error_rate", "warning": 5.0, "critical": 10.0, "unit": "%"},
            {"name": "service_availability", "warning": 99.0, "critical": 95.0, "unit": "%"},
        ]
    ...
    def log_result(self, test_name: str, alert_type: str, status: str, message: str,
                   response_time: float = 0, severity: str = "info", details: Dict = None):
        ...
        logger.info(f"{status_emoji.get(status, '?')} {severity_emoji.get(severity, '')} [{alert_type}] {test_name}: {message}")
    ...
    async def test_health_monitoring_endpoints(self):
        ...
        health_endpoints = [
            ...
            ("backend_monitoring", f"http://172.16.168.20:8001/api/monitoring/services"),
    ...
    async def _simulate_threshold_breach(self, test_config: Dict) -> bool:
        ...
        try:
            ...
            metric_config = next((m for m in self.critical_metrics if m["name"] == test_config["metric"]), None)
            if metric_config:
    ...
    async def test_performance_degradation_detection(self):
        ...
        for i in range(5):
            start_time = time.time()
            try:
                response = requests.get("http://172.16.168.20:8001/api/health", timeout=10)
                response_time = time.time() - start_time
                ...
                if accessibility:
                    try:
                        ...
                        if isinstance(data, dict):
                            ...
                            expected_keys = ["status", "health", "services", "metrics", "timestamp"]
                            found_keys = sum(1 for key in expected_keys 
    ...
    async def test_log_aggregation_and_analysis(self):
        ...
        log_sources = [
            {"name": "Backend Logs", "path": "/home/kali/Desktop/AutoBot/logs/backend.log"},
            {"name": "Frontend Logs", "path": "/home/kali/Desktop/AutoBot/logs/frontend.log"},
            {"name": "System Logs", "path": "/var/log/syslog"},
            {"name": "Docker Logs", "command": ["docker", "compose", "logs", "--tail=100"]},
        ]
            ...
            try:
                if "path" in log_source:
                    ...
                    if log_file.exists():
                        ...
                        severity = "warning"
                        log_analysis_results[log_source["name"]] = {"accessible": False, "reason": "file_not_found"}
                    ...
                    if result.returncode == 0:
                        ...
                        severity = "warning"
                        log_analysis_results[log_source["name"]] = {"accessible": False, "reason": "command_failed"}
                ...
                severity = "critical"
                log_analysis_results[log_source["name"]] = {"accessible": False, "error": str(e)}
    ...
    async def test_incident_response_automation(self):
        ...
        incident_scenarios = [
            ...
            },
            {
                ...
                "expected_actions": ["scale_resources", "alert_team", "diagnostic_capture"]
            },
            {
                "name": "Error Rate Spike Response",
                "trigger": "error_rate_high",
                "expected_actions": ["circuit_breaker", "rollback_check", "investigate_logs"]
            }
        ]
    ...
    async def _simulate_incident(self, scenario: Dict) -> bool:
        ...
        try:
            ...
            return scenario["trigger"] in ["service_down", "resource_high", "error_rate_high"]
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if isinstance(data, dict):
                            ...
                            response_indicators = ["incidents", "responses", "actions", "triggered"]
    ...
    async def generate_monitoring_report(self):
        ...
        for result in self.results:
            ...
            if category not in test_categories:
                test_categories[category] = {"total": 0, "passed": 0, "failed": 0, "warnings": 0}
