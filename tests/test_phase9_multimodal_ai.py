#!/usr/bin/env python3
"""
Test script for Phase 9: Advanced AI Integration and Multi-Modal Capabilities
Validates multi-modal processing, computer vision, voice processing, context-aware decisions, and modern AI integration
"""
...
from src.voice_processing_system import voice_processing_system, AudioInput
from src.context_aware_decision_system import context_aware_decision_system, DecisionType
from src.modern_ai_integration import modern_ai_integration, AIProvider
...
def test_api_connectivity():
    ...
    try:
        ...
        print("⚠️ Cannot connect to backend API at http://localhost:8001 (not required for Phase 9 tests)")
        return False
    except Exception as e:
        print(f"⚠️ API connectivity test failed: {e} (continuing with Phase 9 tests)")
        return False
...
async def test_multimodal_processor():
    ...
    try:
        ...
        print(f"   Combined results available: {len(result.results.get('combined_results', {}))}")
...
async def test_computer_vision_system():
    ...
    try:
        ...
        print(f"✅ Screen analysis completed")
        print(f"   Elements detected: {analysis_result.get('screen_analysis', {}).get('elements_detected', 0)}")
        print(f"   Confidence: {analysis_result.get('screen_analysis', {}).get('confidence_score', 0):.2f}")
        print(f"   Opportunities: {len(analysis_result.get('automation_opportunities', []))}")
    except Exception as e:
    ...
    try:
        ...
        if 'latest_analysis' in summary:
            ...
            print(f"   Latest: {latest.get('elements_detected', 0)} elements, confidence {latest.get('confidence', 0):.2f}")
    except Exception as e:
...
async def test_voice_processing_system():
    ...
    try:
        ...
        print(f"   Speech recognition available: {status.get('speech_recognition_available', False)}")
    ...
    try:
        ...
        if 'speech_recognition' in result:
            print(f"   Transcription: {result['speech_recognition'].get('transcription', 'N/A')}")
            print(f"   Confidence: {result['speech_recognition'].get('confidence', 0):.2f}")
    ...
    try:
        ...
        for i, cmd in enumerate(history):
            print(f"   {i+1}. {cmd.get('type', 'unknown')} - {cmd.get('intent', 'N/A')}")
    except Exception as e:
...
async def test_context_aware_decision_system():
    ...
    try:
        ...
        print(f"✅ Automation decision: {decision.chosen_action.get('action', 'unknown')}")
    ...
    try:
        ...
        print(f"✅ Escalation decision: {decision.chosen_action.get('action', 'unknown')}")
        print(f"   Confidence: {decision.confidence:.2f}")
    ...
    try:
        ...
        print(f"✅ Decision system status:")
...
async def test_modern_ai_integration():
    ...
    try:
        ...
        for provider, info in status.items():
            ...
            print(f"   {availability} {provider}: {info.get('model_name', 'N/A')} - {len(info.get('capabilities', []))} capabilities")
    except Exception as e:
    ...
    try:
        actions = await modern_ai_integration.natural_language_to_actions(
            ...
            context={"current_page": "form", "available_elements": ["submit_button", "cancel_button"]}
        )
...
async def test_integration():
    ...
    try:
        ...
        print(f"✅ Multi-modal + CV integration:")
        print(f"   Modal confidence: {modal_result.confidence:.2f}")
        print(f"   CV elements: {cv_result.get('screen_analysis', {}).get('elements_detected', 0)}")
    ...
    try:
        ...
        decision = await context_aware_decision_system.make_contextual_decision(
            DecisionType.AUTOMATION_ACTION,
            "Integrate AI analysis with decision making"
        )
        
        # Use AI to elaborate on the decision
        ai_response = await modern_ai_integration.process_with_ai(
            provider=AIProvider.LOCAL_MODEL,
            prompt=f"Elaborate on this automation decision: {decision.chosen_action.get('action', 'unknown')}",
            task_type="decision_elaboration"
        )
        
        print(f"✅ Context + AI integration:")
    ...
    try:
        ...
        print(f"✅ Full pipeline integration completed:")
        print(f"   Screen analysis confidence: {screen_analysis.get('screen_analysis', {}).get('confidence_score', 0):.2f}")
...
async def main():
    ...
    try:
        ...
        if all_passed:
            ...
            print("   ✅ Context-aware decision making with comprehensive context collection")
            print("   ✅ Modern AI model integration framework (GPT-4V, Claude-3, Gemini)")
