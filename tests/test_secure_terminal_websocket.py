...
class TestSecureTerminalSession:
            ...
            with patch('os.read') as mock_read:
                with patch('asyncio.run_coroutine_threadsafe') as mock_run_coro:
                    with patch('asyncio.get_event_loop') as mock_get_loop:
                        # First call returns data, second call returns empty (to break loop)
                        ...
                        call_args = mock_run_coro.call_args[0][0]
                        # Can't easily test the exact message here due to coroutine wrapping
    ...
    async def test_audit_command_with_risk_assessment(self):
        ...
        with patch.object(self.session, 'send_message') as mock_send:
            ...
            assert self.mock_security_layer.audit_log.call_count == 2  # Command + risk assessment
    ...
    async def test_execute_command_error(self):
        ...
        with patch.object(self.session, 'send_input', side_effect=Exception("Send failed")):
            with patch.object(self.session, 'send_message') as mock_send:
    ...
    def test_disconnect_process_kill(self):
        ...
        self.session.active = True
        self.session.process = MagicMock()
        self.session.process.wait.side_effect = Exception("Timeout")  # Simulate TimeoutExpired
        
        with patch.object(self.session.process, 'kill') as mock_kill:
            self.session.disconnect()
...
class TestSecureTerminalWebSocketHandler:
    ...
    async def test_handle_secure_terminal_websocket_success(self):
        ...
        with patch('backend.api.secure_terminal_websocket._secure_sessions', {}) as mock_sessions:
            # Should complete without raising exception
            await handle_secure_terminal_websocket(mock_websocket, session_id, mock_security_layer)
    ...
    async def test_handle_secure_terminal_websocket_existing_session(self):
        ...
        with patch('backend.api.secure_terminal_websocket._secure_sessions', 
                  {session_id: existing_session}) as mock_sessions:
    ...
    async def test_handle_secure_terminal_websocket_message_types(self):
        ...
        with patch('backend.api.secure_terminal_websocket.SecureTerminalSession') as MockSession:
