...
async def test_workflow_endpoints():
    ...
    async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if workflow_response:
                        ...
                        print(f"   🤖 Agents: {', '.join(workflow_response.get('agents_involved', []))}")
                        print(f"   ⏱️  Duration: {workflow_response.get('estimated_duration')}")
            ...
            try:
                async with session.get(f"{base_url}/workflow/{workflow_id}/status") as response:
                    if response.status == 200:
                        ...
                        print(f"   Current step: {status.get('current_step', 0) + 1}/{status.get('total_steps', 0)}")
                    else:
        ...
        try:
            async with session.post("http://localhost:8001/api/chat", json=chat_request) as response:
                if response.status == 200:
                    result = await response.json()
                    print(f"✅ Chat integration working")
                    
                    response_text = result.get('response', '')
                    if any(keyword in response_text.lower() for keyword in ['workflow', 'orchestration', 'classification', 'agents']):
...
async def demonstrate_workflow_orchestration():
    ...
    test_requests = [
        ...
        ("Find information about Python libraries", "Research request - should trigger research workflow"),
        ("How do I install Docker?", "Installation request - should plan installation steps"),
        ("find tools that would require to do network scan", "Complex request - should coordinate multiple agents")
    ]
                ...
                async with session.post(f"{base_url}/workflow/execute", json=workflow_request) as response:
                    if response.status == 200:
                        ...
                        if result.get('type') == 'workflow_orchestration':
                            ...
                            print(f"   🎯 Result: {classification.title()} workflow planned")
                            ...
                            if classification == 'complex':
                                ...
                                if preview:
                                    ...
                                    for j, step in enumerate(preview[:3], 1):
                                        print(f"      {j}. {step}")
                                    if len(preview) > 3:
                                        print(f"      ... and {len(preview) - 3} more steps")
                            
                        else:
                            print(f"   💬 Direct response: {result.get('type', 'unknown')}")
