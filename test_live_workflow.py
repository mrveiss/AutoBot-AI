            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if result.get('workflow_id'):
                        ...
                        print(f"      Classification: {workflow_response.get('message_classification', 'unknown')}")
                        print(f"      Agents: {', '.join(workflow_response.get('agents_involved', []))}")
                        print(f"      Steps: {workflow_response.get('planned_steps', 0)}")
                        print(f"      Duration: {workflow_response.get('estimated_duration', 'unknown')}")
        ...
        try:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    print(f"   Step {current_step + 1}/{total_steps} - {progress:.0f}% - {workflow_status}")
