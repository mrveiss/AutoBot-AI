...
mkdir -p "$CONFIG_DIR" "$LOGS_DIR" "$DOCS_DIR" "$STATIC_DIR" "plugins"
...
sudo apt update && sudo apt install -y make build-essential libssl-dev zlib1g-dev \\
  ...
  libxml2-dev libxmlsec1-dev libffi-dev liblzma-dev git
    ...
    echo "📦 Installing Xvfb and x11vnc for GUI automation..."
    sudo apt-get update
    sudo apt-get install -y xvfb x11vnc || { echo "❌ Failed to install Xvfb or x11vnc."; exit 1; }
...
# --- 4. Create OpenVINO venv and install dependencies ---
OPENVINO_VENV_PATH="$VENV_HIGHER_DIR/$OPENVINO_VENV_NAME"
if [ ! -d "$OPENVINO_VENV_PATH" ]; then
    echo "📦 Creating OpenVINO venv: $OPENVINO_VENV_PATH..."
    "$HOME/.pyenv/versions/$PYTHON_VERSION/bin/python" -m venv "$OPENVINO_VENV_PATH" || { echo "❌ Failed to create OpenVINO venv."; deactivate; exit 1; }
else
    echo "✅ OpenVINO virtual environment $OPENVINO_VENV_PATH already exists."
fi

echo "📦 Installing OpenVINO dependencies into $OPENVINO_VENV_PATH..."
source "$OPENVINO_VENV_PATH/bin/activate" || { echo "❌ Failed to activate OpenVINO venv."; deactivate; exit 1; }
pip install openvino-dev || { echo "❌ Failed to install openvino-dev."; deactivate; exit 1; }
deactivate # Deactivate OpenVINO venv
