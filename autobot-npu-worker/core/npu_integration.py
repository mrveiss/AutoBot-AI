...
Extracted to autobot-npu-worker/core/ in Phase 2+ refactoring (Issue #4311).
"""
...
# Add backend path for imports when running from npu-worker context
...
try:
    ...
    # Fallback when running in npu-worker context without backend imports
    class LLMDefaults:  # type: ignore
    ...
    class TimingConstants:  # type: ignore
        STANDARD_DELAY = 1
        SHORT_TIMEOUT = 30
    ...
    def get_service_url(service_name: str) -> str:  # type: ignore
        return f"http://localhost:8001/{service_name}"
