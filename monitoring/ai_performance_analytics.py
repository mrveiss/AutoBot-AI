...
class AIPerformanceAnalytics:
    ...
    def __init__(self, redis_host: str = "172.16.168.23", redis_port: int = 6379):
        ...
        self.analytics_data_path = Path("/home/kali/Desktop/AutoBot/logs/ai_performance")
    ...
    async def initialize_redis_connection(self):
        """Initialize Redis connection for AI metrics storage."""
        try:
...
if npu_devices:
    ...
    print("NPU_NOT_FOUND")
            ...
            if "NPU_FOUND" in openvino_check.stdout:
                ...
                return NPUMetrics(
                    ...
                    operations_per_second=1000.0 / inference_latency if inference_latency > 0 else 0,
    ...
    async def monitor_knowledge_base_search(self, query: str, search_type: str = "semantic") -> KnowledgeBaseMetrics:
        ...
        try:
            ...
            relevance_score = 0.85 + (len(query) * 0.01)  # Higher score for longer queries
    ...
    async def monitor_llm_performance(self, request_data: Dict) -> LLMPerformanceMetrics:
        ...
        try:
            ...
            processing_time_sim = (input_tokens * 0.5 + max_tokens * 1.0) / 1000  # seconds
            await asyncio.sleep(min(processing_time_sim, 2.0))  # Cap at 2 seconds for simulation
        ...
        try:
            ...
            if self.performance_history["npu"]:
                ...
                trends["npu"] = {
                    ...
                    "efficiency_score": min(statistics.mean(npu_utils) / self.performance_targets["npu_utilization"]["optimal"], 1.0)
                }
        ...
        try:
            ...
            if npu_metrics:
                report["hardware_efficiency"] = {
                    ...
                    "npu_memory_usage": (npu_metrics.memory_used_mb / npu_metrics.memory_total_mb * 100) if npu_metrics.memory_total_mb > 0 else 0,
                    "inference_performance": "excellent" if npu_metrics.inference_latency_ms < 100 else "good" if npu_metrics.inference_latency_ms < 300 else "needs_optimization"
                }
    ...
    async def main():
        ...
        if args.test:
            ...
            if npu_metrics:
                ...
                print(f"NPU Inference Latency: {npu_metrics.inference_latency_ms:.1f}ms")
